...
角色协作走**主对话身份切换**——切换角色 = 切换 checklist + 强制重读，不 dispatch Subagent。
...
**第一段 push**（在 worktree 内 sanitize → push 分支 → CLI 创 MR）→ ⏸️ 用户在平台合并 → **第二段 finalize**（在主工作区验证合并 + worktree 清理 + state.json 标 completed）。
...
完整红线条文见 [SKILL.md](./skills/teamwork/SKILL.md) 与 [docs/v8-redesign/00-MANIFESTO.md](./skills/teamwork/docs/v8-redesign/00-MANIFESTO.md)。
...
当前 **v8.0**。完整变更记录见 [skills/teamwork/docs/CHANGELOG.md](./skills/teamwork/docs/CHANGELOG.md)。
