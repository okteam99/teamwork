...
[English](./README-EN.md) · Version: **v8.276**（版本单源 = [SKILL.md](./skills/teamwork/SKILL.md) frontmatter）
...
| **质量盲区**（做坏东西） | 自评盲区是数学性的：同模型看不见自己的坑 | 多角色视角切换 + **隔离冷审**（第三视角独立采样 · 跨模型异质可选升级） |
...
- **第三视角独立 Review**：评审引入独立第三视角（默认同模型 subagent **隔离冷审** · fresh session 独立采样揭露自评盲区）· 可 opt-in 升级**跨模型异质**（`disable_external_review: false` · claude 主窗口时 external = codex · 反之亦然）
...
# 小改动（轻量 Feature · roster/clarity 自动缩仪式）
...
| 技术方案 | 等 | RD 起草 TECH + QA 起草 TC + 架构师 + 第三视角评审 |
...
## 流程类型 — 该走哪条（闭集 · v8.220-223 收缩）

机器层：`flow_type ∈ {Feature, Bug}` + Feature 重量档 `preset ∈ {full, micro}`。轻量工作由**动态评审 roster + clarity** 维度承担（不再有独立「敏捷」类型）；legacy 名（「敏捷需求」「Micro」）只作别名自动映射。
...
- **`auto_mode`**：AI 代你完成 **stage 间流转** —— 只对「用户决策」类暂停点（如 PRD / UI 确认）代你接受 + 文档化（写 `concerns WARN` 留痕），**评审工作（多角色 + 第三视角隔离冷审）照常真跑**。
...
🔴 **yolo 不是「简化 / 提速」，而是「加重审核」**：无人值守 = 没人在看 → 自动化评审是唯一安全网，必须保留 / 加重、**绝不削弱**。三视角评审（架构师 + QA + 第三独立视角）全真跑、一个不少；🔴 v8.204：第三视角**默认 = 同模型 subagent 隔离冷审**（external 异质默认关 · 省 CLI 冷启动）· 跨模型异质 = opt-in 升级。零 stop **只**针对人工决策点（prepare / pm_acceptance / MR merge）；评审角色、第三视角冷审（默认）/ 真调异质（opt-in · 实跑日志物化校验）、测试轮次**一个不少**。失败 / 卡点 / 重试耗尽 / bypass 由 AI **自主解决**（优先级:解决 > 绕过；bypass 仅作穷尽后兜底 + WARN 留痕，`bypass_log` 频率 = yolo 健康度）。
...
- **第三视角 Reviewer**：独立第三视角代码评审（默认同模型隔离冷审 · opt-in 跨模型异质 codex / claude · 立场独立性硬约束）
...
- **第三独立视角（External）**：独立 review（默认同模型隔离冷审 · opt-in 跨模型异质 · claude 主窗口时 external = codex），第三视角必跑一次
...
- 评审引入独立第三视角 review（默认同模型隔离冷审 · 可选跨模型异质），独立采样揭露同模型自评盲区
...
| **R2** 流程类型闭集 | `{Feature, Bug}` + preset `{full, micro}`(v8.220-223)+ 不进状态机的 Feature Planning / 问题排查 · 禁止自创变体 |
...
| [STAGES.md](./skills/teamwork/STAGES.md) | 13 stage 索引 + 通用 cite 纪律 |
...
当前 **v8.276**（版本单源 = [SKILL.md](./skills/teamwork/SKILL.md) frontmatter）。变更记录见 [docs/CHANGELOG.md](./skills/teamwork/docs/CHANGELOG.md)（最近 5 版）· 更早历史走 git 提交历史（CHANGELOG-ARCHIVE **定期清空**）。
