...
[English](./README-EN.md) · Version: **v7.3.10+P0-23**
...
默认 OFF（v7.3.10+P0-9 决策）。开启后（在 `.teamwork_localconfig.md` 配置）Plan Stage 入口为 Feature 创建隔离的 worktree，Dev Stage 在 worktree 内开发，Ship Stage 合并回主分支。适合多个 Feature 并行开发 + 主分支保持稳定的场景。
...
**v7.3.10 + P0 系列**（当前，累计 23+ 个 P0 补丁）：
- **P0-23** Prompt Cache 友好改造（R1+R2+R3：动态内容后置 + 入口 Read 顺序固定 + state.json ≤5 次/Stage）
