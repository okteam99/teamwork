...
> 🎚️ **本 stage 默认执行档**(用户拍板 · 档位单源 [SKILL § subagent](../SKILL.md)):micro 的准入白名单已经把改动性质卡死成**零逻辑变更**(文案/样式/资源/配置常量/注释),但它仍是**把东西写出来**,不是「照着已有清单核对」—— 按定档判据(核对 → 验证档 · 写出来 → 执行档 · 要想出清单上没有的 → 深度档)**归执行档,不归验证档**。零门禁 ≠ 零成本意识:**门槛最低的 stage 没有理由烧深度档**。
