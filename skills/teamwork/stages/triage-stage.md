...
- PMO 自报宿主 + `command -v {cli}` 候选 CLI 可用性（v7.3.10+P0-72 删探测脚本）
...
Step 4: 用户原始消息                              ← 🔴 最后，动态入口（L3）
        + PMO 自报宿主 + `command -v` 候选 CLI 检查（v7.3.10+P0-72）
```
...
### 🔴 Step 1.5: 意图轻重分流（v7.3.10+P0-81 新增 · pull 模式根本改造）

> 实证触发：用户「看下 aon-link 是否需要 argocd 创建项目」→ PMO 走 20+ 工具调用读 init-stage / SKILL / flow-transitions / triage-stage / pmo / KNOWLEDGE / teamwork_space / ADR INDEX / localconfig 一堆框架文档 · 1 分多钟才开始 grep 实际代码。
> **根因**：triage 默认 push 模式 · 不分轻重一律前置全量扫描。
> **修复**：意图分流前置 → 轻型走 pull · 重型走 push。

🔴 **Step 1 用户承接后立即识别意图轻重**（先于 Step 2 KNOWLEDGE 扫描），二选一：

**轻型意图（pull 模式 · 跳过 Step 2-7）**：
- 关键词命中其一：「看下 / 看一下 / 看看 / 调研 / 解释 / 怎么 / why / 为什么 / 是否需要 / 是否应该 / 给我看 / 是否符合 / 分析下 / 检查下 / 定位 / 排查」
- 用户**没有**修改 / 实现 / 修复 / 创建 类动词
- 用户**没有**指定具体 Feature 编号 / 需求文档 / 已存在 PRD 等"已锁定流程"信号
- 满足以上 → **轻型** → 进入 Step 1.6 Pull 路径

**重型意图（push 模式 · 走完整 Step 2-8）**：
- 用户明确要"做 / 实现 / 修复 / 改 / 创建 / 优化 / 加 / 删 / 重构"
- 用户给出明确需求规格（不是问题 · 是需求）
- 用户继续推进进行中的 Feature
- → **重型** → 走 Step 2 KNOWLEDGE 扫描 + 后续完整链路

**判定不明（safe failover）**：默认走**轻型**（轻 → 重升级成本低 · 重 → 轻已浪费的工具调用回不来）。轻型答完后用户可显式说"按 Feature 流程走"主动切重型。

🔴 **轻型意图禁止动作**（违反 = 流程偏离）：
- ❌ 读 init-stage.md / SKILL.md / flow-transitions.md / triage-stage.md / pmo.md 等框架文档（已在主对话上下文 · 不重复 read）
- ❌ 全量扫 KNOWLEDGE / ADR（按需 pull · 在 grep 命中具体 Feature / 技术决策时再 read 对应段）
- ❌ 角色可用性扫描 / 外部模型探测（轻型不出代码 · 不需要评审角色）
- ❌ 创建 Feature state.json
- ❌ 输出 4 选 1 流程确认暂停点

🔴 **silent execution 硬规则（v7.3.10+P0-98）**：轻型意图判定**不在主对话输出**任何 trace。具体禁止（详见 [standards/output-tiers.md § 反模式 5](../standards/output-tiers.md)）：
- ❌ 「Step 1.5：意图轻重分流」标题
- ❌ 「用户消息：「...」」echo
- ❌ 「关键词命中：「看下」+ 「报 502」」逐项 trace
- ❌ 「→ 轻型意图（Pull 模式） → 跳过 Step 2-7 框架仪式」路由叙事
- ❌ 「按需直查：定位 X 路由实现 + ...」计划叙事
- ✅ 直接进 Step 1.6 grep 实际代码 · 不输出判定过程

### Step 1.6: Pull 路径（轻型意图直接执行 · v7.3.10+P0-81 新增 / +P0-98 silent execution 强化）

🔴 **PMO 直接 grep 用户消息中的关键词找实际代码** · 给初步答案：

```
1. grep / Glob 用户提到的关键词（如 "aon-link" / "argocd"）→ 找入口文件 / 配置 / 模块
2. Read 找到的关键文件（≤5 个 / ≤500 行 · 与 PRD 起草前代码 Read 同标准）
3. 给初步答案（**不暂停** · 直接输出 · 不输出"流程步骤描述"）
4. 按需补充 read：
   ├── 答案涉及"是否做过 X" / "是否有历史决策" → pull KNOWLEDGE.md 相关段（仅相关 · 不全扫）
   ├── 答案涉及具体架构决策 → pull ADR INDEX 检查相关 ADR
   └── 答案涉及多子项目协作 → pull teamwork_space.md 相关行
5. 答案末尾加跟进引导：
   "如果你想动手 {改/做/修}，请回复'按 Feature 流程' / 'Bug 流程' / 'Micro 流程' / '敏捷需求'，我会切到完整 triage 走前置仪式。"
```

🟢 **Pull 模式核心原则**：先看实际东西 → 缺 context 时再 read。**不做 reactive push 全扫**。

🔴 **silent execution 硬规则**（v7.3.10+P0-98 强化 · 单源 [standards/output-tiers.md § 反模式 5](../standards/output-tiers.md)）：

```
✅ 正确：直接 grep / Read / Bash 工具调用 · 中间过程不输出 · 直接给答案。

❌ 禁止（实战 case 触发）：
- 「Step 1.6 Pull 路径执行」标题
- 「按需直查：定位 X 路由的实现 + ...」计划叙事（→ 直接 grep 就好）
- 「找到核心链路。现在查 SVC-PLATFORM 上游...」进度叙事（→ 多个工具调用之间不解释）
- 「为了找到 X · 我先 grep ...」工具调用前缀解释

主对话只有 1 类输出：**实际答案**（用户问什么 → 答什么 + 跟进引导一句话）。
```

### Step 2: KNOWLEDGE 扫描（v7.3.10+P0-22 · v7.3.10+P0-81 改为重型路径专用）

> 🔴 **执行条件**：仅 Step 1.5 判定为**重型意图**时执行。轻型意图跳过本 Step（Pull 路径按需补充 read · 不全扫）。
...
### Step 3: ADR 索引扫描（v7.3.10+P0-21 · v7.3.10+P0-81 改为重型路径专用）

> 🔴 **执行条件**：仅重型意图时执行。轻型意图跳过（Pull 路径按需 pull · 仅在 grep 结果命中具体技术决策时才 read 相关 ADR）。
...
🟢 **v7.3.10+P0-81 跳过条件**：Step 1.5 判定为**轻型意图**时，本 Step 整体跳过（轻型不创建 Feature · 不需要角色 · 不需要外部评审）。
...
external 角色（PMO 直接判定 · v7.3.10+P0-72 删脚本）：
  ├── PMO 自报当前主对话宿主（基于自身运行环境，不读项目目录标记）
  │   ├── Claude Code 主对话 → host_main_model = "claude-code"
  │   ├── Codex CLI 主对话 → host_main_model = "codex-cli"
  │   ├── Gemini CLI 主对话 → host_main_model = "gemini-cli"
  │   └── 通用 / 无法识别 → host_main_model = "unknown"
  ├── bash `command -v {cli}` 检查候选 CLI 是否在 PATH
  │   ├── command -v codex
  │   ├── command -v claude
  │   └── 未来加候选只需新增一行
  ├── 应用 E1 同源约束识别异质 CLI：
  │   ├── 主对话 = claude-code → 找 codex
  │   ├── 主对话 = codex-cli   → 找 claude
...
- external 是否在内取决于异质性匹配 + CLI 可用性
- `command -v` 检查失败（CLI 不存在）→ INFO + external 不在 available_roles，**不阻塞 triage**
...
### Step 6: 跨 Feature 冲突检查 + 跨项目依赖识别 + 变更归属检查（v7.3.10+P0-33 新增 · v7.3.10+P0-81 改为重型路径专用）

> 🔴 **执行条件**：仅重型意图时执行。轻型意图跳过（不创建 Feature · 不触发跨 Feature/项目依赖检查）。
...
| 「📜 相关 ADR」段 | 历史决策扫描（如适用） |
| 「🌐 外部模型判定」段 | PMO 直接判定渲染（v7.3.10+P0-72） |
| 「流程类型识别」段 | 六种之一 + 理由 |
...
| [standards/external-model.md](../standards/external-model.md) | Step 4 PMO 直接判定规范（v7.3.10+P0-72 PMO 自报宿主 + `command -v`） |
