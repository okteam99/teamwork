...
🔴 **请求确认前先 emit「重点 review 指引」**(v8.137 · 用户重点 review 的导读 · 镜像 v8.120 流程目标的校准定位):固定 6 节 · **首节冷审逐条**(每条 NEEDS_REVISION finding 一行)· 余 5 节 ≤2 行 · 🔴 **全部照实抄自既有落盘产物 · 不即兴总结不美化不 collapse** · 空节显式写「无」(证明查过 · 不可省节):

```
📌 重点 review 指引(导读 · 不替代 PRD)
- 冷审逐条(🔴 **每条 NEEDS_REVISION finding 一行 · 全列 · 禁 collapse 成「全部 ADOPT」**):
  · `[QA] <一句话问题 · 通俗易懂>` → ✅ADOPT:<一句话改了啥> | 🔴REJECT:<一句话理由> | 🟡DEFER:<延后到哪>
  · `[PL] …` · `[Arch] …`(逐条 · 源 `findings[].pm_response`)
  🔴 **重点抽查 🔴REJECT / 🟡DEFER**(AI 判断替代了 reviewer 建议 · 抽查 ROI 最高 · 无则写「全 ADOPT · 无替代」)
...
🔴 **冷审逐条放第一节 · 全列不 collapse**:reviewer 每条 NEEDS_REVISION 发现 + 处置都**一句话列全**(用户看得到「冷审实际抓了什么 + 怎么处理」· 不藏在「全部 ADOPT」一句里)· 其中 🔴REJECT/🟡DEFER = AI 判断替代了 reviewer 建议 · 标重点抽查(ROI 最高)。六节来源全部是已落盘结构化数据(`findings[].pm_response` / `PL-CHALLENGE` / `revision_history` / `§Out of Scope` / PM 自查 / `§待决策项`)—— 脱离产物自由发挥 / collapse 成一句 = 违规(同 PROCESS-LEDGER「照实抄」纪律)。
...
- 🔴 重点 review 指引反模式(Substep 8):复述 PRD 全文 / 营销式总结(「本 PRD 经多轮评审已完善」)/ **多条 finding collapse 成「全部 ADOPT」一句**(藏掉 reviewer 实际发现 · 必逐条一句话列)/ 省略空节 / 内容无产物出处 —— 指引是导读不是宣传 · 冷审逐条 + 余节照实抄 · 空节写「无」
