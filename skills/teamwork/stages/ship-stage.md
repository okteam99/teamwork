...
- **然后**:`state.py ship-finalize --feature <path> --planning-artifacts <逗号分隔相对路径> --archive-desc '<≤50 字描述>'` → state.py 把这些文件随 {归档 zip + 删目录 + 终态 state.json} **同一收尾分支**暂存(并还原工作树 HEAD 防 step7 ff-pull 冲突)→ 你创建并合并**一个**收尾 MR(归档 + 规划翻牌原子)→ 重跑 ship-finalize 续清理 worktree + 主分支 pull。
  - **`--archive-desc`(v8.94)**:给本 feature 一句 **≤50 字**极简描述 · 写进归档 `_archive/INDEX.md` 的「描述」列(便于日后不解压识别)· 超 50 字自动截断 · 缺省 `—`。
...
2. step 5 把整个 feature 目录打成 `features/_archive/<id>.zip`(arcname=`<id>/...` · 自描述)· 追加 `_archive/INDEX.md` 一行 · 并在收尾 commit 里 **删除 feature 目录的所有 tree 条目** · push 到 `ship-finalize/<id>` 分支 → emit `PENDING`(交接 AI 创 MR + 自动合 · 同 §12)。
   - **v8.94 INDEX 描述列**:`INDEX.md` 表为 `| Feature | 描述 | 交付归档时间 | 归档物 |`。「描述」= AI 经 `--archive-desc '<≤50 字>'` 给的极简 feature 描述(超 50 字截断为 49+`…` · `|`/换行净化 · 缺省 `—`)· 便于日后不解压就识别归档内容。旧 3 列 INDEX 行下次归档时自动迁移补 `—`。
