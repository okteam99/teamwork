...
> 🧭 **四段结构**(v8.284 标准 · 详 [STAGES.md §3](../STAGES.md)):目标 + 硬规则白名单 + 手段菜单 + 产物契约 · 手段 AI 自选。
...
3. 🧾 **退役/替换类改造:按口径分张台账 · 总数不作验收门**(v8.299)—— 调用面 / 构造面 / 约束面各一张(口径定义 + 逐文件清单 + 每文件处置)· 🔴 **不同口径的总数不可互相印证**(why:实证中 TECH 与 TC 各算出「35 处/7 文件」并都自称与 PRD 吻合,实为两个互斥 population;真实是 25/6、71/6、18/5,且第三张是评审才发现的 —— **数字碰巧接近比对不上更危险,它让人以为核过了**)。
...
6. **两路并行同发 · 互不喂对方产出 · 🎭 模型错开**(外审路 ≠ 主审路 · v8.268)(why:同模型双路 = 盲区相关 · 错开是零成本的近异质)。
...
🔴 **变更点明细是本暂停点的正文 · 不是可选附件**(v8.242 实证:只给「增加诊断投影与快照序号」类分类概括 + 指针 → 用户被迫追问「DB 变更方案是什么」= 暂停点白 emit 一轮)。两类都未命中 → 跳过。
