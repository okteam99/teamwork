...
1. **工程规范 = `standards/tech-rules.md`(三时点唯一必读 · 起草重点 §三 方案与架构门)+ 项目 `project-specs/DEV-RULES.md` + `ARCHITECTURE.md` 的并集 · 🔴 冲突以项目为准**(DEV-RULES 不存在则只读前者;方案与规范冲突 → 要么改方案、要么在 TECH 显式记原因)(why:项目主权高于框架缺省;起草时读的就是 review 会查的 —— 门与预告同源)。
2. **§现状基线 grounded 真实代码**(不靠假设)· **§依赖与影响改契约必 grep 消费方**(不凭记忆)· **§查询性能与数据量涉批量读取必给理由**(🔴 **查询 + 搬运两段都算** —— 查询快 ≠ 这一步快;「调用方要多少 vs 搬回多少」差 ≥2 数量级 = 架构问题;兜底〔超时/分页/上限〕不算「够快」的理由;「够快」必须带规格)(why:三者都是「写得出但没核实」的高发区 —— 对着假设设计是扩已有页 / 改已有契约类事故的上游;而**只问查询侧的性能表会全绿放行**:实证一次报表 BQ 查询 1s 完成、索引分页全命中,45MB 结果集搬回 0.2 核服务端聚合 → 线上 13–19s)。
...
5.5 🔁 **验证轮范围锁定**(Round 2+ · 协议单源 [review-stage 规则 5](./review-stage.md)):只核实上轮 finding 的处置 + **修订处的连带影响** · ❗ **禁全量重扫**(新 finding 仅限出自修订处、或 BLOCKER 级并附「为何首轮未发现」)· rejected 不得复提(除非新证据)(why:TECH 与 PRD 一样是**纯文档评审** —— 方案类 finding 没有代码可回读证伪,Round 1 找完真问题后仍要「找新」,剩下能找的只有边角 · 多轮幻觉的高发处)。
...
8. 🧾 **冷审清单统一 · N 路都过全清单**(lane 标识只决定产物落点,不决定查什么)· 三段缺一即漏 · 段记 `coverage: [...]`:
   - ⚔️ **对抗**(形式与数量**单源** [SKILL § 冷审清单三段](../SKILL.md)):这个方案是不是最简形态?有没有为不会发生的场景加结构?TECH 里哪条约束是「我加的」而非需求要求的?
     🔴 **守卫必要性**(判据一句话):**这道守卫是需求要求的,还是我加的?**「我加的」→ 要么说出「不加会出什么事」**且那件事不会被别的机制拦住**,要么删。❗ **先问冗余再算 ROI**:「删了行为会变吗」答**不变**(后面必然有另一道拦同一件事)= **纯冗余**,不进 ROI 表直接删 —— ROI 对纯冗余也会算出「保留」。why:**防护读起来永远像更负责任**(同「限制读起来永远像最小范围」)—— 冷审的 mandate 是找缺陷,而多一道无害断言不是缺陷,实证一份 TECH 26 条冷审全处置、双路 APPROVE,冗余守卫仍要用户追问两次才挖出来。
   ...
   - 💡 **清单外洞察**:清单没问、但你认为该关注的 ≥1 条,或显式「无 + 为什么没有」· **不为凑内容而写**
   (why:物化门 `cross_review_coverage` —— 申报是冷审没退化成泛谈的最低物证;三段分开是因为**对抗项混进中性核对就会退化成打勾**)。
9. ⏸️ **方案要素确认(条件暂停点 · 见 ④)**:TECH 涉 **DB 数据结构变更** 或 **🛡️ 兜底与守卫清单非空**(含 DB CHECK/约束/trigger/前置断言/常驻门禁 · 不止 fail-safe) 或 **🚦 功能生效闸非空**(用户拍板:生效闸与 DB 变更同级) → blueprint-complete 前必 emit R5 暂停点等用户拍板(why:schema 与兜底是「高影响且不可轻易回退」的方案要素 · ROI 账与拍板权都归用户 · 不许默默做)。
9.5 🔴 **功能优先 · 复杂度守恒**(TECH/TC 冷审 finding 准入 · 单源 [review-stage 规则 2.5](./review-stage.md)):方案真缺陷(不可实现 / 契约错 / 关键路径缺 / TC↔AC 断链)**必报**;不推防御式设计与预防性抽象、不泛加测试门(TC 分层已有 `ci_reason` 门)· 修复代价 > 缺陷危害 → REJECT 合法;「更简方案达成同等功能」永远合法。
...
```markdown
⏸️ TECH 方案要素确认(DB 变更 / 兜底策略 / 🚦 功能生效闸 · 命中项如下)· 请确认:
...
**🛡️ 兜底与守卫清单**(TECH 含兜底**或守卫**〔DB CHECK/约束/trigger/前置断言/常驻门禁〕时必给 · 无则省略 · **照抄 TECH §兜底与守卫清单**原样贴出 · 含 💬 大白话列与「删了行为会变吗」列)。

**🚦 功能生效闸**(TECH §功能生效闸 非空时必给 · 无则省略 · **照抄原样贴出**)—— 每条一行:`闸 | 默认值 | **不满足时用户看到什么** | PRD 承诺的是什么 | 谁能解开`。
🔴 **重点念出「不满足时用户看到什么」与 PRD 承诺不一致的那几条** —— 那是**产品决策伪装成技术细节**:实证事故里 PRD 要的是「诚实展示:可以 partial/unavailable」,TECH 落成「没配齐就整页 503」,**功能上线了但用户根本看不到**。「别报假数」≠「宁可什么都不给」。

12. **确认方案 · 进入 dev** 💡 推荐   2. **调整方案**(对表结构/字段/迁移/兜底/生效闸有异议 → RD 修订 TECH)   3. **其他指示**
