...
9. ⏸️ **方案要素确认(条件暂停点 · 见 ④)**:TECH 涉 **DB 数据结构变更** 或 **🛡️ 兜底清单非空** 或 **🚦 功能生效闸非空**(用户拍板:生效闸与 DB 变更同级) → blueprint-complete 前必 emit R5 暂停点等用户拍板(why:schema 与兜底是「高影响且不可轻易回退」的方案要素 · ROI 账与拍板权都归用户 · 不许默默做)。
