...
🔴 §数据模型 必明确标注:本方案**是否涉及数据库数据结构变更**(新建/删除/修改 表、字段、索引、约束、migration)· 涉及 → 触发 §7.5 用户确认暂停点。🔴 **变更最小化先问**(v8.255 · 设计时前移 · 不是确认时补):每个新表/新列过四问(复用既有表列 / 应用层计算 / 不入库 / 并入 JSONB 扩展列)· 全否才入变更表 · 每项带「解决什么问题 + 为何非更简方案不可」(templates/tech.md §变更表清单)—— DB 变更数是简洁性 counter-lens 的重点审查对象。
...
```markdown
⏸️ TECH 方案涉及数据库数据结构变更 · 变更点如下 · 请确认:
