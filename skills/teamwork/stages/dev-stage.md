...
| 可配置点 | 默认值 | 控制字段 | 决策时机 |
|---------|-------|---------|---------|
| `execution` | main-conversation（v7.3.10+P0-14 默认）| `state.planned_execution.dev.approach` | Stage 入口 AI Plan 模式决策 |
| Subagent dispatch 触发条件 | TECH 文件数 >10 / 产出 >500 行 | spec 内嵌 | Stage 入口判断 |
| 懒装依赖检测 | Stage 入口必做 | spec 内嵌（dev-stage Step 0）| Stage 入口 |
| build 硬门禁（v7.3.10+P0-25）| 启用 | spec 内嵌 | Stage 出口 |
| 模型偏好（v7.3.10+P0-40）| Opus（AI Plan 模式默认）| `state.planned_execution.dev.model` | Stage 入口 |
| auto-commit 触发 | 强制（output_satisfied=true 前）| spec 内嵌 | Stage 出口前 |

🔴 不变内核：TDD 红绿循环 + RD 自查（roles/rd.md 自查段）+ 单测全绿 + build 通过 + commit message 格式约束。
...
**Dev Stage 特定 Tier 应用**：

- **Tier 1（永远输出）**：5 行 Execution Plan / TDD 红绿循环关键节点（首次跑测红 / 实现后跑测绿）/ build 失败 / 自查关键 verdict（PASS / FAILED）/ auto-commit hash
- **Tier 2（命中折叠）**：依赖懒装命中（首次安装时一行）/ 跨子项目改动文件清单（仅 >5 文件时）/ 自查 finding 摘要（仅 PASS_WITH_CONCERNS 时）
- **Tier 3（不输出，走 state.json）**：每个 commit 的 diff 详情 / 单测列表 / build 命令完整输出 / files_changed[] 详细 / dispatch 配置详细字段

---

### Stage 入口 Preflight：懒装依赖（P0 新增）

> 🟢 **P0 懒装依赖模型**：Feature worktree 创建时不装依赖（见 goal-plan-stage.md）。进入 Dev Stage 第一件事是检测依赖并按需安装。
...
- [ ] **Build：`npm run build` / `next build` / `go build ./...` / `cargo build` 等 exit 0**（v7.3.10+P0-25 新增硬门禁；如项目无 build 步骤——纯库 / 纯脚本 / Python 应用——必须在 dev-report.md 显式标注 "无 build 步骤"，不能省略）
- [ ] 无 TODO/FIXME：`grep -rE "TODO|FIXME" {变更文件}` 为空（除非标注"out-of-scope"）
- [ ] 代码实际落盘（不是只在对话中展示）
- [ ] 测试文件与 TC.md `tests[]` 对应

> 🔴 **Build 必须 RD 阶段跑通，禁止依赖 CI 兜底**（v7.3.10+P0-25）。CI 是最后一道安全网，不是第一道发现机制。Build 失败 = Dev Stage 不能完成 = PMO 拦下来不让进 Review。
>
> 🟡 worktree 场景：lazy install (P0-3) 模型下，单测能跑不代表 build 能跑——build 工具链 deps 可能未在 worktree 安装。处理方式见 [standards/common.md § 验证证据](../standards/common.md)。
...
- 上述所有机器校验 exit 0
- RD 自查通过（含验证证据）
- 出口走 `tools/state.py satisfy-gate --stage dev --gate output --auto-commit $HASH [--artifacts a,b]` + `complete-stage --stage dev`（v7.3.10+P0-128 物化 · 物理拦截 gate 顺序 / 三 gate 全满足 / artifact_root 写边界）
...
🔴 **v7.3.10+P0-40 默认 Opus 模型**：RD 开发涉及业务代码 + TDD + 复杂逻辑判断，模型默认 `opus`（不可降级到 sonnet）。
- 主对话模式：继承用户会话模型（如非 Opus，建议用户切换；关键 Feature 强烈推荐 Opus 主对话）
- Subagent 模式：dispatch 时显式指定 `model: opus`（Claude Task tool 的 model 字段 / Codex agent toml 的 model 字段）
- 例外：RD 在 Bug 流程的初步排查（BUG-REPORT.md 排查段）属执行型分析，可用 Sonnet
...
### Worktree 集成（PMO 执行，v7.3.8 改为校验存在；创建已前移到 Goal-Plan Stage 入口）

```
前提：worktree=auto 时 worktree 已在 Goal-Plan Stage 入口创建（见 stages/goal-plan-stage.md §Worktree 集成）
...
├── 前移到 Goal-Plan Stage 后，PRD/UI/Blueprint 所有产物都在 feature 分支
...
> 🔴 **单源（v7.3.10+P0-63）**：完整 TDD RED-GREEN-REFACTOR 5 步流程 + Iron Law + 自检清单 + 反模式 + ≥3 次失败升级规则见 [standards/tdd.md](../standards/tdd.md)。本段仅保留 Dev Stage 特有的"UI 还原集成"补充。

**TDD 5 步流程**：见 [standards/tdd.md §二](../standards/tdd.md)（RED → VERIFY RED → GREEN → VERIFY GREEN → REFACTOR）。

**Dev Stage 特有补充**：

```
Step 5b（如有 UI）: 按「UI 还原权威层级」实现 + 做「UI 还原自检」
└── 详见 §3 UI 还原（有 preview 时必做 / 与 TDD 5 步并行）
```

#### 2.1 开发约束

> 🔴 **TDD 强制要求 + 反模式禁令** → 见 [standards/tdd.md §一](../standards/tdd.md) Iron Law + [§四](../standards/tdd.md) 反模式 + [§五](../standards/tdd.md) 例外条款。

**Dev Stage 特有约束**：

```
🔴 强制要求（Dev Stage 特有 · TDD 通用规则见 standards/tdd.md）：
├── 禁止遗留 TODO/FIXME/占位符
├── 禁止输出不完整的代码片段
├── 所有文件直接写入项目目录
