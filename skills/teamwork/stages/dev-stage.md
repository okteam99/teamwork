...
1.7 🎛️ **主对话 = Orchestrator(默认姿态 · 用户拍板)**:**不建议在主对话(主循环)直接进行成块开发** —— 主对话优先用做编排:**任务拆解 · 阶段规划 · 子代理调度 · 集成接线(契约层)· 提交/推送 · 验证门禁 · 小型精准修改**;成块实现派 subagent(worktree 内路径 · 执行档继承会话模型不降档 · Meta 首行申报)。派发判据沿用:子任务边界清晰且够大才派 —— 小 / 耦合 / 强串行的改动主对话直接做(即「小型精准修改」出口 · 派发协调开销反拖慢)(why:主对话 context 是最稀缺资源 —— 跨 stage 编排状态、用户拍板记忆、集成全景全活在这里,灌满实现细节 = 编排记忆被挤出;且 Orchestrator 姿态让并行成为默认而非事后补问)。
