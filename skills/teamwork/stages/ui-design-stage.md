...
- **`same-stack`**(v8.56):在 `{子项目}/docs/design/preview-project/` 搭**同栈独立项目**(自带目标库 + mock data · `src/Preview` 渲染各 page)· `npm run build` → `{子项目}/docs/design/preview/<page>.html`(可视全景 · **必产**)· UI.md `pages_changed[].panorama_file` 链到 `docs/design/preview/<page>.html` · 本 Feature **引入新库**时 preview-project 独立装该库(解鸡蛋问题)· 🔴 **验证渲染用 `python3 -m http.server` 起本地服务再 browse 截图**(`file://` 在 browse 里不加载 · 治本 CW-F002 误判"渲染正常")
