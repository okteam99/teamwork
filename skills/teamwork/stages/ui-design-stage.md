...
- **`same-stack`**:后台跑 `bash {子项目}/docs/design/preview-project/preview.sh` → 把 `PREVIEW_URL=` 那行的 URL 给用户 browse(dev server 实时 · 动态端口 · 不在 teamwork 层起 server · 详 § 预览)
