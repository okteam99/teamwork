...
- 🔴 **分层同构反模式**(same-stack):新设计页渲染在 `/` 顶掉首页 / 省略 router / 路由与 sitemap 不一致(IA 层);基建层不走共享包而自造组件·自配主题·版本漂移(Layer 1);全景页缺意图四要素(Layer 2);拿停更全景页纠正已 ship 页面(权威倒置);改共享基建不验全景编译(下游契约 · dev-stage §3.5)
