...
## preview dev 工具面板(🔴 右下角悬浮 · 非顶栏 · 只放页面到不了的 · 真实交互优先 · 页面=真代码)

> 🔴 **真实可达的交互 → 页面内做成真实交互(默认)**(治本:别把按钮→Drawer 这种真实交互也赶到工具面板 · 那样预览不反映真实交互路径):真实 app 里**点页面按钮能到的** —— 新建/编辑 **Drawer · Modal · 行→详情 · 页内导航/Tab · 配置弹窗** —— 在 preview 里就做成**真实可点**(点页面的「新建模型」就开对应 Drawer · 同真实 app)。这正是「设计=代码」要的**交互保真**。
>
> 🔴 **位置 = 右下角悬浮面板(v8.206 · 非顶栏)** —— 治本(顶栏反模式):**顶栏是 layout bar · 会把真实页面内容整体下推、offset 掉真实位置/尺寸**,恰恰违背 same-stack「**零预览痕迹 · 页面=真实代码**」的核心目标(真实 app 没这条顶栏 → 加了预览就不像真实 app)。**悬浮面板是 overlay · 不占布局流 · 不 shift 真实页面**(页面在真实位置/尺寸渲染)· 右下角 = dev 工具通行惯例(devtools / toolbar 都在角落 · 用户一眼识别「这是预览工具不是产品 chrome」)· 🔴 **可折叠**(收成小角标 · 不挡内容)· 半透明/低视觉层级不抢戏。
>
> 🔴 **工具面板只放「页面入口覆盖不到的」**(preview-project 的 dev 外壳 · 真实 app 不存在):
> - **页面区(Prototype Nav)**:每个全景页一个入口 → `route_path`(直达 · 与 sitemap 一致)· 🔴 页间跳转若真实 app 有**页内入口**(导航/链接/按钮)则**页内优先** · 面板只兜底无页内入口的页。
> - **状态区**:**真实交互无法自然触发的态** —— `Loading / Error / Empty / 边缘态`(没有「让我看 loading 态」的真实按钮)+ 难自然触发的展示态 / preset 直开某特殊态(评审便利)。🔴 **能在页面内点出来的态不放这里**。
>
> 🔴 **「禁内嵌 switcher」澄清**(修订 v8.169 过度收口):禁的是**预览专属控件**(真实 app 没有的 state-switcher 下拉 / 场景 toggle 嵌进页面)· **不是禁真实交互按钮**(新建/编辑/详情那些真实 app 有的 · 该真实工作)。状态**注入**走**悬浮工具面板**(mock-data provider 按状态注入 · 页面组件不知自己在被预览 = Storybook args),但**交互可达性走页面**。
> 🔴 **不违 IA 镜像**:`/` 仍是真实首页 · 工具面板是**悬浮工具层**(overlay · 不占真实路由、不占布局流)。
