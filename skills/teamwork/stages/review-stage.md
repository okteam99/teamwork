...
> 📎 **本文件即架构师 + QA 在 Review Stage 的完整任务规范**（v7.3.10+P0-19-B 合并 agents/arch-code-review.md + agents/qa-code-review.md）。外部 Codex 视角单独走 codex CLI spawn，prompt 模板见本文件 §外部视角 Codex Review。
...
🔴 外部 Codex 视角独立 dispatch 时，Codex session 内不经过 state.json 入口流，只读 diff + PRD/TECH/TC + standards（保持独立性）。
...
│       ├── 第三方 SDK 调用是否有异常捕获？
│       └── 异步消息消费失败是否有死信队列或告警？
├── 🔴 三方/外部服务调用异常 ERROR 日志审查（硬规则，见 standards/backend.md「三方/外部服务调用异常 ERROR 日志规则」）
...
> 外部视角通过 codex CLI 独立 spawn fresh session 执行（详见 §Process Contract §4 外部独立审查）。本节为外部视角的 prompt 模板。
...
```
review-codex.md，frontmatter：
---
perspective: external-codex
executor: codex-cli-subprocess | codex-cli-subagent
