# Change Request 模板（CHG/BG · DEPRECATED）

> 🔴 **v8.49 DEPRECATED**：变更规划已统一为 **Workstream（WS）** —— 新规划一律用 [`templates/workstream.md`](workstream.md)（落 `product-overview/workstream/WS-NN.md`），不再新建 `changes/CHG|BG-NNN.md`。
> WS 吸收了本模板的"拆一组 feature + 跨子项目 launch_order + lock（未规划完成禁启动）"职责，并统一了**初始规划**与**变更规划**为同一产物。
> 老项目 `product-overview/changes/*.md` **向前兼容**（保留可读 · 不强迁）。本文件留作历史参考。
...
🔴 **核心约束**：`status != locked` 时**禁止启动**任何归属本变更的子 Feature；PMO 在 triage 时硬阻塞，仅允许"用户明确说强制启动"逃生舱。
...
## 与 teamwork_space.md 的关系

`teamwork_space.md` **完全不维护变更类信息**——本文件 `product-overview/changes/{change_id}.md` 是变更的唯一权威源（status / 简介 / 影响子项目 / 子 Feature / 推进顺序 / 联调依赖 / 锁定决策 / 变更日志全部在本文件 frontmatter + 正文�）。
