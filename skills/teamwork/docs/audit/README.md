...
## 一份审计记录含什么
- **实际数据**(工具确定性抽 · 不可幻觉 · 喂 kill-criteria 决策):来源项目 / flow / 实走 stages / 总时长 / **各阶段耗时**(stage_contracts.duration_minutes)/ **耗时分析**(阶段总和 + 最耗时阶段% · 与总时长差 = 阶段间等待)/ **主对话 host + 模型**(model 由 PMO ship-finalize `--main-model` 声明 · 缺省只记 host)/ concerns / bypass · 细数据指向该项目 `PROCESS-LEDGER.md` 行。
