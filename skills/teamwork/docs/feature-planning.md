...
- emit **拆解讨论稿**:候选 BL 清单(每条:一句边界理由 + `current_state` 摘要〔出自哪些实读文件〕)+ 波次草案 + **粒度自检**(🔴 反压:BL > 8 或存在「无独立交付价值/纯机械半天活」的 BL → 必须给「为什么不合并」;镜像 goal 的 AC>10)。
- R5 标准选项:1. 按草案拆 💡 / 2. 讨论调整(合并 X+Y / 砍 Z / 改边界 —— 收敛后再落)/ 3. 其他。**用户有异议就地讨论收敛 · 不落成品后返工**。
- `auto_mode/yolo`:按推荐继续 + `add-concern WARN "auto skip: 拆解讨论 · 草案摘要"` 留痕(同全景确认模式)。
