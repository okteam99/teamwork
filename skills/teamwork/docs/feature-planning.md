...
**复杂度触发**(关键词命中任何执行类流程时 PMO 必再扫 [prepare.md §2.1](./prepare.md) 的 5 项信号:跨独立 git 仓库 / 数据模型重构 / 老需求架构性废弃 / 影响 ≥2 BL / 方向级业务变更):
...
2. 继续讨论 —— 对 design system / 页结构 / 关键页有想法或疑虑,先把方向聊清楚(收敛后我改全景重出预览再确认)
3. 其他指示
```
...
- emit **拆解讨论稿**:候选 BL 清单(每条:🔴 **大白话目标**〔一句人话:**这条单独上线后,谁能干什么/得到什么** —— 写不出可感知目标 = 横切件,并回宿主 · 「后端接口就绪」「partner 侧改动」不算 · 落 WS `features[].goal_plain` · ws-progress 总览表直出〕+ 一句边界理由 + `current_state` 摘要〔出自哪些实读文件〕+ **含金量标注**〔真新增工程量 vs 薄配套:承接/枚举/配置级〕)+ 波次草案 + **粒度自检**(🔴 反压:BL > 6 或存在「无独立交付价值 / 纯机械半天活 / 按评审面横切」的 BL → **必须给「为什么不合并」**;默认姿态 = 合并 · 镜像 goal 的 AC>10 反压)。
- **边界判据 = 交付内聚(唯一主判据)**:一个 feature = **能独立交付的一刀**(合起来才有用户价值的东西不拆开)。🔴 **默认合并 · 拆分是例外** —— 每一刀都要说得出「**为什么这两件不能一起交付**」,说不出就并回去。
  - 🔴 **不按评审面拆**(用户拍板):**代码在不同子项目 / 前后端分属 / 改动面大不好评审 —— 都不是拆分理由**。按评审面横切出来的件各自**不能独立上线**(前端等后端、后端没人用),徒增 feature 数与协调成本,而评审总量并没变少。
    若内聚单元确实大到评审吃不消:① 找**更小的内聚切片**(仍是端到端可交付的纵切 · 不是横切),或 ② 接受多轮评审(review 收敛协议管这个:severity 门 / 验证轮 / 轮次预算)· **不要**为了好评审把不能独立交付的东西拆开。
  - **薄承接件默认并入宿主件**(只承接另一件的产物 / 同 surface 严格串行 / 跨件协调点可内化为件内里程碑)。**含金量悬殊 = 强合并信号**(几件薄件工程量加起来不如一件实体)。
  - 保持独立的硬理由**只有三类**:**外部依赖 gate**(等三方 · 不绑架宿主交付)/ **交付节奏不同**(如运营慢周期 · 上线时点本就分开)/ **管辖边界**(不同团队拍板)。
- **id 纪律**:草案期编号随便改;**落盘后合并/砍件不重排幸存 id** —— 被并件留一行遗迹(`S2 → 已并入 S1`)· 缺号不补(重排 = 整卷重写、易漏引用)。
- R5 标准选项:1. 按草案拆 💡 / 2. 继续讨论(合并 X+Y / 砍 Z / 改边界 / 方向疑虑 —— 聊清楚收敛后再落)/ 3. 其他。**用户有异议就地讨论收敛 · 不落成品后返工**。
...
- 🔒 **规划期用户拍的板逐件落到 `features[].user_decisions`**(🔴 **原样搬不润色**):讨论里用户**选了什么**(如「税务方案 A 不是 B」「打款 failed 分两态」)· **排除了什么**(并标「做不到」还是「我们选的边界」)· **定了什么口径**。未经用户单独拍板填 `[]`(`ws-lint` 查槽位在不在 · 不查非空 —— 强制非空会逼出编造的决策)。
  🔗 **链条**:这些事实是起 feature 时 `init-feature --user-intent` 等参数的**输入**,再落进 PRD §已确认意图。🔴 **断在这里的代价**:规划讨论只活在当时的对话里,换 session / 派 subagent 就没了 —— 下游 PM 只能凭「范围」一句去猜用户当初要什么,而**意图错了是唯一一类下游全部质量门都拦不住的错**。
- 🔴 **拆解明细宁详勿略**:§拆出的 feature 是下游**唯一**能拿到规划期上下文的地方 · AC 可以写到具体字段/路由/状态值(不必留到 PRD 才写细)· 该节由本流程产出,**不是「人维护」**。
...
规划产出(WS + 各 ROADMAP 登记 + preview-project/sitemap if 涉 UI + 业务架构 if 改)是 **Step 0 worktree 内未提交的改动**。规划完成 → **必 emit R5 暂停点问用户如何收尾** —— 不擅自 commit / 合并,也不放任改动悬着。🔴 **选项 1 / 3 = 一步到位**(收尾不该是「建 MR → 等你告知已合并 → 再收尾」的多段接力 · 第 2 项恒为继续讨论):
...
2. 继续讨论 —— 对拆分结果 / 收尾方式 / BL 启动顺序还有想法,先聊清楚再收
3. 确认 · 合入收尾 + 启动首个 BL <BL-xxx> —— 同 1 · 收尾完直接 prepare 首波 ready BL(execution_waves W1)进 Feature 流
4. 建 MR · 我自己平台 review 再合 —— commit + push + 开 MR → 你平台合(或我 `await-merge` 30s 轮询)→ 合后收尾
5. 先不提交(继续调整 / 你稍后自己提)
6. 其他指示
```
...
PMO(主对话):
  ...
  → state.py init-feature --flow-type Feature --feature-id <PROJ>-F<NNN>-<name> --bl BL-007 ...
    (🔴 `--bl` 推荐必带:写 state.json.bl · ship 翻牌/ws-progress 优先读它 · 不再单靠 ROADMAP 手填)
