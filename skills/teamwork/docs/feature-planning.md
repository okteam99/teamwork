...
- emit **拆解讨论稿**:候选 BL 清单(每条:🔴 **业务交付物**〔用户/调用方视角一句:**这条单独上线后,谁得到什么** —— 写不出可感知交付 = 横切件,并回宿主 · 「后端接口就绪」「partner 侧改动」不是交付物〕+ 一句边界理由 + `current_state` 摘要〔出自哪些实读文件〕+ **含金量标注**〔真新增工程量 vs 薄配套:承接/枚举/配置级〕)+ 波次草案 + **粒度自检**(🔴 反压:BL > 6 或存在「无独立交付价值 / 纯机械半天活 / 按评审面横切」的 BL → **必须给「为什么不合并」**;默认姿态 = 合并 · 镜像 goal 的 AC>10 反压)。
