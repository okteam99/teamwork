...
- 🔴 **调研深度契约(v8.239)**:调研产出必须能回答「每个候选 BL 的 `current_state` 出自哪些文件」—— current_state **必附来源文件路径**(ws-lint 抓占位/缺失 = 调研浅硬信号)· 浅调研拆出的 WS 必散。
- 🔴 **退役/替换类 BL:current_state 必增记「测试痕迹规模」**(v8.299 · 一次 grep 即得:引用该机制的**测试文件数 + 调用点数**)—— why:**退役一个机制的真实成本主体常常是测试改写**,规划期只按「新增能力」估工作量会**系统性低估 3~5 倍**(实证:某次退役的注册机器由三个历史 feature 建成,测试痕迹散在 13 个文件 114 个点,规划期无人估过这部分)。
...
- emit **拆解讨论稿**:候选 BL 清单(每条:一句边界理由 + `current_state` 摘要〔出自哪些实读文件〕+ **含金量标注**〔真新增工程量 vs 薄配套:承接/枚举/配置级〕)+ 波次草案 + **粒度自检**(🔴 反压:BL > 6 或存在「无独立交付价值 / 纯机械半天活 / 按评审面横切」的 BL → **必须给「为什么不合并」**;默认姿态 = 合并 · 镜像 goal 的 AC>10 反压)。
- **边界判据 = 交付内聚(唯一主判据 · v8.292)**:一个 feature = **能独立交付的一刀**(合起来才有用户价值的东西不拆开)。🔴 **默认合并 · 拆分是例外** —— 每一刀都要说得出「**为什么这两件不能一起交付**」,说不出就并回去。
  - 🔴 **不按评审面拆**(v8.292 用户拍板):**代码在不同子项目 / 前后端分属 / 改动面大不好评审 —— 都不是拆分理由**。按评审面横切出来的件各自**不能独立上线**(前端等后端、后端没人用),徒增 feature 数与协调成本,而评审总量并没变少。
