...
5. ✅ **INIT.md CLAUDE.md 模板更新** — 已适配（红线 R8 条+8-stage 引用 flow-transitions.md）
...
配套：红线 R1 ("PMO 非 Micro 流程下不得改代码") 开了 Ship 例外条款，允许 PMO 解 git 冲突标记（仅消除 marker，解完必须单测全绿）。

### 问题

1. **职责越界**：PMO 是"流转守门员"，现在变成"代码合入执行者" —— 做 rebase / merge / 解冲突都是代码层决策
2. **红线 R1 被稀释**：绝对红线开例外条款，信任边界表达弱化
...
**红线 R1 简化**：移除 Ship 例外条款。"PMO 非 Micro 流程下不得改代码" 回到真正的绝对红线，不再维护例外清单。
...
v7.3 早已放宽：FLOWS.md §六「Micro 流程」+ SKILL.md 红线 R1 Micro 例外 + SKILL.md L122-126「AI Plan 模式 Micro 例外」三处明确「PMO 可直接改代码，无需 Subagent / Execution Plan / dispatch」。但其他 7 个文件未同步更新：
...
- **与用户意图错位**：红线 R5 已强制每个 ⏸️ 暂停点必须输出 💡 推荐 + 📝 理由，用户回复 ok 在"有 💡"的上下文中 ≈ 100% 是"按推荐走"意图。但规范要求 PMO 复述 + 二次确认，给用户额外打字负担
- **多决策点摩擦放大**：用户想"两项都按推荐"要打 `1A 2A`（5 字符），而 ok 只需 2 字符
- **规范自相割裂**：红线 R5 投入成本让 💡 一定出现，但 💡 的语义价值未被充分使用——💡 只是"参考建议"，而不是"可以被 ok 直接引用"的编号

### 根因

旧规则的设计语境是"防止无上下文的 ok 被误解"。但红线 R5 本身已经建立了上下文（💡 明确存在于每个暂停点），ok 不是孤立语句而是对 💡 的引用。旧禁令属于在过强约束下二次保险，代价是日常交互摩擦。
...
## 问题 19：agents/ 目录混合 Subagent 执行协议 + 角色任务规范，与 stage 级权威层级产生双重归档（2026-04-23 新增 · P0-19）

### 问题诊断

v7.3.9+P0-14 dual-mode 化后，RD / 架构师 / QA 的主对话 / Subagent 两种执行模式契约一致，但规范文件仍按"subagent 归档"组织：
- `agents/rd-develop.md` — RD TDD 开发 + UI 还原 + 自查
- `agents/arch-code-review.md` — 架构师 Code Review
- `agents/qa-code-review.md` — QA Code Review
- `agents/integration-test.md` — 项目集成测试
- `agents/api-e2e.md` — API E2E 验收

问题：
1. **文件名暗示"Subagent 专属"**：PMO 在 Plan 模式选 `main-conversation` 时不确定是否仍需加载这些文件 → 心智负担
2. **Stage 契约和角色任务规范分居两处**：`stages/dev-stage.md`（Input/Process/Output）+ `agents/rd-develop.md`（角色任务细节）→ PMO 派发时要引用两条路径，漏读风险
3. **Stage 级权威层级被稀释**：v7.3 改造的核心思想是"一个 Stage 一个契约"，但角色任务规范停留在 agents/ 破坏了这一理念
4. **`agents/` 混合了两类性质不同的文件**：`README.md`（Subagent 执行协议：dispatch / Progress / 通用约束）+ `{role}.md`（具体角色任务规范）→ 目录语义不纯粹

用户反馈原话："从合理的方向看，是不是弱化 subagent, 强调 stage, 增加各个 stage 中的规范文档，因为执行层面 pmo 可以按需选择 subagent"。

### 优化方案：P0-19-B Stage 升格 + agents/ 物理合并

**核心：Stage 升格为权威层级，Subagent 降格为执行选项。**

处理（对应 CHANGELOG v7.3.10+P0-19 §P0-19-B）：
1. 物理合并 5 个 agents/*.md 到对应 stages/*.md
   - `agents/rd-develop.md` → `stages/dev-stage.md §RD 角色任务规范` + §RD 执行输出模板
   - `agents/arch-code-review.md` → `stages/review-stage.md §架构师 CR 任务规范`
   - `agents/qa-code-review.md` → `stages/review-stage.md §QA CR 任务规范`
   - `agents/integration-test.md` → `stages/test-stage.md §集成测试任务规范`
   - `agents/api-e2e.md` → `stages/test-stage.md §API E2E 任务规范`
2. 物理删除 5 个 agents/*.md 文件，`agents/` 仅保留 `README.md`
3. 瘦身 `agents/README.md` 为纯 Subagent 执行协议（6 个顶级章节：执行方式与模型 / 通用 Subagent 执行约束 / Codex CLI 调用规范 / PMO 启动规范 / 主对话产物协议 / 目录结构索引）
4. 迁移 16 处活引用（SKILL.md / RULES.md / roles/rd.md / roles/qa.md / templates/* / codex-agents/*.toml）指向新路径
5. 保留 CHANGELOG / OPTIMIZATION-PLAN 历史引用不改写
6. SKILL.md frontmatter version 7.3.10+P0-18 → 7.3.10+P0-19

### 预期收益
- PMO 心智负担 -50%：派发 / 切换角色 / 主对话执行从"同时引用 stages/ + agents/"简化为"只引用 stages/"
- Stage 文件自包含：Input Contract → 角色任务执行流程 → Output 模板串联在同一文件，阅读上下文连贯
- Subagent 回归本位：`agents/README.md` 只负责「跨宿主派发 + 可观测性」，不再承担规范归档
- Plan 模式纯粹化：approach 只影响执行方式，不影响"读哪个规范"
- 目录语义清晰：`stages/` = 业务契约 + 角色任务规范（权威）；`agents/` = Subagent 执行协议（手段）

### 兼容性（非破坏性）
- 所有规范内容原样迁移，仅物理位置变动 + 章节编号微调
- Subagent dispatch 协议（dispatch 文件 / INDEX / Progress Log）完全不变
- `.teamwork_localconfig.md` 的 `teamwork_version` 缓存会在下次启动自动捕获 7.3.10+P0-19，触发一次 CLAUDE.md / AGENTS.md 校验（漂移则自愈）
- 用户侧无任何行为变更

### 优先级

P0（用户反馈，架构清理，非破坏性向前兼容）
