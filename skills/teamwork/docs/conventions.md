...
> 各 stage spec / state.py / templates / SKILL.md § Triage 入口规范 一律以本文件为单源引用。
...
示例: SVC-PLATFORM-F260601143012-Offer-Ranking   (Feature · utc 默认)
      ...
      PTR-M003-Footer-Copyright-Year             (存量 legacy Micro · 新 init 产 F · preset=micro)
```
...
- **名称**:多词用 `-` 拼接 · 不超过 6 词
- **目录**:`{docs_root}/features/{artifact ID}/`(完整 ID · 不省略名称 · 各 flow 同放 `features/` 下)
...
- **BL → F 升级时机**:用户拍板某 Backlog 启动 Feature 流程时,PMO 分配下一个 F-NNN(各项目当时 sequence 的下一个)+ `init-feature --bl BL-NNN`(🔴 推荐必带 · 写 state.json.bl · ship 翻牌/ws-progress 优先读)+ 同步回填 ROADMAP「对应 F编号」列
...
```bash
# triage 阶段(PMO 主对话 · SKILL.md § Mode B 必移交 prepare 子流程):
...
| 面 | 症状 | 纪律 |
|---|---|---|
| 依赖目录(node_modules / venv) | worktree 内无依赖 · 首次必装 | 装进 worktree 内;或软链主树**只读用 · 用完即删** —— 别指望自动共享;worktree lazy-install 踩坑:单测能跑但 build 找不到工具链 = 只装了部分 deps → `npm install --include=dev` 补装 |
...
## 12.48 临时产物目录(scratch)(单源 · 迁自 standards)
...
## 12.49 迁移文件命名与起号纪律(多 worktree 并行形态约定 · 单源)

> Schema 变更的**方案门**(影响分析 / FK 决策)在 [standards/tech-rules.md §三](../standards/tech-rules.md);本节只管**写迁移文件那一刻**的命名与起号。
...
- **统一位置**:worktree 模式(缺省)= `<worktree>/.teamwork-scratch/screenshots/`(ignored · 不进 commit/diff · 随 worktree 消亡);off 模式 = `${TMPDIR:-/tmp}/teamwork/<feature_id>/screenshots/` —— scratch 根通则与回收三通道详 [§12.48](#1248-临时产物目录scratch单源--迁自-standards)。
  ```bash
...
🔴 **`.teamwork-local-env/` ≠ `.teamwork_localconfig.json`**:前者是**你的** secret(kubeconfig/密码/key · gitignored)· 后者是 **teamwork 自己**的配置(worktree/scope/id_strategy/max_review_rounds 等 · 🔴 **本机级 · bootstrap 自动 gitignore**——`_bootstrap` 段含机器态〔host/maintain 时间〕不宜入库 · 团队要共享档位靠口头约定各机自配)。
...
- [docs/feature-planning.md § Step 7](./feature-planning.md) — ROADMAP 起草时 BL-NNN 分配
- [stages/goal-stage.md](../stages/goal-stage.md) — Feature ID 已在 init-feature 时确定
- [stages/ship-stage.md § 6](../stages/ship-stage.md) — ship-finalize 清理 worktree
