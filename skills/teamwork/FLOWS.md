...
## 问题排查梳理流程（v7.3.10+P0-30 简化 · v7.3.10+P0-81 默认 pull）
...
> 🟢 **v7.3.10+P0-81 进一步**（pull 模式根本改造）：triage Step 1.5 判定为**轻型意图**（看下/调研/解释/why/是否需要 等）时 → 直接进 Pull 路径（grep 实际代码 → 给初步答案 → 按需补 read KNOWLEDGE/ADR）· **不前置全量扫**框架文档 / KNOWLEDGE / ADR / 角色 / 外部模型。详见 [stages/triage-stage.md § Step 1.5/1.6](./stages/triage-stage.md)。
...
**🔴 PMO Bug 判断必须输出（每次都必须，禁止省略 · 下游：QA 读判断块决定是否走简化流程 / 完整流程 · 缺判断块 → QA 必须打回 PMO 重做 · 实证 case：4.6 自承"跳判断块直接进简化流程"被用户对抗 · v7.3.10+P0-146）**：
...
> 下游：跳判断块 → 用户无法看到流程依据 · 后续 Stage（QA/RD/架构师）无 reference 文件可 cite · 导致评审退化 · 实证 case 用户对抗后被迫补判断块（v7.3.10+P0-146）。

**🔴 Bug 流程合规确认（PMO 判断完简单/复杂后必须输出 · 下游：架构师 CR 入口校验"必经阶段"清单 · 缺合规确认 = state.review_substeps_config 不完整 · CR 退回 PMO 重出 · v7.3.10+P0-146 加 consumer）**：

```
📋 Bug 流程合规确认
├── 完整阶段链：{列出当前流程的所有阶段，含架构师 Code Review}
├── 🔴 必经阶段（禁止跳过）：架构师 Code Review · 下游：跳 Code Review = state.review_substeps_config.review_roles[] 缺 architect 项 · Ship Stage finalize gate 校验失败 · exit 1
└── 确认：按完整阶段链执行，逐项完成
```
...
> 下游：豁免后走敏捷 / Micro 流程跳过 PRD / 评审 / TC → Ship Stage finalize 时 PR diff 暴露真实改动规模 · 评审组合不齐 · 用户对抗（实证 4.6 case："我跳"写了没人读"的步骤"）。v7.3.10+P0-146 加 consumer。
...
```
PMO 分析 → ⏸️ 用户确认走 Micro（含流程步骤描述）→ 主对话 PMO→RD 身份切换（Read rd.md + standards/*.md + cite）→ RD 改动 → RD 自查 → ⏸️ 用户验收 → Ship Stage 第一段（commit + push + 创建 MR）→ ⏸️ 用户合 MR → Ship Stage 第二段（合入验证）→ ✅ 完成（v7.3.10+P0-74：补 Ship 双段）
```
...
⏸️ 用户验收（手测/目视确认）
    ↓ 用户确认通过
🔴 Ship Stage 第一段（v7.3.10+P0-74 新增）
    ├── PMO auto-commit（micro: {简述}）
    ├── PMO push feature 分支
    └── PMO 创建 MR（标题 micro: {简述} · 描述含变更清单 + RD 自查摘要）
    ↓
⏸️ 输出 MR URL · 等用户在平台合 MR
    ↓ 用户告知"已合 MR"
🔴 Ship Stage 第二段：合入验证（v7.3.10+P0-74 新增）
    ├── git fetch origin {merge_target}
    ├── git merge-base --is-ancestor {commit_hash} origin/{merge_target}
    │   ├── exit 0 → 已合入 ✅
    │   └── exit 1 → 未合入（MR 关闭 / pending / 平台异常）→ concerns + 不进 ✅ 完成
    └── 通过验证 → 进完成报告
    ↓
PMO 完成报告（含 Micro 事后审计 + commit hash + merge_commit_hash + 已合入 merge_target 证据）
    ↓
Micro 需求完成 ✅
```
...
│   ├── [文件1]：[改动摘要]
│   └── [文件2]：[改动摘要]
├── 📋 流程步骤描述（v7.3 必填 / v7.3.10+P0-20 身份切换表述统一 / +P0-74 加 Ship 双段）：
...
│   4. 用户验收（手测/目视确认）
│   5. Ship Stage 第一段：PMO auto-commit + push feature + 创建 MR（标题 micro: {简述}）→ ⏸️ 等用户合 MR
│   6. Ship Stage 第二段：合入验证（git merge-base --is-ancestor）→ 通过则进完成报告
│   7. PMO 完成报告（含事后审计 + 自查摘要 + commit hash + merge_commit_hash + 已合入证据）
├── 阶段链：PMO 分析 → 用户确认 → PMO→RD 身份切换 + 加载规范 + cite → RD 改动 → RD 自查 → 用户验收 → Ship 第一段 → ⏸️ MR 合并 → Ship 第二段合入验证 → ✅ 完成
├── ⏸️ 请确认走 Micro 流程
└── ✅ 自检通过
```
...
├── 🔴 **身份切换第一人称锚点（P0-20-B）**：身份切换完成后的阶段摘要首句必须以「作为 RD，……」开头，作为身份锚点，防止中途漂回 PMO 口吻 · 下游：缺锚点 → AI 中途漂回 PMO 口吻输出"建议" / "应该"（不再以执行者视角行动）· 用户对抗时被迫重写 · 实证 P0-20-B trigger case（v7.3.10+P0-146 加 consumer）
...
│   └── 🔴 禁止在 RD 身份下直接接收新需求（防止身份蠕变、Micro 越扩越大）· 下游：违反 → Micro 范围突破白名单 · 准入条件事后失效 · Ship Stage finalize 时 PR diff 暴露 5+ 文件改动 / 引入新依赖 · 用户对抗时 PMO 已无法解释为何当时不升级（v7.3.10+P0-146 加 consumer）
├── 🔴 改动后必须执行 RD 自查（见 `roles/rd.md` 自查段）：至少规范符合 + 跑已有测试无回归 · 下游：跳自查 → 用户验收暴露规范违反 / 测试回归 · PMO 必须重做（loop 倒退）· 实证 4.6 自承"跳 RD 自查"被用户对抗（v7.3.10+P0-146 加 consumer）
