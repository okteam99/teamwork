...
> 隶属于 📎 [ARCHITECTURE.md](./ARCHITECTURE.md)，数据库层面的详细设计。
...
│
├── {子项目A}/                  # business 子项目 — {一句话职责说明}
│   ├── docs/                  #   子项目文档（PROJECT.md + features/ + architecture/）
│   └── src/                   #   子项目源码（内部按技术职能分层）
│
├── {子项目B}/                  # business 子项目 — {一句话职责说明}
│   ├── docs/
│   └── src/
│
├── {中台子项目C}/              # midplatform 子项目 — {一句话职责说明}，消费方：{消费方列表}
│   ├── docs/
│   └── src/
│
└── ...其他子项目
...
| 缩写 | 名称 | 类型 | 简介 | 承接执行线 | 技术栈 | 需要 UI | 消费方 | 项目详情 |
|------|------|------|------|-----------|--------|---------|--------|----------|
| AUTH | 认证服务 | business | [简介] | Line 1 | | 否 | - | [链接] |
| WEB | 前端应用 | business | [简介] | Line 1, Line 2 | | 是 | - | [链接] |
| PAY | 支付中台 | midplatform | [简介] | Line 1 | | 否 | AUTH, WEB | [链接] |
| INFRA | 基础设施 | midplatform | [简介] | - | | 否 | 全部子项目 | [链接] |
...
> 其他模块对本模块的依赖请求。本模块负责人应定期检查并处理。

## 请求列表

| 编号 | 请求方 | 请求方模块 | 依赖描述 | 优先级 | 期望完成时间 | 状态 | 处理说明 |
|------|--------|-----------|----------|--------|-------------|------|----------|
| DEP-001 | @张三 | WEB | 需要 AUTH 模块提供 SSO 回调接口 | 🔴 阻塞 | 2026-03-20 | ⏳ 待处理 | |
| DEP-002 | @李四 | ADMIN | 需要 AUTH 模块开放角色查询 API | 🟢 非紧急 | 2026-04-01 | ⏳ 待处理 | |
