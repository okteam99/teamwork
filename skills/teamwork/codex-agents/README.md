...
> session 启动时由 `tools/bootstrap.py` 自动部署到项目的 `.codex/agents/` 目录。
> 
...
| reviewer.toml | Review Stage | 架构师 CR ∥ QA 代码审查（并�） |
| tester.toml | Test Stage | 集成测试 ∥ API E2E |
| planner.toml | Goal Stage | PM PRD + PL-PM 讨论 + 评审 |
| designer.toml | UI Design / Panorama | Designer UI 设计 |
| e2e-runner.toml | Browser E2E Stage | 浏览器端到端测试 |
| **prd-reviewer.toml** | Goal Stage（PRD 交叉评审） | 外部视角审查 PRD.md（详见 templates/external-cross-review.md）|
| **blueprint-reviewer.toml** | Blueprint Stage（TC+TECH 交叉评审）| 外部视角审查 TC.md + TECH.md（详见 templates/external-cross-review.md）|

---

## 🔴 默认推理深度 + service_tier 配置
...
⏫ 调高（reasoning_effort = xhigh）：
 ├── 极复杂业务逻辑的关键决策点（如新业务线启动的 Goal Stage 评审）
...
📎 **历史 case**� 7 个 profile 没显式设置 `model_reasoning_effort` → fallback 到 codex 默认 `xhigh` → 用户实战中 codex 调用经常超时或表面"无响应"（实际仍在深度推理）。本次统一显式默认到 high 解决这一问题。
