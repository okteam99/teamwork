...
- [ ] QA 集成测试：✅ 已执行（附实际输出） / ⏭️ 用户确认跳过（原因：____）
- [ ] QA E2E 验收：✅ 已执行 / ⏭️ 子项目 E2E=否
...
```
PMO 提示 PM 在 PRD 中补充「消费方分析」章节（模板见 TEMPLATES.md）：
...
- **🤖 E2E 端到端验收**（子项目启用 E2E 时触发，规范：agents/qa-e2e.md）——AI 浏览器操作，从最终用户视角验证完整功能链路
- 输出实现完整性报告
...
**代码审查流程**（架构师 Code Review 通过后，QA 执行）:
...
**完成后**: 集成测试通过 → 自动进入 QA Lead 质量总结

---

## QA Lead (质量负责人)

**触发**: PMO 在集成测试（或 E2E）通过后自动启动
...
**审查维度**：
1. **验收标准覆盖率** — 逐条对照 PRD 验收标准，每条是否有测试覆盖（单元/集成/E2E）
2. **测试策略合理性** — 单元/集成/E2E 分层是否合理，是否过度依赖单层
...
│   ├── 是否有 RD 自查遗漏的性能隐患
│   ├── 是否有 RD 自查遗漏的安全风险
│   └── 🔴 并发写入安全（涉及数据库写操作时必审，详见 standards/common.md）：
