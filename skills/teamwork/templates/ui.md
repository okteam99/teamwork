...
> 🔴 **分层同构**(same-stack):全景页 = **意图权威**(四要素:布局结构 / 交互流 / 状态 / 字段映射 —— 即本文 body 各段 · dev 还原对照物 · 像素/代码组织自由)· 设计权威**至该页 ship 止**(此后代码即真相 · 全景页转历史参考);基建层(shell / 组件库 / 主题 / 架构)走**共享包**完全一致。详 [stages/ui-design-stage.md § 分层同构律](../stages/ui-design-stage.md)。
