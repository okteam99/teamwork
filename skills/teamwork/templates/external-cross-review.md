...
> 🟢 ：本模板原名 `codex-cross-review.md`，重命名为 `external-cross-review.md`。
> "外部模型"概念 + 候选清单 + 同源约束 + 调用规范 + PMO 运行时探测的权威规范见 [standards/external-model-usage.md](../standards/external-model-usage.md)。
> 本模板专注：checklist、Output schema、PMO 整合流程、成本治理（与外部模型实例无关）。

> 🔴 适用：Goal Stage（PRD）/ Blueprint Stage（TC + TECH）的前置阶段交叉审查。
...
| 流程 | Goal Stage（PRD）| Blueprint Stage（TC+TECH）|
|------|-----------------|--------------------------|
...
PMO 完整决策流角色契约见 [roles/pmo.md](../roles/pmo.md)。简版：
...
用户选择后 PMO 写入 state.json（完整 schema 见 [standards/external-model-usage.md](../standards/external-model-usage.md) §五）：
```json
...
**降级路径**（与 standards/external-model-usage.md §六 E3 规则一致）：
...
🔴 静默降级（不写 state.concerns）违反 R7 evidence 闭环红线（详 SKILL.md § PMO 软约束）。
...
| 文件 | 关联 |
|------|------|
| [standards/external-model-usage.md](../standards/external-model-usage.md) | 🔴 外部模型概念 + 候选清单 + 同源约束 + PMO 直接判定 + 调用规范的权威规范 |
| [stages/goal-stage.md](../stages/goal-stage.md) | Process Contract 已扩展为 5 视角（含外部模型） |
