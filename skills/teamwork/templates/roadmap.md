...
- **状态**：Feature 用 \`待开始\` / \`进行中\` / \`⏳ 等待外部依赖\` / \`已完成\`（同义词 \`已交付\` / \`已上线\` 工具同等识别）/ \`已取消\`；技术债用 \`待清理\` / \`清理中\` / \`已清理\`。状态词须写在状态格**起始位置**（可带前导 emoji/加粗），词表外写法 ws-progress 按「待开始」计并 emit \`unrecognized_status\` 警告（如「基本已完成」不算完成）
