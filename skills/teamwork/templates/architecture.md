...
| 子文档 | 内容 | 创建时机 |
|--------|------|----------|
| 📎 [database-schema.md](./database-schema.md) | 数据库 schema 设计、ER 图、核心表说明、分库分表策略 | 有数据库设计时(本模板附骨架)|
| 📎 api-design.md | API 总览、版本策略、核心接口清单、认证方案 | 有 API 设计时(无骨架 · AI 自拟)|
| 📎 deployment.md | 部署架构、环境拓扑、基础设施、CI/CD pipeline | 需要部署说明时(无骨架 · AI 自拟)|

> 🟢 **拆不拆、几时拆 AI 自决**(v8.293 去掉原「超 50 行必拆」的能力上限规则)—— 初期全写本文件也行,
> 拆了就在本索引更新链接即可。
