...
> - **必填核**(每个 Feature 都填):背景 / **已确认意图** / 用户故事〔业务类;refactor 可写"使用方故事"或省〕/ 交付预期 / 待决策项 / 验收标准 / Out of Scope —— 被 verify-ac / PM / dev / 冷审消费 · 或不可省的 what/why/scope/决策。
...
## 已确认意图（🔴 必填 · prepare 原样搬运）

> 🔴 **PRD 的脊,也是冷审唯一能拿到的用户原话** —— 起草不得偏离,冷审据此核对「有没有偏离用户要的」。**原样搬不润色**(润色 = 二次解释,偏差正是这么进来的);goal brief 已把 `state.confirmed_intent` 渲染好,照抄即可。用户中途改口 → **append「✏️ 修订」不覆盖**。

- 🗣️ **用户原话**（逐字）：{「…」}
- 🎯 **理解**：{要达成什么} · 🧩 **我补的假设**：{…／请求明确 · 无补}
- 📦 **范围**：做 {Y} · 不做 {Z} · 🔁 **既有行为**：{改「原 A → 现 B」／否} · ✏️ **修订**（有才写）：{原本 X → 改为 Y}
...
> 必须画图（任一命中）：用户操作 >3 步且有分支 · 涉 ≥2 系统/服务交互 · 有状态流转（订单/审批）· 有异步或回调。都不命中可省本节。
...
### API 契约（产品视角）
<!-- 只写产品视角:method/path、入参出参的业务字段含义、错误时用户感知、能力承诺。
     鉴权/限流/序列化/SLA/缓存/DB/status code/错误 schema 归 TECH.md 接口实现段。 -->

### 兼容性承诺
<!-- 对现有消费方:是否破坏现有接口、迁移方案 -->
...
| 用户说的词 | 我理解成 | 依据：用户说过（引原话）/ 🔴 我推的 | 🔴 若这条理解错了 → 最坏会怎样 |
|---|---|---|---|
| {词} | {我的解释} | {🔴 我推的 · 用户没说过是否含 …} | {具体后果：如「投放链接的点击全部不回传,广告优化拿不到转化数据」→ 生产/不可逆 → 已进 D-N} |
