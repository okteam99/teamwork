...
## 送审前自检(评审关注点前置 · v8.262 · 起草完先过 · 别等冷审打回)

<!-- 🔴 冷审要查什么 · 起草时就按什么写 —— 每项自查过再送审(治 Round 2+ 修订循环:finding 采纳率 80-90% 说明多数问题可预见 · 前置消掉最省)。人读 · 逐项打钩或写「N-A + 理由」。 -->

- [ ] **PL 六问自问过**:价值前提 / 问题定义 / 范围最小化 / 上游对齐 / 复活检查 / 既有行为变更 —— 每问能一句自答(答不出的 → 先补 · 别指望冷审替你想)
- [ ] **可实现自查**:方案技术可行?依赖的接口/字段/能力**真实存在**(读过真实代码 · 非假设)?复杂度与业务价值配比?
- [ ] **可验证自查**:每条 AC 可测(有明确判定动作与预期)?边界与异常路径写了?无「尽量 / 合理 / 优化 / 提升」类不可测含糊词?
- [ ] **高频 finding 预检**:术语都在 GLOSSARY 或文内定义?错误/失败路径有归宿?AC 之间无互相矛盾?
