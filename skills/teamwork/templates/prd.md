...
> 🔴 **只能主对话 PM 做,不可委托冷审**：冷审拿不到用户原话,只能核对「PRD 内部自洽」——而**范围被悄悄收窄时,PRD 是完全自洽的**。why 与实证单源 [goal-stage 规则 4.5](../stages/goal-stage.md)。
> ②排除项定性并进下方 §Out of Scope 的「性质」列（排除项已在那儿列过,不两处写）。

**① 术语解释对照** · 🔴 标「我推的」的行 = 偏差风险最高处,不确定 → 进 §待决策项

| 用户说的词 | 我理解成 | 依据：用户说过（引原话）/ 🔴 我推的 |
|---|---|---|
| {词} | {我的解释} | {🔴 我推的 · 用户没说过是否含 …} |
