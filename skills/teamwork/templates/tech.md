...
- 🛡️ **兜底默认不做**(v8.265 · 复杂度×收益):每个**降级兜底/安全兜底**(fallback / degradation / 重试熔断 / 防御层)逐项过——保护的失败场景**真实概率×后果** vs **实现与维护成本**,写不出必要性 → **删**(AI 天然偏加兜底 · 每层兜底都是复杂度)。

**🛡️ 兜底清单**(确需保留的逐项列 · **无则写「无兜底」** · 🔴 本清单随 §7.5 暂停点透出给用户拍板 · 不许默默做):
