...
| Feature | flow | 实走 stages | 时长(总·AI自主·待用户) | review/test 轮 | external 总/采/驳 | 角色真 finding | 暂停点 改:默 | bypass/WARN | 反思摘要(≤1 行) | 各阶段耗时 | 用户邮箱 | 宿主 | 分诊校准(预测→实际) | 🛡️ 起草可预防性(可预防/总·缺考虑点) | ⏱️ 耗时归因(协调开销轮/总轮·最大一笔) |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| <ID> | <Feature / Feature·micro / Bug> | <goal→blueprint→dev→…→ship> | <2.4h·AI 88m·待 32m> | <1/1> | <3/1/2> | <arch:1 qa:0 ext:1> | <1:2> | <0/0> | <external 拦 1 真问题 · ui_design 零 finding 过场> | <goal 20m(+等5m)·dev 40m·review 8m·pm_acceptance 30m> | <git user.email> | <claude-code/codex-cli/gemini-cli> | <explicit·goal→qa → diff 14 files·PRD 0 revision·review 1 轮> | <3/12 可预防·缺:并发时序> | <协调开销 2/9 轮·blueprint:TECH↔TC 双档同步·类型:双档同步> |
...
> 🔴 **⏱️ 耗时归因(v8.295)**:各 stage 收敛后跑 `state.py stage-cost --stage <goal|blueprint|dev|review|test|…> --rounds N --overhead-rounds M --kinds '双档同步;门禁重试' --note '最大的一笔开销是什么'` · ship 聚合照抄本列(数据源 = ship1 archive emit 的 `ledger_stage_cost`)。
> **为什么要它**:机器已采 duration / active_minutes,但那只有**数字**没有**归因** —— 「这 318 分钟花在哪」只有 stage 结束时当场记得住(ship 时回填要靠产物 mtime 反推)。实证 SVC-PLATFORM-F260726:blueprint 6 波往返里波 5、6 是纯文档对齐无设计价值,双档同步吃掉 ~35% 轮次 / ~25% token —— 这类事实机器算不出、事后也复原不了。
> **用途**:年检看**协调开销占比**跨 feature 趋势 + 开销类型复发;🔴 也是**验证提效改动是否真起效的唯一手段**(v8.294 的收敛期归一 / TC 边界 / 投机窗准入都声称能砍这块 —— 没有这列就无法证伪)。非门禁 · 没记录留空。
