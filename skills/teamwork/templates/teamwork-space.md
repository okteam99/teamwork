...
>
> 🔴 **核心简化原则**：teamwork_space.md 是**全景索引**，不是事件日志 / 进度看板 / 评审记录。**任一表格的任一单元格都应 ≤ 1 行**；详情一律外迁到对应文档（workstream/WS-NN.md / Feature 自己的 state.json + PRD / PROJECT.md / ADR）。这能让 teamwork_space.md 永远保持"一眼看懂全景"，避免演化到几百行难以维护。
...
| 文档 | 路径 | 规划状态 |
|------|------|----------|
| 业务架构与产品规划 | 📎 [\`product-overview/{项目名}_业务架构与产品规划.md\`](product-overview/)（愿景 + 执行线列表） | 📝 草稿 / 🔄 讨论中 / ✅ 已确认 |

> Workstream（规划单元）落 \`product-overview/workstream/WS-NN.md\` · 进度见下方 § 规划状态。
> 🔴 v8.49：执行手册已废弃（执行线移入业务架构）· 老项目 \`执行手册.md\` 向前兼容。
...
> 🔴 **项目现状统计（v8.49）= 「未完成 WS」(规划态 · 本表) + 各子项目「完成度」(执行态 BL · 见子项目清单)**。WS 转 \`✅ 规划完成\` 即从「未完成 WS」移除（feature 转 ROADMAP 跟踪 · 不双计）。
>
> 🔴 **硬规则**：每个槽位值 **≤ 1 行**。"最近规划"/"最近更新"只写「日期 + 一句话事件 + 链 WS-NN / Feature 路径」；多事件累积时**只保留最近一次**，旧事件移到对应 \`workstream/WS-NN.md\` 的「变更日志」段。

---

## 执行线概览（有 product-overview/ 时 · 可选派生视图）

> 🔴 执行线的**权威源是** `业务架构与产品规划.md` 的「执行线列表」(taxonomy)。本表是可选派生视图（同步自权威源 · 嫌重复可省，直接看业务架构）。
> 执行线不反向绑定子项目或 Feature 编号 —— **WS 在自己文档里 tag「承接 1+ 执行线」**，反查得映射（v8.49）。

| 执行线 | 使命（一句话） |
|--------|---------------|
| Line 1 · XXX | [取自业务架构执行线列表] |
| Line 2 · YYY | [取自业务架构执行线列表] |

> 🔴 **硬规则**：每格 **≤ 1 行**，原文取自业务架构（不复述背景 / 不加事件级补充 / 不出现子项目缩写或 Feature 编号）。
...
> - 转 ❌ 不做 → **从表删** · 不做原因可入对应 workstream/WS-NN.md 或仅留 git log
...
>
> **规划 / 阻塞 / 历史的单源**（v8.49 · WS）：
> - 🔴 **活跃规划（WS）**（含状态 / 拆解 / 承接执行线 / 影响子项目 / 拆出 feature / launch_order / 风险）→ [`product-overview/workstream/`](./product-overview/workstream/) 下每个 `WS-NN.md` 自己的 frontmatter + 「变更日志」段（schema 见 [templates/workstream.md](~/.claude/skills/teamwork/templates/workstream.md)）
> - 🔴 **当前阻塞** → 对应 `workstream/WS-NN.md` 的「风险与缓解」段 / 或单 Feature 的 `state.concerns`
> - 🔴 **Feature 级事件**（Stage 流转 / Ship / 评审 finding）→ 各 Feature 自己的 `state.json` + `review-log.jsonl` + git log
> - 🔴 **跨 WS 回溯查询**：`ls product-overview/workstream/*.md` / `grep -l "✅ 规划完成" product-overview/workstream/*.md`
>
> **Feature → WS 反查**：ROADMAP「关联 WS」列 + Feature `state.json` 反向引用所属 WS。
>
> 🔴 **核心硬约束（保留）**：WS 状态 != `✅ 规划完成` 时禁止启动归属本 WS 的子 Feature，PMO 在 triage 时硬阻塞（保留"用户明确说强制启动"逃生舱）。校验由 PMO 直接读 `workstream/WS-NN.md` frontmatter 完成。老项目 `changes/*.md`(CHG/BG) 向前兼容。
\`\`\`
...
阶段 2 · 架构规划（逐个子项目 Planning 或首个 WS 落地时）
├── 触发：用户确认开始子项目 Planning / 起 WS（进 feature-planning）
├── PM 在 Workspace Planning 中更新子项目清单、架构全景、各子项目 ROADMAP
├── 结果：⏸️ 用户确认后进入阶段 3

阶段 3 · 开发期（正常运转）
├── 子项目清单 + 项目架构全景 + 项目目录结构按结构变更同步更新
├── 变更 / 阻塞 / 事件历史一律落 product-overview/workstream/WS-NN.md 或 Feature state.json，本文件不再维护此类表格
└── 自上而下 / 自下而上双向更新
\`\`\`
