...
> 🔴 **业务术语主权威已迁到独立文档**：项目根 `GLOSSARY.md`（teamwork prepare-stage 自动创建空骨架）。
...
> **触发写入时机**：Goal / Blueprint Stage 评审中发现术语漂移 → 业务术语写 GLOSSARY.md / 实现层术语写本段（inline · 不批处理）。

### 通用架构词汇Ｔ mattpocock/skills improve-codebase-architecture · 8 词）
...
> 🔴 **业务实体关系主权威已迁到 GLOSSARY.md § 二**。本段保留仅供子项目内部模块关系（如 Service ↔ Repo 调用关系）补充。

### "删除测试"启发式Ｔ improve-codebase-architecture）
...
## 🔀 Flagged Ambiguities（已澄清的歧义 · 借鉴 grill-with-docs）
...
- **沟通**: PR-003
- **拒绝**: OS-001, OS-002
- **术语**: Module, Interface, Seam（通用架构词汇 · 项目内部实现层 · 业务术语主权威见 GLOSSARY.md）
- **歧义**: FA-001, FA-002

## ❌ Out of Scope（已拒绝过的方案/方向 · 借鉴 mattpocock/skills triage）

> 拒绝过的方案 / 方向 / Feature 候选——防止 AI 反复提同一个被否的方案。
> 触发写入时机：评审循环中明确 REJECT / PM 验收时拒绝某方向 / Goal Stage 讨论中确认"这个方向不做"。
...
🔴 **PMO 起草 Goal / 评审循环时必须先扫 OS-NNN 列表**：发现 PRD 草案重新提了被否方向 → 直接打回让 PM 改写或显式说明"为什么本次重新审视这个方向"（必须有新的触发原因，否则违规）。
...
- **Convention 写入时机**：
 ...
 - Goal Stage 用户强调格式要求 + 本要求跨 Feature 适用 → PM 记录 CV-NNN
...
- Preference：PR-NNN
- Out of Scope：OS-NNN
- Glossary：术语**直接用术语本身作为锚点**（Term 段加粗 · 不编号）
- Flagged Ambiguities：FA-NNN
- 🔴 编号连续不复用，归档条目保留原 ID
