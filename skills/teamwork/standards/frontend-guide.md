# 前端实施指南（frontend.md 各节的展开 · 按需查阅）

> 🔴 **定位**：本文件是 [standards/frontend.md](./frontend.md)（规范骨架 · must/must-not）的**实施指南层** —— 选型对比 / 配置样例 / 代码示例。**不默认加载**（subagent 按需查对应 § · 防上下文超载）。与规范冲突时以 frontend.md / tdd.md / common.md 为准。
> ⚠️ **历史损坏说明（v8.122 记录）**：本文示例代码曾在一次格式清理中丢失全部**空括号 `()`**（如 `jest.fn()` → `jest.fn` · `.focus()` → `.focus`）。箭头函数参数括号已批量修复；**其余空调用括号未逐例修复** · 读示例按语义补 `()` · 示例为指南性质非可直接运行代码。
