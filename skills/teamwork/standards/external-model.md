...
>
> 🟢 **适用范围（v7.3.10+P0-83 收紧）**：external 评审仅适用 **Blueprint Stage**（详细技术方案 · 出问题后续改动成本高 · 异质视角对架构 / 数据模型 / 接口设计盲区有补） + **Review Stage**（代码层最后 gate · 异质视角对实现盲区 / 第三方依赖真实性 / 并发安全价值高）。**Goal-Plan Stage 不适用**（PRD 文档层 · 5 内部视角 + PM 对抗自查 + 评审循环已多层兜底 · external 边际价值低 · 已删）。
