...
| 豁免场景 | 允许操作 | 约束 |
|---------|---------|------|
| **内部评审修复循环**（Blueprint Stage TC/TECH 评审、Review Stage fix 循环） | 每轮修复结束时 1 次 Write | 🔴 Blueprint 至多 3 轮 · **Review Stage 至多 5 轮**（v7.3.10+P0-139）· 对齐各 stage 修复上限规则 |
...
- [ ] `grep -E "今日|当前时间|YYYY-MM-DD" SKILL.md RULES.md stages/ roles/ templates/` 返回空（除非是占位符说明；stages/prepare-stage.md / triage-stage.md 含格式示例不在内）
...
- 🔗 [stages/prepare-stage.md Step 2](../stages/prepare-stage.md) — prepare-stage（v7.3.10+P0-106 重构后）的版本缓存机制（缓存"已校验状态"）与本规范（让 LLM prompt cache 命中）叠加使用，两套缓存层级互补
