...
> 🔴 **单源**：完整 TDD 自检清单（§三 9 条）+ 反模式（§四）+ ≥3 次失败升级（§六）见 [standards/tdd.md](./tdd.md)。本段不复制清单正文 · 提交前自检一律以 tdd.md §三 为准（避免双源漂移）。
...
> 🔴 **单源**：QA Code Review 的 TDD 规范检查（自检清单 §三 / 反模式 §四）见 [standards/tdd.md](./tdd.md)。本段不复制清单正文 · 一律以 tdd.md 为准（避免双源漂移）。TDD 检查为辅助手段，QA 核心是上方「TC 逐条验证」。
