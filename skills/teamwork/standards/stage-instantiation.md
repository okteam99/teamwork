...
3. 输出 Execution Plan（红线 R7 的 5 行核心）：
...
→ 🔴 **自动续接子步骤 1**（v7.3.10+P0-102 强化）：
   ├── 写完 state.json 之后必须立即开始子步骤 1 实际工作（QA 起草 TC / RD 起草 TECH / PM 起草 PRD 等）
   ├── 禁止插入"等用户 ok / 等 auto 继续"等假暂停
   ├── 禁止以"避免单回合溢出 / 让你看到 X 进度 / 为下回合留预算"为由人工拆分回合（roles/pmo-auto-mode.md § 十「容量焦虑暂停反模式」措辞黑名单）
...
🔴 **入口实例化 → 子步骤 1 自动续接硬规则**（v7.3.10+P0-102 新增 · SKILL.md 红线 R4 扩展）：
- 默认通道：写完 `<stage>_substeps_config` 之后**同一回合内**必须开始子步骤 1 实际工作 · 中间禁止插入任何形式的"等用户确认 / 等 ok / 等 auto"
- 标准通道：⏸️ 5 选 1 暂停点是 spec 标注的合法暂停 · 用户回数字后必须立即续接子步骤 1 · 禁止再以"容量"为由二次暂停
- 反模式（v7.3.10+P0-102 实战 case · 详见 roles/pmo-auto-mode.md § 十）：
  - ❌ "为避免单回合溢出 · 本回合在此暂停让你看到 X 进度 · 下一条 ok / auto 继续"
  - ❌ "TC + TECH 600 行单回合放不下 · 先停"
  - ❌ "本回合在此暂停 · 下一步开始"
- 物理拦截：含上述措辞 = 自创暂停 = 红线 R4 违规 · 必须删除暂停意图 · 自动续接

📎 与红线 R7（AI Plan 模式）的关系：本"入口实例化段"产出的就是红线 R7 要求的 5 行 Execution Plan + state.<stage>_substeps_config 写入，二者合一。
📎 与红线 R4（非暂停点禁止暂停）的关系：本规范定义"入口实例化 → 子步骤 1"是 🚀 自动流转节点 · 不允许插入暂停 · v7.3.10+P0-102 物理拦截"容量焦虑暂停"变种。
