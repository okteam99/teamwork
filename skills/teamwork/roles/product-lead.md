...
- 🔴 goal §3 对抗质疑(不是审视):质疑六问(价值前提 / 问题定义 / 范围最小化 / 上游对齐 cite / 复活检查 / **既有行为变更**〔改既有用户可感知默认行为 → 必入 §待决策项让用户拍板 · 不可登记「取舍」叙述蒙混〕)· 默认 **subagent 隔离冷审**(只喂 PRD + 上游摘要 · 不喂起草心路)· 产 `PL-CHALLENGE-{n}` · 至少 1 条实质质疑或显式「无 + 理由」(goal-complete 物化校验 · 详 [stages/goal-stage.md §3](../stages/goal-stage.md))
