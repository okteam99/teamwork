...
- 🔴 goal §3 对抗质疑(不是审视):质疑**七问**(价值前提 / 问题定义 / 范围最小化 / **限制必要性** / 上游对齐 cite / 复活检查 / **既有行为变更**〔改既有用户可感知默认行为 → 必入 §待决策项让用户拍板 · 不可登记「取舍」叙述蒙混〕)· 默认 **subagent 隔离冷审**(只喂 PRD + 上游摘要 · 不喂起草心路)· 产 `PL-CHALLENGE-{n}` · 至少 1 条实质质疑或显式「无 + 理由」(goal-complete 物化校验 · 详 [stages/goal-stage.md §3](../stages/goal-stage.md))
- 🔴 **「限制必要性」为何单列**(用户拍板「不要做没必要的限制」):判据一句话 —— **这条限制是「业务真要求的」还是「我加的」?**「我加的」→ 要么写出**不加会出什么事**,要么去掉。PL 的 mandate 是把范围往下压,所以**加法侧**(别扩展没必要的功能)天然有对抗;**减法侧没有** —— **限制读起来永远像「最小范围」**,越保守越没人质疑。代价还不对称:多做一个功能 = 浪费工时(当场看得见)· 多加一条限制 = 用户真想做的事做不了、且要到上线才发现。载体 = PRD §Out of Scope 与限制 的「性质」列。
