...
- 流程类型识别:闭集 Feature/Bug + preset(full/micro)+ 不进状态机的 Feature Planning / 问题排查
- 🔴 管辖判据先行:输入不属四类工作(功能/缺陷/排查/规划)—— 尤其**现状/知识问答** —— **直接回答 · 零 teamwork 仪式**(无 audit_line/状态行/流程推销)· 详 SKILL § 直答通道· 由 [SKILL.md § Triage 入口规范](../SKILL.md) 入口分诊 + [docs/prepare.md](../docs/prepare.md) 子流程承接(PMO 主对话执行)
