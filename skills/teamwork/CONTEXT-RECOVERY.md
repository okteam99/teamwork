...
└── 🔴 看板后必须附 💡 建议 + 📝 理由（红线 R5）· 下游：缺 💡/📝 → R5(b) 违规 · 用户对抗"为什么不给建议"· PMO 必须重出（实证 4.6 case "spec 起作用部分 vs 跳掉部分"分析）· v7.3.10+P0-146 加 consumer
```
...
```
├── 输出 Feature 看板
├── 🔴 PMO 必须分析优先级并给出推荐（红线 R5：禁止只抛状态不给建议）· 下游：只抛状态 → 用户必须问"那建议怎么走" · PMO 多走一轮 · 实证 4.6 case "spec 起作用 vs 跳掉"分类 · v7.3.10+P0-146 加 consumer
│   ├── 分析维度：
