...
```
stages/ ← Stage 定义(PMO dispatch 的单位 + 角色任务规范权威)
├── goal-stage.md PM 写 PRD + PL-PM 讨论 + 多角色技术评审
├── blueprint-stage.md QA 写 TC + TC 评审 + RD 技术方案 + 架构师评审
├── blueprint-lite-stage.md 敏捷需求快速蓝图
├── dev-stage.md RD TDD 开发 + 单元测试
├── review-stage.md 架构师 CR ∥ QA 代码审查
├── test-stage.md 集成测试 ∥ API E2E
├── ui-design-stage.md 当前 Feature UI 设计
├── browser-e2e-stage.md Browser E2E 端到端验收
├── pm-acceptance-stage.md PM 验收
└── ship-stage.md 合并 + 清理

agents/ ← Subagent 执行协议(非任务规范)
└── README.md(本文件) 执行方式偏好 + 通用约束 + Codex CLI spawn + PMO dispatch 规范 + 主对话产物协议

roles/ ← 角色定义(8 文件 · 见 ROLES.md 索引)
└── architect / designer / external-reviewer / pm / pmo / product-lead / qa / rd
```
