...
> Teamwork 以 **Stage 为权威**，各 Stage 的角色任务规范在对应 `stages/*.md`。本文件只保留 **Subagent 执行协议**（PMO dispatch / dispatch 文件格式 / 通用约束 / Progress Log / 外部 Codex spawn / 主对话产物协议），不定义具体角色任务。
> PMO 在 Subagent 路径下必须让 Subagent 先读本文件 + 对应 Stage 文件；主对话路径下可跳过本文件直接读 Stage 文件。

---

## 一、执行模型偏好（PMO dispatch 选模型）

> 🟢 **执行方式**（主对话身份切换 vs subagent）见 [STAGES.md §4](../STAGES.md)（PMO 自决 · 不枚举）。是否出 Plan + 模型（opus / sonnet / haiku）由 AI 在 Plan 模式自主决策 · 本节只列**模型选择**通用原则。
...
│ ├── PM 起草 PRD（Goal Stage 子步骤 1）：业务方向澄清 + AC 结构化
...
🔴 关键约束：
 ...
 - QA Code Review 默认 Sonnet（执行型校验，TC 覆盖判断 Sonnet 够用）
```
...
### 2.5 Progress Log 实时维护（🔴 硬规则 · flush 语义 + 运行中轮询）
...
Progress Log 格式见 §四 Dispatch 文件协议(若采用文件化 dispatch)· 非 state.py 生成。
```
...
> 🔴 **本节为历史记录 · 现行权威 = [standards/external-model-usage.md §11](../standards/external-model-usage.md)**。下文「session 隔离不依赖跨模型 / PMO 内省自审兜底 / external = codex 或 claude-cli」均已被 §11 取代:① **异质必须 host-aware**(同模型 session 隔离 ≠ 异质 · §11.1)· ② 降级走 **subagent 显式标 degraded**(非"内省自审"· §11.5)· ③ 走 `state.py external-review`(host 自动映射异质模型)。冲突以 §11 为准。

Codex CLI 用于 Review Stage 外部 Codex Review（见 `stages/review-stage.md`）+ Plan/Blueprint 交叉评审（见 `templates/external-cross-review.md`）的独立外部审查。
...
### 可用性检测（prepare 子流程角色可用性检测完成时缓存 · 见 docs/prepare.md · 结果缓存至会话结束）
...
├── 独立性强度需求（Review Stage 最高；external-cross-review 次之）
...
### Dispatch 文件协议（可选实践 · PMO 若用 subagent 可采用 · 非系统强制 · 详 [STAGES.md §4](../STAGES.md)）

> 🟢 **定位**:用不用 subagent、要不要文件化 dispatch,均由 PMO 自决([STAGES.md §4](../STAGES.md):**不强制 · 无系统级 dispatch 预检协议**)。本节描述的是 PMO **若选择**文件化 dispatch 时的一套推荐做法 —— 把 dispatch 信息写到 markdown 文件,Subagent 读该文件执行,完成时 append Result 回同一文件(避免在 prompt 里塞长文本 + 留审计记录)。

```
若采用文件化 dispatch,推荐遵循：
├── 一次 Subagent dispatch = 一个 dispatch 文件
├── dispatch 文件 = Subagent 入参 + 审计记录（同一份，消除重复劳动）
├── Subagent prompt 极简（~5 行），只含 dispatch 文件路径
├── Subagent append Result 区域，否则 PMO 视为 FAILED
└── PMO 先写 dispatch 文件再 dispatch（文件化模式下的自然顺序 · 非系统强制）
```
...
📎 **subagent-id 说明**：
- `{subagent-id}` 是 dispatch 文件的**标签**（rd-develop / arch-code-review / qa-code-review / integration-test / api-e2e / blueprint / designer 等），PMO 按 Stage + 角色组合填写。
- 🔴 **角色任务规范**在 `stages/*.md § 角色任务规范`（dev-stage.md / review-stage.md / test-stage.md）。
- dispatch 文件的 Input files 清单指向 `stages/{stage}-stage.md §{角色任务规范节}`。
```

**dispatch 文件**:由 **PMO 手动撰写**(若采用文件化 dispatch 这一可选模式)· 字段见下方「关键字段责任划分」表 · 🔴 **state.py 不生成 dispatch 文件**(用不用 subagent / 要不要文件化 dispatch 全由 PMO 自决 · 详 [STAGES.md §4](../STAGES.md))。
...
├── （若用文件化 dispatch）dispatch 文件是否已写好并保存到 dispatch_log/？
...
│ ├── ✅ 用户在 Goal Stage 拍板「用 PostgreSQL 不用 MySQL」（附 PL-PM 纪要引用）
...
```
├── Goal Stage → PRD.md（如有草稿）、PROJECT.md、KNOWLEDGE.md
...
├── （若用文件化 dispatch）dispatch 文件已写好并保存到 {Feature}/dispatch_log/
...
本协议适用于**主对话内执行**的任务：
- Goal Stage PRD 起草、PL-PM 讨论、多视角技术评审
...
| 任务 | 产物文件路径 | 格式 |
|------|------------|------|
| Goal Stage（PRD） | `{Feature}/PRD.md` | Markdown + YAML frontmatter（含 acceptance_criteria）|
| Goal Stage（技术评审）| `{Feature}/PRD-REVIEW.md` | Markdown |
| Goal Stage（PL-PM 讨论）| `{Feature}/discuss/PL-FEEDBACK-R{N}.md` + `PM-RESPONSE-R{N}.md` | Markdown |
...
> 🟢 stages/ 定义 + 编排见 [STAGES.md](../STAGES.md) · roles/ 8 角色定义见 [ROLES.md](../ROLES.md) · agents/README.md（本文件）= Subagent 执行协议（通用约束 / Progress Log / Codex spawn / PMO dispatch / 主对话产物协议）。
