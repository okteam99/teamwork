...
├── Sonnet 推荐：校验型 / 执行型任务（Review / Browser E2E）
│   ⚠️ v7.3.9：Test Stage（含 API E2E）改为 AI 自主决策模型，不再硬默认 Sonnet
│   详见 stages/test-stage.md「AI Plan 模式指引」+ agents/api-e2e.md「模型选择」
├── —（主对话继承）：主对话任务继承当前会话模型
...
### 2.5 Progress Log 实时维护（🔴 硬规则，v7.3.7 增补 flush 语义 + 运行中轮询）
...
├── 🚫 禁止：忽略时间戳（无法评估每步耗时）
└── 🎯 双重目的（v7.3.7 修订）：
    ├── 运行中：主对话可并发 Read 本段获取实时进度（见 §五 Step 2.5）
...
Codex CLI 仅用于 **Codex Code Review Subagent**（[review-stage.md](./review-stage.md)）+ Plan/Blueprint 交叉评审（[codex-cross-review.md](../templates/codex-cross-review.md)）的独立外部审查。各阶段的执行方式（Subagent / 主对话）见上方 §一「执行方式速查表」。

### 宿主无关的独立性保证（v7.3.9+P0 强化）
...
│   └── 可由机器校验（见 review-stage.md Output Contract）
└── dispatch 文件协议统一（agents/README.md §四），prompt 模板统一
```
...
> 🔴 **纠正反事实陈述**（v7.3.7）：v7.3.6 及之前本段写作"Subagent → 主对话实时通道不存在"是错误的——文件系统就是实时通道。该陈述已修正。"禁止依赖宿主特有 peek 能力"仍成立（如读 subagent 的 session JSONL），但**读 dispatch 文件的 Progress Log 段是协议允许的首选路径**。
...
┌─ 阶段 3：dispatch 运行中（主对话按需轮询，v7.3.7 新增）┐
