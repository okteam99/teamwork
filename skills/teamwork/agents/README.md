...
Progress Log 格式由 `tools/state.py` 各 stage-start 生成的 dispatch 文件中给出。
```
...
├── 🔴 Subagent prompt 极简（~5 �），只含 dispatch 文件路径
...
└── INDEX.md # 汇总索引（每次 dispatch 完成后 append 一�）
...
**dispatch 文件**:由 `tools/state.py` 各 stage-start 自生成,字段定义在 `tools/_v8_stage_specs.py` 各 StageSpec。
...
- `started_at` / `completed_at` 必填（审计 + 独立性校�）
