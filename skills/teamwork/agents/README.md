...
**并行姿态(鼓励 · v8.225)**:每个 stage 开工先问一句「**哪些子任务可以并行?**」—— 评审天然并行(冷审 N 路同发)· dev 多模块各派一个 teammate(命名 subagent · 长任务用 Progress Log 汇报)· 调研/探索 fan-out 保主对话 context 干净。护栏不变:子任务**边界清晰且够大**才拆(小/耦合/强串行自己做)· 代码类只写 worktree 内路径 · **stage 流转 / commit / complete / 最终整合永远归主对话**(并行的是执行 · 不是编排)。台账已记 host+model(v8.208/209)—— 年检可验档位建议是否伤质量。
