...
**深度判断/创造性产出用强模型(Dev 实现 · 架构 Code Review · PRD/TECH 起草 · UI 设计);校验型/执行型可用轻模型(TC 覆盖校验 · 测试执行 · 多角色并行评审)。** 默认继承会话模型;dispatch 时按宿主机制指定(Claude: Task model / Codex: agent toml)。架构师 Code Review 是架构层最后 gate · **不降级**。
