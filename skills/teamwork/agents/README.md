...
| **验证档**(轻) | 🔴 **白名单一律降到这档**(用户拍板):**写测试用例(TC 起草)· 执行测试 · 单测 · 集成测试 · e2e · TC 逐条对照 · 冷审执行 · 格式核对 · 机械外化 · 路由分诊 · 调研采集**(fan-out 检索/读取/结构化汇报 —— **整合与判断留深度档**:不新立「调研档」· 采集/判断二分落既有档) | Sonnet / Haiku | terra / luna | 冷审/验证 fan-out(隔离并行) |
...
- 🔴 **禁向用户提问(对着工具名 · 任何宿主)**:子代理**禁止调用**任何「向用户提问/确认」类工具(`AskUserQuestion` / `ask_user_question` / 各宿主变体)—— 缺信息/拿不准 → **写进返回结果**(verdict `NEEDS_CONTEXT` + 具体缺什么),由主对话处理。用户拍板:**子代理的问题由主对话自行处理;只有主对话判断属于用户主权的,才走 R5 授权暂停点交给用户** —— 主对话拿到 NEEDS_CONTEXT 后二分:事实/实现细节(测试放哪个文件 · 命名 · 用哪个既有函数)→ 主对话自答后补上下文重派;真用户主权(偏好/业务取舍/外部事实)→ 主对话按 R5 编号选项 escalate。why:主对话有全量 context,子代理的问题多数只是它缺上下文;用户面前只该出现 R5 格式的决策点,不该出现实现细节弹窗(实证 Grok 宿主:写测试的子代理把「测试写哪个文件」直接弹到用户屏幕 —— stage 暂停点纪律只管主对话,子代理侧此前无对着工具名的红线,派发 prompt 也没带,规则等于不存在)。
