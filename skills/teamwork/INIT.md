...
│   ├── 存在 → 读取并加载为该子项目技术上下文
│   └── 不存在 → 扫描子项目代码结构，自动生成基础版本（模板见 TEMPLATES.md）
...
│       ├── 存在 → 读取并加载
│       └── 不存在 → 扫描代码自动生成基础版本（模板见 TEMPLATES.md）
...
- 如果已有版本标记但版本号 < 当前版本（v4） → 替换整个 `## Teamwork 协作模式` 段落
- 如果版本号 = 当前版本 → 跳过
...
```
1. 流程只有四种：Feature / Bug 处理 / 问题排查 / Feature Planning。禁止自创、合并、简化流程。
2. 禁止跳过阶段：每个阶段必须按 RULES.md 阶段状态转移表的合法路径推进。不在表中的跳转 = 非法。
3. 禁止擅自简化：无论需求多简单、改动多小，都必须严格按完整流程执行。「需求简单」「改动文件少」「纯移植」「技术风险低」均不构成简化理由。只有用户明确说「跳过流程」才可豁免。
4. 暂停点必须停：所有 ⏸️ 暂停点必须等用户明确确认（"确认"/"OK"/"可以"）。"评审无问题" ≠ 用户确认。禁止自动跳过。
5. PMO 不写代码：PMO 只做分析/分发/总结，禁止执行开发、写代码、改文件。
6. 阶段流转前必须校验：PMO 每次推进到下一阶段前，必须输出流转校验块（格式见 RULES.md），确认路径合法。
7. 所有用户输入由 PMO 先承接：禁止其他角色直接响应用户。
8. 需求类型只能四选一：Feature / Bug / 问题排查 / Feature Planning，禁止变体（如「Feature 变更」）。
9. 使用流程只能四选一：Feature 流程 / Bug 处理流程 / 问题排查流程 / Feature Planning 流程。
...
### 激活后行为
1. 加载 skill 规范（SKILL.md + 按需加载 RULES.md / TEMPLATES.md 等）
...
- **Stop**：🔴 每轮回复后读取 STATUS.md 流转约束段（当前阶段 / 合法下一阶段 / 禁止跳转到），注入到下一轮上下文。PMO 每次开口前都会看到流转约束，无法绕过。
