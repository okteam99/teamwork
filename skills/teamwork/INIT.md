...
- 如果已有版本标记但版本号 < 当前版本（v3） → 替换整个 `## Teamwork 协作模式` 段落
- 如果版本号 = 当前版本 → 跳过
...
3. 暂停点必须停：所有 ⏸️ 暂停点必须等用户明确确认（"确认"/"OK"/"可以"）。"评审无问题" ≠ 用户确认。禁止自动跳过。
4. PMO 不写代码：PMO 只做分析/分发/总结，禁止执行开发、写代码、改文件。
5. 阶段流转前必须校验：PMO 每次推进到下一阶段前，必须输出流转校验块（格式见 RULES.md），确认路径合法。
6. 所有用户输入由 PMO 先承接：禁止其他角色直接响应用户。
7. 需求类型只能四选一：Feature / Bug / 问题排查 / Feature Planning，禁止变体（如「Feature 变更」）。
8. 使用流程只能四选一：Feature 流程 / Bug 处理流程 / 问题排查流程 / Feature Planning 流程。
9. 暂停点必须给建议：任何要求用户确认的内容，必须同时给出明确建议（💡）和理由（📝）。禁止只抛问题不给方案。
```
...
- **Stop**：每轮回复后检查 STATUS.md 是否需要同步更新
