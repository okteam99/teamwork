...
│ ├── 更新对应 changes/{change_id}.md「变更日志」段（v7.3.10+P0-59）│
│ └── ⏸️ 最终确认                                               │
│                                                              │
...
>
> 🔴 自下而上的每一次升级都必须暂停等用户确认，禁止自动向上传播。· 下游：自动传播 → 用户事后发现"Feature 影响 PROJECT.md / Roadmap"· R5(a) 暂停红线违规 · 必须 rollback 重做（v7.3.10+P0-146 加 consumer）
