...
**业务架构与产品规划 · 建议章节**（怎么按复杂度取舍 = PL 自判 · 不给裁剪教程）：

- **核心章节**（所有项目）：规划状态（状态表 · 见上方）· 产品定位（一句话）· 业务架构（核心模块 + 角色关系 + 业务流程 · 建议 Mermaid）· 执行线列表（详下方）· MVP 范围定义 · 待决策项
- **扩展章节**（中大型按需）：收入模型 · 分阶段路线图 · 规划议题追踪（议题表 · 见上方）· PL 可追加项目特有章节
...
> 与自上而下级联（上节）相对：Feature 开发中发现影响超出当前层级（范围溢出 / 与业务架构假设冲突 / 方向质疑 —— 识别这些信号 = AI 自判 · 不给枚举清单）→ **逐级向上反馈 · 每级升级必经用户确认**。
> 文档层级（影响归位用）：`业务架构与产品规划.md` ↕ `workstream/WS-NN.md` ↕ `teamwork-space.md` ↕ 各子项目 `ROADMAP.md` ↕ Feature PRD/TECH/TC。

**协议（🔴 硬约束 · 违者 = R5(a) 暂停红线违规 · 必须 rollback 重做）**：

1. **PM/RD 只标记 · 不改上游**：在当前阶段输出中标「⚠️ 上游影响」（影响描述 + 可能影响层级 + 建议）→ PMO 承接评级：仅影响同子项目 ROADMAP / 跨子项目 teamwork-space（= Level 2）/ 业务架构·执行线·产品定位（= Level 3）。PMO 只评级 · 不做产品方向决策。
2. **每级升级必须暂停等用户确认 · 禁止自动向上传播**（自动传播 = 用户事后才发现 Feature 动了 Roadmap）。升级到 Product Lead 后**进 feature-planning 产 WS**（不在流程外 ad-hoc 手搓）。
3. **触发升级的原 Feature 挂起**（PMO 记挂起原因 + 关联 WS 编号）· 上游级联完成后由**用户**决定：恢复（可能更新 PRD）/ 重走 PRD / 废弃 —— 🔴 上游未决前禁止恢复。
4. **用户可选「不升级」**（调整当前 Feature 范围在现有框架内完成）· 该决策记入 PRD（设计决策/妥协留痕）。
