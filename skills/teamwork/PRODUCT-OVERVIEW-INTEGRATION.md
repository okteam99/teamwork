...
```
变更级联流程（Level 2 / Level 3）· 逐层推进 · 每层完成必须 ⏸️ 用户确认后才进下一层：

第一层 · Product Lead 影响评估（执行模式）
├── 前提：product-overview 文档已在讨论模式中更新完成
├── PL 读取待执行变更记录（CHG-{编号}）→ 输出「变更影响评估报告」：
│ 变更概述（引用已更新章节）/ 受影响执行线（目标·阶段·交付物变化）/ 受影响子项目 + 预计 ROADMAP 变更 / 里程碑调整建议
└── ⏸️ 用户确认 → 变更状态 📝 → 🔄 执行中 → PL 完成交还 PMO

第二层 · Workspace 架构更新（Level 3；Level 2 跳过此层）
├── PMO 触发 Workspace Planning：PM 更新 teamwork-space.md（架构图 + 子项目清单）+ 执行线映射表
└── ⏸️ 用户确认

第三层 · 子项目 Feature List 级联更新（对每个受影响子项目）
├── PM 读取该子项目 ROADMAP.md，对照影响评估标记：🆕 新增 / ✏️ 修改（范围·验收标准变化）/ 🗑️ 废弃 / ✅ 不受影响（明确标注避免遗漏）
└── 更新 ROADMAP.md → ⏸️ 每个子项目的 ROADMAP 变更独立确认

第四层 · 收尾
├── PMO 输出「变更级联完成报告」：product-overview 变更摘要 / 各子项目 ROADMAP 变更摘要（新增·修改·废弃数）/ 更新 changes/{change_id}.md「变更日志」段
└── ⏸️ 最终确认 → 用户确认后逐个启动新增/修改的 Feature 流程
```
