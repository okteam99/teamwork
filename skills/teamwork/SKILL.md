...
1. 代码写权归 RD（v7.3.10+P0-20 重构）：
   ...
   - 📎 **流程选择**：非 Micro 流程走完整 Stage 链；Micro 流程省略 Plan/Blueprint/UI/Review/Test Stage，保留「必读规范 → 改 → 自查」最小 RD 闭环（详见 FLOWS.md §六）。Micro 不是红线例外，是独立流程。
   ...
   - 🟢 **Ship Stage 行为（v7.3.10+P0-15）**：Ship Stage PMO 不做本地 merge / push merge_target / 冲突解决，只负责净化 + push feature + 生成 MR 创建链接；合并权由平台和用户处理（此为 Stage 行为描述，不是红线例外）。
...
└── 最小闭环：PMO 分析 → 用户确认 → **PMO→RD 身份切换 + 加载规范 + cite** → RD 改动 → RD 自查 → 用户验收

📎 与红线 #1 的关系：Micro 流程**不是**红线 #1 的例外，而是独立流程——它省去了 Plan/Blueprint/UI/Review/Test 等 Stage，但保留了「RD 身份 + 读规范 + 自查」这一 RD 本职契约，与红线 #1（代码写权归 RD）完全自洽。
```
...
| [stages/](./stages/) | 各 Stage 契约 + 角色任务规范（v7.3.10+P0-19-B 物理合并 agents/*.md → stages/*.md §角色任务规范） | PMO / Stage Subagent |
