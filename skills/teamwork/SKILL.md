...
- **不膨胀**:自动流转节点禁止插入暂停 · "回合边界/容量预算/让用户看进度" 不构成暂停理由
  ...
  - 🔴 **工作量大是 AI 自己的执行问题 · 不甩给用户**:session / 上下文吃紧 → **自己** plan + 派 subagent 并行消化(`Agent` 工具)· 不是停下来问用户"怎么推进";"用户参与了设计" ≠ "用户该决定执行节奏" —— 设计决策已在**授权暂停点**收敛 · 落地是 AI 的活 · 闷头干到 stage 完成 · 后果由 review/test/pm_acceptance 下游 gate 兜
