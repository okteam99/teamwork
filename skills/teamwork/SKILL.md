...
## 🔴 绝对红线 · 8 条（v7.3.10+P0-103 归并 16→8 + 层级化）

> 📎 **红线层级化**（v7.3.10+P0-103）：本段是 **L1 核心红线**（必读 · ≤8 条）· 详细 schema / 反例 / 出口校验落 **L2 专项规范**（standards/* + roles/* sub-file）· 物理拦截机制（黑名单 / verify-ac.py / state-patch.py）落 **L3 工具层**。
...
R5. 暂停点协议（v7.3.10+P0-103 合并原 #5+#10）：
...
R7. 证据闭环红线（v7.3.10+P0-103 合并原 #9+#14+#16）：
...
🟢 **TROUBLESHOOTING.md 设计要点**（v7.3.10+P0-109 / v7.3.10+P0-110 简化）：
...
CLAUDE.md / AGENTS.md → 宿主指令文件（teamwork 注入段）
```
...
L3 工具层（templates/*.py + 机器校验）
   ├── verify-ac.py / state-patch.py / detect-external-model.py
   └── 真正的物理拦截 · 不依赖 AI 自觉
```
