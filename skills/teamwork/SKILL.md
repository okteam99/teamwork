...
- **不膨胀**:自动流转节点禁止插入暂停 · "回合边界/容量预算/让用户看进度" 不构成暂停理由(🔴 v8.246 实证:test→browser_e2e 流转后汇报完即结束回合 · 用户被迫问「为什么暂停了」—— 汇报/总结不是回合终点 · 流转 emit 已机械附带 `continue_reminder` 提醒立即继续)
  ...
  - 🧩 **subagent / teammate(`Agent` 工具)= 提升并行度的默认考虑项(v8.225 姿态:鼓励)**:每个 stage 开工先问「**哪些子任务可以并行?**」—— 评审冷审天然 N 路同发 · dev 多端/多模块各派一个 teammate(命名 subagent · Progress Log 汇报)· 调研/探索 fan-out 保主编排 context 干净 · 验证类走轻档模型(档位框架详 [agents/README.md §一](./agents/README.md))。🔴 v8.254 两问补丁:此问**每进新子阶段重问**(实现→测试编写→修复 · 耦合度随阶段变 · 开工一次会过期);派发后**等待窗口主对话不闲置**(干自己能干的:自查证据/再拆剩余工作 —— 实证:整包塞单 agent + 主对话裸等 · 用户点破才拆三线)。护栏不变:子任务**边界清晰且够大**才拆(小/耦合/强串行自己做 · 协调开销反拖慢)· 代码类只写 worktree 内路径 · **stage 流转 / commit / complete / 最终整合永远归主对话**(并行的是执行 · 不是编排)。🎚️ **dispatch 档位(全局规则 · 任何 subagent/teammate/workflow 适用)**:不传 model = 继承会话模型 · **常费而不自知** —— 派前按任务性质定档:**校验/枚举型**(QA 冷审 · TC 对照 · 测试执行 · 机械外化)→ **验证档**(如 sonnet/haiku);**判断/创造型**(Architect/PL 冷审 · 方案 · 裁决)→ **不降档**。🔴 **v8.235 声明制:每次派发必声明「model + 一句为什么」**(主对话派发语句 / dispatch 文件 Meta / workflow 脚本 agent() 旁注释)—— 例:`model: sonnet(TC 对照 = 校验型 · 验证档够用)`;**不声明 = 默认继承没思考**(台账 dispatch_models 的 unspecified 就在数这个)。档位表与三条硬边界单源 = [agents/README.md §一](./agents/README.md)。⚡ **鼓励并行**:能并行的不串行 —— 冷审/验证/多模块实现/调研 fan-out 默认考虑 subagent/teammate/workflow(ultracode 开启时 workflow 优先)· 并行度是效率的第一杠杆。🔑 **授权已长期化**(v8.135 · v8.211 载体迁本文件):本 SKILL § Subagent 默认授权 已含用户对 teamwork 流程内 subagent 的**默认授权**(满足各宿主工具契约的 "user explicitly asks" · 无需每次口头确认)——授权解决「能不能用」· 本条判据仍管「该不该用」
