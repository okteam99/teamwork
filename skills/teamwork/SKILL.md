...
- **不膨胀**:自动流转节点禁止插入暂停 · "回合边界/容量预算/让用户看进度" 不构成暂停理由
  ...
  - 🔴 **规模 / 节奏是 AI 自己的执行问题 · 不甩给用户**:subagent(`Agent` 工具)是**标准执行手段** · **每 stage 起手**就评估哪些独立子任务可并行 / 隔离委托(dev 多模块、goal/blueprint 多方案调研、review 多关注点……)—— **不是任务大才想起** · session / 上下文吃紧更要用(主编排 context 干净 + 并行提速);"用户参与了设计" ≠ "用户该决定执行节奏" —— 设计决策已在**授权暂停点**收敛 · 落地是 AI 的活 · 闷头干到 stage 完成 · 后果由 review/test/pm_acceptance 下游 gate 兜
