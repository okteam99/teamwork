...
6. 需求类型只能填：Feature / Bug / 问题排查 / Feature Planning，禁止变体（如「Feature 变更」）
7. 使用流程只能填：Feature 流程 / Bug 处理流程 / 问题排查流程 / Feature Planning 流程
...
| TEMPLATES.md | ~2600 | 仅读 STATUS.md 模板段（搜索 `## STATUS.md`） | 按需读取对应模板 |
...
│                     │                              │                             │ ✅ 🌐 「重新规划整个项目」    │
├─────────────────────┼─────────────────────────────┼─────────────────────────────┼─────────────────────────────┤
│ → Bug 处理流程       │ → Feature 流程               │ → 问题排查流程               │ → Feature Planning 流程      │
...
**状态行格式**（放在回复末尾，🔴「流程」字段必填，取值只能是 Feature / Bug 处理 / 问题排查 / Feature Planning 之一）：
...
```
---
🔄 Teamwork 模式 | 流程：Feature Planning（🌐 工作区级）| 角色：[PM/PMO] | 阶段：[当前阶段] | 受影响子项目：[AUTH, WEB, ADMIN] | 下一步：[下一步事项]
```

**🌐 工作区级 Planning 状态行示例**：
```
---
🔄 Teamwork 模式 | 流程：Feature Planning（🌐 工作区级）| 角色：PM | 阶段：架构讨论中 | 受影响子项目：待定 | 下一步：讨论子项目拆分方案
---
🔄 Teamwork 模式 | 流程：Feature Planning（🌐 工作区级）| 角色：PM | 阶段：⏸️ teamwork_space.md 待确认 | 受影响子项目：AUTH, WEB | 下一步：⏸️ 等待用户确认架构变更
---
...
| QA 集成测试 | 阶段：🤖 Subagent 执行中 | 下一步：E2E 启用 → QA E2E 验收 / 未启用 → QA Lead 质量总结 |
| QA E2E 验收 | 阶段：🤖 AI 浏览器 E2E 执行中 | 下一步：通过 → QA Lead 质量总结 / 有问题 → RD 修复（子项目未启用 E2E 时跳过） |
