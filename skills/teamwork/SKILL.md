...
- **每进新子阶段重问**(实现 → 测试编写 → 修复):耦合度随阶段变 · 开工问一次会过期。
...
- 📣 **声明制**:每次派发必声明「**model + 一句为什么**」(派发语句 / dispatch 文件 Meta / workflow `agent()` 旁注释)· 例 `model: sonnet(TC 对照 = 校验型 · 验证档够用)`——不声明 = 默认继承没思考(台账 `dispatch_models` 的 unspecified 就在数这个)。
