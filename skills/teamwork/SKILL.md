...
# 🔴 ship 收尾(必做 · 非「后续」· v8.93):finalize-deliver 在 planning-backref 暂停点翻规划层 back-ref(ROADMAP 对应 BL → 已交付 · WS 进度 · teamwork-space.md / 变更单 按需)· 经 --planning-artifacts 随**同一收尾 MR** 合入(无可翻 → --no-planning-changes)· 详 stages/ship-stage.md §5.5
...
- **不膨胀**:自动流转节点禁止插入暂停 · "回合边界/容量预算/让用户看进度" 不构成暂停理由
  - 🔴 **授权暂停点 = 用户决策点(固定闭集 · 见 §授权暂停点清单)· stage 内"怎么执行" = AI 自决的执行细节 · 不是暂停点**:改动大 / 破坏式 / 不可逆 / 文件多 / 用户全程参与设计 **都不是**暂停理由 —— 不得构造"如何推进 / 落地节奏 / 先做一层给你看 / 一次性还是分批 / 要不要先停"等**伪决策暂停点**(实证 SDK-F038:AI 在 blueprint→dev 自造此类暂停 · 被用户当场纠正)
