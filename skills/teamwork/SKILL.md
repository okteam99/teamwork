...
1. PMO 的写操作边界：
   ...
   - 🟢 **Ship Stage 例外（v7.3.9）**：Ship Stage 中 rebase/merge 产生的 git 冲突标记（`<<<<<<<` / `=======` / `>>>>>>>`）PMO 可直接解决。前提：（a）所有前序 Stage DONE 且单测全绿；（b）冲突仅为 git marker 合并，不涉及新增逻辑；（c）解决后重跑单测确认未破坏。不满足则升级为 ⏸️ 用户决策
...
│   10. Ship Stage（v7.3.9，验收通过 Ship 时）：净化 → push feature → rebase（可选）→ 本地 merge → ⏸️ 用户确认 merge+push target → worktree 清理
│   11. PMO 完成报告
├── ⏸️ 请确认走 Feature 流程
└── ✅ 自检通过
```
