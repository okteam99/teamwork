...
```
PMO 识别需求应路由到中台子项目的信号：
├── 用户明确提到共享模块/公共库/基础设施/中间件
├── 需求的受益方是多个子项目而非终端用户
...
**✅ 自动继续的节点（无待确认项）**：
```
├── PM 完成 PRD 初稿 → 🤖 PMO 启动 PL-PM Teams 讨论（PL Agent + PM Agent 交替）
├── PL-PM 讨论收敛（共识→PM更新PRD / 分歧→⏸️用户决策→PM更新PRD）
├── PRD 定稿 → 🤖 PMO 启动 Subagent 执行 PRD 多角色评审
...
│   └── 无 UI → 🤖 PMO 启动 Subagent 执行 QA 代码审查（读代码 + TC 逐条验证）
├── UI 验收通过 → PMO 摘要 → 🤖 PMO 启动 Subagent 执行 QA 代码审查（读代码 + TC 逐条验证）
├── QA 代码审查 Subagent 返回 → PMO 摘要 → 自动进入 QA 集成测试前置检查
