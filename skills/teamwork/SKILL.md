...
10. 暂停点必须给建议：任何要求用户确认的内容，必须同时给出明确建议（💡）和理由（📝），禁止只抛问题不给方案；🔴 v7.3.5/v7.3.6：单决策点用 1/2/3 数字编号（用户回一个数字）；多决策点用"数字决策点+字母选项"组合（1./2. 决策点 + A./B. 选项，用户回 `1A 2B` 这种组合），禁止 ①②③ 等需要输入法切换的字符。推荐项标 💡 列首，末项始终「其他指示」。详见 RULES.md「暂停输出规范」第 2 条
