...
## Subagent 默认授权(v8.211 · 载体自宿主注入块迁入本文件)

用户明确授权 AI(适用宿主:Claude Code / Codex CLI / Gemini CLI)在 Teamwork 流程中**默认使用 subagent · 无需每次另行确认**(满足各宿主 subagent 工具契约的 "user explicitly asks")。适用:PRD / Blueprint / Review 各阶段 cross-review 与多角色评审、PL 对抗质疑(隔离执行)、并行探索调研、互不重叠 write scope 的实现子任务、验证/测试类 sidecar。
约束:子任务**边界清晰且够大**才派(R4 判据 ·「该不该用」仍 AI 自决)· 代码类 subagent 声明文件/模块范围且只写 worktree 内路径 · stage 流转 / commit / complete 仍由主对话掌控(不外包流程)· 主对话负责最终整合与交付。

> 📎 v8.211 起 teamwork **不再注入**宿主指令文件(CLAUDE.md / AGENTS.md / GEMINI.md)—— 共享仓库里注入块会被 commit · 污染不用 teamwork 的用户。本段与 PMO 定位 / worktree 纪律等关键信息**以本 SKILL 为唯一载体**(加载 skill 即生效 · 只影响用 teamwork 的 session)· bootstrap 会自动清理项目里的历史注入块。
...
- **不膨胀**:自动流转节点禁止插入暂停 · "回合边界/容量预算/让用户看进度" 不构成暂停理由(🔴 v8.246 实证:test→browser_e2e 流转后汇报完即结束回合 · 用户被迫问「为什么暂停了」—— 汇报/总结不是回合终点 · 流转 emit 已机械附带 `continue_reminder` 提醒立即继续)
  ...
  - 🧩 **subagent / teammate(`Agent` 工具)= 提升并行度的默认考虑项(v8.225 姿态:鼓励)**:每个 stage 开工先问「**哪些子任务可以并行?**」—— 评审冷审天然 N 路同发 · dev 多端/多模块各派一个 teammate(命名 subagent · Progress Log 汇报)· 调研/探索 fan-out 保主编排 context 干净 · 验证类走轻档模型(档位框架详 [agents/README.md §一](./agents/README.md))。🔴 v8.254 两问补丁:此问**每进新子阶段重问**(实现→测试编写→修复 · 耦合度随阶段变 · 开工一次会过期);派发后**等待窗口主对话不闲置**(干自己能干的:自查证据/再拆剩余工作 —— 实证:整包塞单 agent + 主对话裸等 · 用户点破才拆三线);🔮 goal 终确认等待窗 = 后台起 TECH 草稿(投机窗 · 详 goal-stage ④ · v8.256)。护栏不变:子任务**边界清晰且够大**才拆(小/耦合/强串行自己做 · 协调开销反拖慢)· 代码类只写 worktree 内路径 · **stage 流转 / commit / complete / 最终整合永远归主对话**(并行的是执行 · 不是编排)。🎚️ **dispatch 档位(全局规则 · 任何 subagent/teammate/workflow 适用)**:不传 model = 继承会话模型 · **常费而不自知** —— 派前按任务性质定档:**校验/枚举型**(QA 冷审 · TC 对照 · 测试执行 · 机械外化)→ **验证档**(如 sonnet/haiku);**判断/创造型**(Architect/PL 冷审 · 方案 · 裁决)→ **不降档**。🎭 **双路评审模型错开**(v8.268 · 正常模式):goal(PL+外审)/ blueprint·review(Architect+外审)两路并行冷审**模型必须不同** —— 主审路继承会话主模型 · 外审路错开一档(如 fable5 会话 → 外审 opus;会话 opus → 外审 fable5/sonnet)。why:同模型双路 = 盲区相关(系统性偏差两路同瞎)· 错开 = 零 CLI 成本近异质(上下文与权重双错开);跨厂商异质 opt-in 时天然错开 · **单路评审同样错开**(v8.269:fast 合并单路 / roster 减到一路 → **该路 ≠ 会话主模型** —— 单路是仅有的独立采样 · 同模型 = 自审盲区;不变式:**任何评审配置至少一路 ≠ 会话主模型**)· 验证轮照降档规则(降档本身即错开)。🔴 **v8.235 声明制:每次派发必声明「model + 一句为什么」**(主对话派发语句 / dispatch 文件 Meta / workflow 脚本 agent() 旁注释)—— 例:`model: sonnet(TC 对照 = 校验型 · 验证档够用)`;**不声明 = 默认继承没思考**(台账 dispatch_models 的 unspecified 就在数这个)。档位表与三条硬边界单源 = [agents/README.md §一](./agents/README.md)。⚡ **鼓励并行**:能并行的不串行 —— 冷审/验证/多模块实现/调研 fan-out 默认考虑 subagent/teammate/workflow(ultracode 开启时 workflow 优先)· 并行度是效率的第一杠杆。🔑 **授权已长期化**(v8.135 · v8.211 载体迁本文件):本 SKILL § Subagent 默认授权 已含用户对 teamwork 流程内 subagent 的**默认授权**(满足各宿主工具契约的 "user explicitly asks" · 无需每次口头确认)——授权解决「能不能用」· 本条判据仍管「该不该用」
- **必给步骤描述**:选定流程类型后必须给完整步骤(stage 链 + 每个 stage 大致做什么 + 预期产出)

### R5(b) 暂停点标准格式 🔴

🔴 **emit 任何 R5 暂停点前先跑** `state.py pause-mark --feature <path> --label '<暂停点名>'`(v8.192 计时排毒):等待用户的墙钟计入该 stage `await_minutes`(下一个流程命令自动闭合 · resume 零纪律)· 否则等待混入工作时长 · 耗时数据失真(实证 goal 均值 157m vs 中位 22m)。

🔴 **投递位置(v8.275)**:暂停点 markdown / user_card 必须是**回合最后一条输出 · 其后零工具调用** —— 宿主可能不渲染回合中段文本(实证:ship1 卡片贴出后同回合又调 await-merge · 卡片被吞 · 用户被迫问「url 发下」)。伴随暂停点的监控/标记类命令(pause-mark / await-merge 等)一律**先执行(后台/静默)再贴暂停点**。
...
**红线**:
- **判定**:任何"PMO emit 后需要用户回应才能继续"的点 = 暂停点 = 必走本格式。不可只描述情境让用户自由输入 · 不可用"回复『X』升级"等自由文本 —— 必给编号选项(🔴 斜杠并列候选清单「A / B / C」同属自由文本 · `ok` 无从解析 · v8.245)。
- 不可省略**编号**(用户要打字 = 心智负担)
- 不可省略 **💡 推荐**(用户被迫现做决策)
- 不可省略**理由**(用户不知为什么选这个)
- 🔴 **方案/变更确认类必自带变更点明细**(v8.242):让用户拍板「改 X」时 · 选项之前必给**变更点清单**(对象级 · 每条一行:对象|变更|用途)—— 情境一句 + 分类概括 + 文件指针**不算**(用户被迫追问「方案是什么」= 暂停点白跑一轮 · 实证 DB schema case;决策材料在暂停点内自含 · 指针只作深读补充)
- 单选 → 1/2/3 · 多决策 → 1A/2B
...
### 🔴 管辖判据 · 直答通道(分诊之前先过 · v8.228)

**teamwork 只管四类工作:功能开发 / 缺陷修复 / 问题排查 / 项目规划。** 输入不属于这四类 → **直接回答 · 不走任何 teamwork 输出协议**(无 audit_line · 无 mode 宣告 · 无状态行 · 无 recap · 不套流程建议):

- **现状/知识问答**:「X 现在是怎么实现的」「这个配置什么意思」「为什么用了 Y 库」「项目结构是什么样」—— grep/Read 后**像正常助手一样答完就完**;
- **通用技术问题 / 与本项目开发无关的输入**:直答;
- **边界**:答完后如果用户的追问升级成四类工作(「那把它改成…」)→ 此刻才进分诊;**不要在直答后主动推销流程**(「要不要走个 Feature」——用户要做会说)。
- 📎 mode A/D 与直答的区别:A/D 是**项目内查询/看板**(轻仪式:可带状态行);直答通道是**管辖外**(零仪式)。拿不准 → 按直答处理(误加仪式的代价 > 误省)。
...
🔴 **升级建议是暂停点 · 必走 R5 标准 1/2/3 格式**(不可用"回复『进 Bug』"自由文本 · 用户要打字 = 心智负担)· 🔴 **多候选动作逐一编号**(「Bugfix / 修配置 / Feature」斜杠并列 = 自由文本 · 用户回 `ok` 无从解析 → 白问一轮 · 实证 v8.245)· 选项**具体化自排查结论**(「先修正 staging 配置(不改代码)」而非抽象流程名)· **💡 推荐必给** = 排查结论指向的最可能动作(排查者最有信息量 · 不给推荐 = 判断甩回用户 + `ok` 快捷词失灵):

```markdown
⏸️ <排查完成 / 讨论收敛> · <一句结论> · 请选择:

1. **<按结论最可能的动作 · 具体化>** 💡 推荐
   理由:<为什么它最贴排查结论 · 1-2 句>
   动作:<选后 PMO 做什么 · 进流程的写「prepare → init-feature → <first_stage> stage」>

2. **<候选动作 B · 具体化>**(如:进 Bug 流程 · diagnose 复核排查结论不重查)
   理由 / 动作:<同上>

3. **暂不处理 / 其他指示**
   动作:排查报告留档 · 可记入待规划需求池(product-overview/PENDING.md)
```
...
- **留两端 · 各合并单路**(roster = `{goal: [fast], review: [fast]}` · 「fast」= 合并伪角色 · 单 agent 兼多帽 · 🎭 该单路模型 **≠ 会话主模型**〔v8.269 错开 · 如 fable5 → opus —— 单路是仅有的独立采样 · 同模型 = 自审〕):
...
- **去掉**:blueprint 评审(不产 TECH-REVIEW.md · TC/TECH 写完直进 dev)· 两端的多路独立性(无第二路冷审 / 无 external 独立产物)。
...
🔴🔴 **yolo ≠ 简化/提速 · 是「加重审核」**:无人值守 = **没人在看** → 自动化评审是**唯一安全网** · 必须**保留 / 加重** · **绝不削弱**。**roster 内评审全真跑 · 一个不少**(v8.244 默认两路:Architect 主审 + 覆盖方向制第三视角〔QA 视角并入必覆盖方向〕· roster 加回的照跑);🔴 v8.204/v8.268:第三视角**默认 = 错开模型 subagent 隔离冷审**(≠会话主模型(如 fable5 会话 → 外审 opus) · external 跨厂商异质**默认关** · 省 CLI 冷启动)· 跨模型异质 = **opt-in 升级**(localconfig `disable_external_review: false`)。yolo 的「零 stop」**只**针对**人工决策暂停点**(prepare / pm_acceptance / MR merge)。
...
- 🔴 **异质开关 = localconfig 单源**(v8.204 · 🔴 **默认 true = 关**):external 异质评审受 `.teamwork_localconfig.json` 的 `disable_external_review` 控制 —— **缺省 / true = 关**(第三视角走错开模型 subagent 隔离冷审〔≠会话主模型 · v8.268〕· CLI 冷启动太耗时)· **显式 false = opt-in 跨模型异质**。默认(关)时:① `init-feature --yolo` kickoff 一行 **INFO**(`yolo_external_warning` · 非红线告警 · 告知第三视角为错开模型冷审)② 第三视角**必须是 subagent 冷审**(isolated context · 🎭 模型错开〔≠会话主模型 · v8.268〕· **非主对话热审 / 非 AI 手写**)· 门禁校验 `external-cross-review/*.md` 含 `review_via: subagent`(无 → FAIL)—— **「非异质」也不许「不冷审」**(同模型但冷上下文仍是独立采样;主对话自评 = 无效)。跑 `state.py external-review --stage <X>` 自动 emit 对应配方(默认冷审 / opt-in 异质)。
...
> 🔴 **文件位置权威 = 各 `templates/*.md` 头部「位置：」声明**(单源 · 本表「文档」列的路径只作速查 · 冲突以模板为准)。产物**不在项目根裸放** —— 规划/设计类产物默认落 `docs/`(单项目)或 `{子项目}/docs/`(多项目)· 详各模板。
...
| `docs/ROADMAP.md`(单项目)/ `{子项目}/docs/ROADMAP.md`(多项目) | Feature(BL) 列表 + 优先级 + 排期 + 关联 WS | 讨论 Feature 优先级 / 创建 Feature |
| `{子项目}/docs/design/sitemap.md`(与全景 panorama 同目录) | 信息架构 / 页面层级 | 讨论 UI / 创建含 UI 的 Feature |
| `project-specs/DEV-RULES.md` | **项目强制开发规范(人维护)**:API 契约 / 错误处理 / 其他约定(v8.257 三项制 · 架构归 ARCHITECTURE.md+ADR · 命名/风格/测试策略走 standards 缺省) | **blueprint(TECH)+ dev(实现)必读** · 起草/写码前 |
...
| `project-specs/RELEASE-GUIDE.md` | **版本发布规范(人维护)**:集成分支→生产的发布流程(默认 staging→main MR · URL 置顶 · 提醒用户合入 · 发布后补 release-gated 证据) | 用户说「发布 / 上线 / 发版」时 **PMO 必读照办** |
...
| 产品方向 / Feature 排期 / Roadmap | `{子项目}/docs/PROJECT.md` / `docs/ROADMAP.md`(单项目根)|
| 页面层级 / UI 整体 / 信息架构 | `{子项目}/docs/design/sitemap.md` |
| 开发规范 / 约定 / API 契约 / 错误处理(架构分层 → ARCHITECTURE.md · 命名风格测试策略 → standards 缺省) | `project-specs/DEV-RULES.md` |
| 发布 / 上线 / 发版(集成分支 → 生产) | `project-specs/RELEASE-GUIDE.md`(v8.258 · 必读照办 · 合入归用户) |
...
- CLAUDE.md / AGENTS.md / GEMINI.md **历史注入段清理**(v8.211 注入退役:发现旧 TEAMWORK_BEGIN 块 → 移除 · 用户内容不动 —— 共享仓库不再污染非 teamwork 用户 · 关键信息在本文件)
...
**AI 每次主对话回复(在 teamwork 流程内)末尾必含状态行 · v8 3 行格式**(🔴 v8.228 作用域:**有活动 feature/流程时**才输出 · 直答通道与管辖外轮次**免** —— 给闲聊贴状态行 = 仪式噪音):
