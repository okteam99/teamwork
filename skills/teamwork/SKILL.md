...
1. PMO 的写操作边界：
   ...
   - 🟢 Micro 流程例外（v7.3）：Micro 流程下 PMO 可直接改代码（白名单内零逻辑变更），但必须走 AI Plan 模式规划 + 用户确认流程后执行 → 详见 FLOWS.md「六、Micro 流程」
   - ✅ PMO 可直接改的常规文件：Teamwork 流程文件（STATUS.md/ROADMAP.md/state.json）和纯文档（README/注释/changelog），需在摘要中标注「📝 PMO 直接修改：{文件} {改动}」
   - 📎 执行方式（Subagent / 主对话 / 混合）由 AI Plan 模式决定（见下方「AI Plan 模式规范」），不是红线
...
14. AI Plan 模式红线（v7.3）：每个 Stage 开始前必须输出 Execution Plan 块（含 approach/rationale/steps/loaded_role_specs/loaded_standards/key_context）。未输出 Plan 不得开始 Stage 工作。Plan 写入 state.json.planned_execution。详见下方「AI Plan 模式规范」
...
### Execution Plan 输出格式（硬规则）

每个 Stage 开始时，AI 必须在主对话输出如下块：
...
### Rationale
{为什么选这个 approach，基于任务特性分析}

### Steps
1. {Step 描述} → 在 {主对话 / Subagent-{id}}
2. ...

### Expected Output（引用 Stage 的 Output Contract）
- {产物文件 1}
- {产物文件 2}
- 机器校验命令：{commands}
...
- agents/{id}.md（如适用）
- standards/common.md
- standards/{backend|frontend}.md（如适用）

### Key Context（PMO 注入，逐项判断，无则 `-`）
- 历史决策锚点：...
- 本轮聚焦点：...
- 跨 Feature 约束：...
- 已识别风险：...
...
```
🔴 未输出 Execution Plan → 不得开始 Stage 工作
🔴 Plan 必须写入 .teamwork/state/{feature_id}.json 的 planned_execution[stage]
🔴 Loaded Role Specs 必须真实 Read（不能只写路径不读）
🔴 角色切换时必须 cite 该角色规范的关键要点（防止凭记忆执行）
🔴 实际执行偏离 Plan（如原计划主对话改用 Subagent）→ 必须更新 Plan + 记录偏离理由

Micro 流程例外：
├── Micro 流程下 PMO 可选 approach=main-conversation 直接改代码
├── 仍必须输出 Execution Plan（steps / loaded_specs / key_context 齐全）
└── 改动限于 Micro 白名单（零逻辑变更：资源/文案/样式/配置常量/注释）
```

### 典型 approach 选择指引（非强制，AI 可创新）

| Stage | 默认推荐 | 何时切换 |
|-------|---------|---------|
| Plan Stage | main-conversation | 需求极清晰且无需用户介入讨论 → subagent |
| UI Design / Panorama | subagent | 小 UI 改动 → main-conversation |
| Blueprint Stage | main-conversation | 需求十分清晰 → subagent |
| BlueprintLite | main-conversation | 无需切换 |
| Dev Stage | AI 自主按规模判断 | ≤3 文件 → main；>10 文件 → subagent |
| Review 架构师 | main-conversation | 主对话 context 紧张 → subagent |
| Review QA/Codex | subagent | 独立视角需要 |
| Test 环境 | main-conversation | 环境稳定 → subagent |
| Test 集成/E2E | subagent | 测试命令快速 → main |
| Browser E2E | main-conversation | 无人值守 CI → subagent |

🔴 以上只是指引。AI Plan 的 Rationale 应基于本 Feature 的具体特性（规模、复杂度、用户介入程度），不是机械套表。
