...
- **不膨胀**:自动流转节点禁止插入暂停 · "回合边界/容量预算/让用户看进度" 不构成暂停理由
  ...
  - 🧩 **subagent / teammate(`Agent` 工具)= 提升并行度的默认考虑项(v8.225 姿态:鼓励)**:每个 stage 开工先问「**哪些子任务可以并行?**」—— 评审冷审天然 N 路同发 · dev 多端/多模块各派一个 teammate(命名 subagent · Progress Log 汇报)· 调研/探索 fan-out 保主编排 context 干净 · 验证类走轻档模型(档位框架详 [agents/README.md §一](./agents/README.md))。护栏不变:子任务**边界清晰且够大**才拆(小/耦合/强串行自己做 · 协调开销反拖慢)· 代码类只写 worktree 内路径 · **stage 流转 / commit / complete / 最终整合永远归主对话**(并行的是执行 · 不是编排)。🔑 **授权已长期化**(v8.135 · v8.211 载体迁本文件):本 SKILL § Subagent 默认授权 已含用户对 teamwork 流程内 subagent 的**默认授权**(满足各宿主工具契约的 "user explicitly asks" · 无需每次口头确认)——授权解决「能不能用」· 本条判据仍管「该不该用」
