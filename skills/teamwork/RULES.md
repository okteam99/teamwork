...
| **产品确认** | PRD 技术评审完成后 | PRD 初稿 → 🤖 PL-PM Teams 讨论 → PRD 定稿 → 主对话多角色评审 → 汇总问题，等待用户确认 |
| **设计确认** | UI 设计 Subagent 返回后 | 输出设计稿 + HTML 预览 → PMO 摘要，等待用户确认 |
| **评审问题** | TC 技术评审发现问题 | 汇总问题，等待用户决定修改/忽略 |
...
| **INFRA 影响面确认** | INFRA Feature 或技术类 PRD 影响 ≥3 个子项目 | 架构师方案评审 / PRD 技术评审发现影响面过大，需用户确认后继续 |
...
🔗 Blueprint Stage（stages/blueprint-stage.md）
    内部：QA 写 TC + TC 技术评审 + RD 写 TECH.md + 架构师评审
    ↓
    📊 PMO 摘要 → ⏸️ 用户确认技术方案
    ↓ 用户确认后
...
PM 验收 → PMO 完成报告（含 PROJECT.md 更新 + 全景确认 + 知识库总结）→ ✅
```

### 敏捷需求流程自动流转

> 敏捷需求 = Feature 流程的精简版。砍掉 Plan Stage 内的 PL-PM 讨论+技术评审、UI Design/Panorama、Blueprint Stage。
> 后半段（Dev → Review → Test → PM 验收）与 Feature 完全一致。
...
PM 编写精简 PRD → ⏸️ 用户确认 PRD
    ↓
QA Test Plan → QA Write Cases → RD 实现计划（🚀 自动串联）
    ↓
📋 PMO L2 预检
    ↓
...
└── Blueprint Stage（QA/RD/架构师评审在主对话简化执行）
```
...
├── Architect Code Review Subagent（Dev Stage 内部） → 预期 5-10 分钟
...
⚠️ 同一回复中可包含多个角色的产出：
├── PM 输出 PRD 初稿 → PL-PM Teams 讨论 → 收敛后 → PRD 多角色评审
├── QA 输出 TC → TC 多角色评审
└── RD 开发完成 → 同一回复中 Designer UI 验收（如有）

🔴 自动流转不可跳过的阶段（即使无待确认项也必须显式执行）：
├── 架构师方案评审 → 禁止以「改动小」为由跳过
├── 架构师 Code Review → 禁止从 RD 自查直接跳到 QA 验证
├── QA 代码审查 → 禁止跳过
