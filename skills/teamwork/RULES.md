...
2. 🔴 选项编号化（v7.3.5）—— 用户回复一个数字即可
   ├── ✅ 所有可选项必须以 1/2/3... 编号列出
   ├── ✅ 第一项是 PMO 推荐（与 💡 建议 一致，标注「推荐」）
   ├── ✅ 最后一项始终为「其他指示（自由输入）」
   ├── ✅ 提示语统一："⏸️ 请回复数字（1/2/3...）或其他指示"
   ├── ❌ 禁止写成描述性选项（"- 跳过" / "- 跑 Browser E2E"）
   ├── ❌ 禁止用字母（A/B/C）——统一用数字
   └── 用户可以回 `1` / `2` / `3`，或自然语言覆盖默认
...
4. 模糊确认处理
   ├── 用户回复**纯数字**（1/2/3）→ 直接映射到对应选项执行（编号化的好处）
   ├── 用户回复 ≤5 字且非数字、未提及流程名/阶段名 → PMO 复述阶段链 + 等二次确认
