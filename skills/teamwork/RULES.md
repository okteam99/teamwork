...
🔗 PM 验收（v7.3.10+P0-15，三选项暂停点）
    ...
    ↓ 用户选 1 后
🔗 Ship Stage（v7.3.10+P0-15 MR 模式）
    Step 1: 净化（灰名单策略保留）
    Step 2: git push origin {feature branch} + 识别 git_host + 生成 MR/PR create URL
    Step 3: ⏸️ worktree 清理确认（worktree=off 跳过）
    🔴 PMO 不做本地 merge / push merge_target / 冲突解决；合并权由平台和用户处理
    ↓
