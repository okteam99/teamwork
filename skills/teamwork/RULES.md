...
│   └── PRD 确认后变更规则：
│       ├── 小幅调整（不影响核心逻辑）→ PM 直接修改，不重新评审
│       ├── 重大修改（影响核心逻辑/范围/架构）→ PM 修改后重新触发 prd-review Subagent
│       └── 判定标准：PMO 评估变更是否涉及 RD/QA 评审中提出的已解决问题，涉及则重新评审
├── UI 设计完成后（必须等用户确认设计）
├── 复杂技术方案（必须等用户确认方案）
...
**🔴 核心原则：优先合理方案，而非简单方案**
```
RD 在选择修复方案时，必须优先考虑方案的合理性而非简便性：
├── 优先修复根因（root cause），而非绕过症状
├── 如果根因修复改动较大，仍应选择根因修复 → 走复杂方案流程
├── 仅当问题确实简单（如拼写错误、配置值错误等）时，才适用简单方案
├── 极端情况下允许症状修复（如根因修复需重构核心架构且时间紧急），
│   但必须在排查报告中说明为何不做根因修复 + 后续根因修复计划
└── 🔴 RD 不能因为想走简化流程而刻意把方案往"简单"方向靠
```

满足以下全部条件时，RD 可向用户申请跳过技术方案和 TDD：

```
├── 简单 bugfix（定位明确，改动 1-2 个文件，且修复的是根因而非症状）
├── 单文件内的功能实现
...
🤖 集成测试 Subagent 返回（测试报告 + 问题清单）
    ...
    ↓
    📊 PMO 阶段摘要
    ├── 全部通过（或剩余未通过项已被用户标记为 Known Issue / Deferred）→ 🚀 自动进入 PM 最终验收
...
处理依赖请求（其他模块对我的请求）：
├── PMO 启动时扫描 → 主动提醒（见 SKILL.md Step 0-A3）
