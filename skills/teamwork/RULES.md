...
| **Backlog 确认** | Feature Planning 流程 | PM 完成 BACKLOG.md 后，必须等用户确认优先级和依赖关系 |
...
PM 完成 BACKLOG.md（docs/BACKLOG.md）
    ↓
    📊 PMO 阶段摘要
    ↓ ⏸️ 等待用户确认 Backlog（优先级 + 依赖关系）
...
用户确认 Backlog
    ...
    ↓
    每个 Feature 完成时 → 更新 BACKLOG.md 状态列
```
...
└── QA 集成测试 → 详见 agents/integration-test.md
...
├── QA 集成测试：QA 前置检查通过后自动触发（Feature 流程）
├── 简单 Bug / 非架构变更的复杂 Bug：不触发架构师 Code Review
└── Feature Planning 流程：不使用 Subagent（需要与用户交互，PM 在主对话中执行）
```
...
具体 prompt 格式详见 agents/README.md 第 3.1 节（区分多子项目/单项目模式）。

关键参数：
├── 多子项目模式：必须传递子项目缩写、子项目路径、文档目录
├── 单项目模式：传递功能编号和项目根目录即可
└── 所有模式：必须传递 agents/README.md 和角色规范文件的绝对路径
```
...
用户切换功能：
├── /teamwork status     → 显示所有功能状态（按子项目分组，多子项目模式）
├── /teamwork AUTH-F001 继续   → 继续指定子项目的指定功能
├── /teamwork F001 继续   → 继续指定功能（单项目模式）
...
```
多子项目：OPT-{缩写}-{功能编号后三位}-{三位数字}（如 OPT-AUTH-001-001）
单项目：OPT-{功能编号}-{三位数字}（如 OPT-F001-001）
