...
| **🔴 跨项目需求拆分** | PMO 拆分方案输出后 | 拆分方案必须暂停等用户确认后才能开始各子项目流程 |
| **产品确认** | PRD 评审 Subagent 返回后 | 输出 PRD → 🤖 Subagent 多角色评审 → 汇总问题，等待用户确认 |
...
用户确认 Roadmap（ROADMAP.md 状态更新为：✅ 已确认）→ Planning 完成
    ...
    ↓
    每个 Feature 完成时 → 更新 ROADMAP.md 状态列
```
...
8️⃣ 下一步建议
9️⃣ 📐 文档一致性校验（每完成 3 个 Feature 后触发，或用户主动请求。🔴 触发时必须执行！）
   ├── 抽查 PROJECT.md 与实际代码功能是否一致（功能模块描述是否过时）
   ├── 抽查全景设计 sitemap.md 与实际页面路由是否一致
   ├── 抽查 ROADMAP.md Feature 状态是否全部更新
...
提交命令示例：
```bash
git add packages/auth-service/src/ packages/auth-service/tests/ packages/auth-service/docs/features/AUTH-F001-xxx/
