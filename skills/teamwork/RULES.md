...
| 查合法转移路径 | §四 阶段状态转移表（5 个流程 + PL 模式） | `#### Feature 流程` 起 |
...
| 文件 | 内容 | 加载时机 |
|------|------|----------|
| [rules/flow-transitions.md](./rules/flow-transitions.md) | 阶段状态转移表（8个流程） | PMO 流转校验时 |
| [rules/gate-checks.md](./rules/gate-checks.md) | 门禁规则 + 校验输出格式 | PMO 每次阶段变更时 |
...
**🔴 「流程类型确认」与「方案确认」必须独立**：
```
PMO 初步分析的暂停点必须包含两个明确的确认项：
├── (1) 流程类型确认：「请确认走 {流程名} 流程」
├── (2) 方案/范围确认：「请确认以上分析和影响范围」
├── 用户回复「方案可以」≠ 确认了流程类型
├── 用户回复「走这个流程」≠ 确认了方案细节
└── 🔴 两者都得到明确回复后才能进入下一阶段
    （可以在同一条回复中确认，但 PMO 必须同时列出两个确认项）
```

### 🔴 PMO 暂停点提问必须锚定流程（禁止暗示直接执行）

**PMO 在暂停点向用户提问时，措辞必须锚定到流程选择上，禁止用"要我做 X 吗？"的表述暗示确认即执行。**

```
🔴 提问措辞规范：
├── 禁止：「要我 {做某事} 吗？」→ 暗示"确认就直接干"，绕过流程
├── 必须：「确认按 {流程名} 流程推进？我会切换到 {角色} 开始 {阶段}。」→ 锚定到流程
└── 如果用户的回答可能被误读为"直接执行" → PMO 必须追问明确意图

示例对照：
├── ❌ 「要我参照现有服务创建 staging 部署配置吗？」
│   → 用户说「需要」→ PMO 可能直接写文件，跳过 Feature 流程
├── ✅ 「确认按 Feature 流程推进？我会切换到 PM 开始写 PRD。」
│   → 用户说「确认」→ 进入标准 Feature 流程
├── ❌ 「要我把这个逻辑加上吗？」
├── ✅ 「这属于 Feature 变更，确认走 Feature 流程？」
├── ❌ 「要我修复这个 bug 吗？」
└── ✅ 「确认按 Bug 处理流程推进？我会切换到 RD 开始排查。」
```

**🔴 执行要求**：
```
├── 适用于所有 ⏸️ 暂停点，无例外
├── PMO 已输出流程分析（A/B/C 选项）→ 提问必须回引流程选项，禁止跳到具体执行动作
├── 用户回复「需要」「对」「是的」等简短确认 → 仅视为确认流程选择，不视为确认直接执行
└── 违反本规则等同于违反红线 #3（禁止擅自简化）
```

### 🔴 模糊确认处理规则（禁止把「好」当万能通行证）

**用户回复「好」「可以」「按建议」「行」等模糊确认时，PMO 必须先复述即将执行的完整阶段链，再等用户二次确认。**

```
触发条件：用户确认回复 ≤ 5 个字 且 未明确提及流程名/阶段名
处理流程：
├── (1) PMO 复述：「收到，即将按 {流程名} 流程推进，完整阶段链如下：{阶段1} → {阶段2} → ... → {阶段N}」
├── (2) 等待用户二次确认（「确认」/「可以开始」等明确回复）
├── (3) 二次确认后才切换角色开始执行
└── 🔴 禁止：把模糊确认直接视为对流程+方案+执行的全面授权

豁免条件：
├── 用户回复中明确包含流程名（如「走敏捷」「按 Feature」）→ 无需复述
├── 用户回复中明确包含阶段名（如「开始写 PRD」「切到 RD」）→ 无需复述
└── 用户在同一暂停点已经做过一次明确确认，补充「好」表示同意附加建议 → 无需复述
```
...
🔴 强制规则（双向约束）：
├── 红线 5：暂停点必须停——用户未明确回复「确认」「OK」「可以」等前，禁止进入下一阶段。「无问题」「评审通过」不等于「用户已确认」。即使评审无问题，也必须等用户明确确认！
└── 红线 12：非暂停点禁止停——决策表中标注 ✅ 的操作禁止插入选择/确认/询问，必须自动执行并继续。PMO 不得自创暂停点。

**自动执行 vs 必须暂停 决策表**（🔴 此表是唯一权威来源，表外操作默认自动执行）：
...
> 用户确认 Bug 判断结果后、进入简化/完整流程之前，PMO 必须输出该流程的完整阶段链作为 checklist。
> 目的：防止「简单」标签导致心理捷径，强制把阶段链写下来逐项对照执行。
...
简化流程示例：
├── 完整阶段链：QA 补充用例 → RD 修复 → RD 自查 → 🔴 架构师 Code Review → QA 验证 → PM 文档同步 → PMO 总结
└── 确认：按完整阶段链执行，逐项完成
```

**🔴 执行规则**：
```
├── 无论简单/复杂 Bug，都必须输出此块，无例外
├── 禁止不输出此块直接开始 RD 修复
├── 每完成一个阶段，PMO 摘要中对照阶段链标记 ✅，未到的标记 ⬜
└── 违反本规则等同于违反红线 #3（禁止擅自简化）
```
...
### 🔴 阶段切换预检（Pre-flight Check）

> 每次阶段完成即将切换到下一阶段时，当前角色/PMO 必须执行预检，防止凭记忆误判流转类型。

```
预检步骤：
├── (1) 读取 rules/flow-transitions.md 中当前流程的流转表
├── (2) 确认「当前阶段 → 下一阶段」的流转类型（自动 / 暂停 / 条件）
├── (3) 输出标注：📖 流转：{当前阶段} → {下一阶段}（{自动/⏸️暂停}）
└── 禁止凭记忆判断流转类型，必须查表

示例：
├── 📖 流转：QA Write Cases → TC 评审（自动）
├── 📖 流转：PRD 评审 → Designer（⏸️ 暂停，等用户确认）
└── 📖 流转：RD 自查 → 架构师 Code Review（自动）
```
...
🔴 设计原则：
├── Dev Chain：RD 开发 + 架构师 CR + 内部修复循环 → 一个 Subagent 完成
├── Verify Chain：QA 审查 + 单元测试 + 集成测试 + API E2E → 一个 Subagent 完成
...
🤖 Dev Chain Subagent（规范：agents/dev-chain.md）
...
🤖 Codex Code Review Subagent（规范：agents/codex-code-review.md）
...
🤖 Verify Chain Subagent（规范：agents/verify-chain.md）
  ├── 内部流程：QA 代码审查 → 单元测试门禁 → 集成测试 → API E2E
  ├── ✅ DONE → 继续
  ├── 🔁 QUALITY_ISSUE（代码问题）→ PMO dispatch RD Fix → 重新 Verify Chain
...
🔴 Verify-Fix 循环规则：
├── Verify Chain 返回 QUALITY_ISSUE → PMO dispatch 小型 RD Fix Subagent
├── RD Fix 完成 → PMO 重新 dispatch Verify Chain（默认全量重跑）
├── 如果修复范围极小且 Verify Chain 建议断点续跑 → PMO 可选择从断点开始
...
> 此表覆盖全部五类流程（Feature / Bug 处理 / 问题排查 / Feature Planning / 敏捷需求）+ PL 模式 + 通用特殊状态。
...
```
PMO 启动阶段 X 前：
├── 1. 检查项目根目录是否存在 `CLAUDE.md` / `AGENTS.md` / `GEMINI.md`
│   ├── 存在 → 必须先读取并提取与当前任务相关的约束
│   └── 不存在 → 继续
├── 2. 查阅「阶段状态转移表」→ 确认当前阶段到阶段 X 的路径合法
├── 3. 验证前置阶段产物是否存在：
│   ├── PRD 确认后才能进 QA/Designer
│   ├── TC 评审通过后才能进 RD 技术方案
│   ├── 技术方案确认后才能进 RD 开发
│   └── 架构师 Code Review 通过后才能进 QA 代码审查
├── 4. 验证暂停点是否已获用户确认（检查对话历史中是否有用户明确回复）
└── 5. 如果门禁不通过 → 🔴 禁止进入下一阶段，输出缺失项提醒用户
...
项目根目录规则文件中的约束优先于 teamwork 默认规则；如有冲突，PMO 必须优先遵守仓库规则。
```
...
| PRD 待确认 | Designer 设计 / QA Test Plan | ⏸️ 用户确认（有 UI → Designer；无 UI → QA） |
| Designer 设计 | UI 待确认 | 设计 Subagent 完成 |
| UI 待确认 | QA Test Plan | ⏸️ 用户确认 |
| QA Test Plan | QA Write Cases | Plan 完成（自动流转） |
| QA Write Cases | TC 评审 | 三类 Case 完成（BDD + API E2E + Browser E2E） |
| TC 评审 | RD 技术方案 | 评审 Subagent 完成 |
| RD 技术方案 | 架构师 Review | 技术方案完成 |
| 架构师 Review | 技术方案待确认 | Review Subagent 完成 |
| 技术方案待确认 | RD 实现计划 | ⏸️ 用户确认 |
| RD 实现计划 | Dev Chain | 实现计划完成（自动流转） |
| Dev Chain | UI 还原验收 / Codex Review | DONE（有 UI → UI 验收；无 UI → Codex Review） |
| Dev Chain | ⏸️ 用户决策 | QUALITY_ISSUE / FAILED（≥3 轮内部修复未解决） |
| UI 还原验收 | Codex Review | ⏸️ 用户确认通过 |
| UI 还原验收 | Dev Chain（RD Fix） | 🔁 有问题 → RD 修复 |
| Codex Review | Verify Chain | DONE / DONE_WITH_CONCERNS |
| Codex Review | RD Fix → 重跑 Codex Review | 🔁 NEEDS_FIX（三方修复流程，≤3 轮） |
| Verify Chain | QA Browser E2E / QA Lead 质量总结 | DONE（有 Browser E2E → ⏸️ 用户确认；无 → QA Lead） |
| Verify Chain | RD Fix → 重新 Verify Chain | 🔁 QUALITY_ISSUE（≤3 轮 Verify-Fix 循环） |
| Verify Chain | ⏸️ 用户处理 | BLOCKED（环境问题） |
| QA Browser E2E | QA Lead 质量总结 | 通过 |
| QA Browser E2E | RD Fix → 重新 Browser E2E | 🔁 功能缺陷（≤3 轮） |
...
| 当前阶段 | 允许的下一阶段 | 条件 |
|----------|---------------|------|
| PMO 分析 | 精简 PRD 编写 | 用户确认走敏捷需求流程 |
| 精简 PRD 编写 | QA Test Plan | PRD 完成 |
| QA Test Plan | QA Write Cases | Plan 完成（自动流转） |
| QA Write Cases | RD 实现计划 | 三类 Case 完成（BDD + API E2E + Browser E2E）（🔴 敏捷砍掉 TC 评审，直接流转 RD） |
| RD 实现计划 | RD 开发+自查 | 实现计划完成（自动流转） |
| RD 开发+自查 | 架构师 Code Review | TDD Subagent 完成 + RD 自查通过 |
| 架构师 Code Review | QA 代码审查 | Review 通过（🤖 Subagent）（🔴 敏捷砍掉 QA Lead，QA 验证流程与 Feature 一致） |
| _后续 QA 阶段（代码审查 → 集成测试 → E2E → PM 验收）复用 Feature 流程定义_ | | |
...
📋 PMO L2 预检（见 common.md「PMO 预检流程」）
    ↓
🤖 Dev Chain Subagent（规范：agents/dev-chain.md，PMO 通过 Task 工具启动）
    内部流程：RD TDD 开发+自查 → 架构师 Code Review → 修复循环（≤3 轮）
    ↓
Dev Chain 返回（代码 + 测试 + RD 自查报告 + 架构师 Review 报告）
    ↓
    📊 PMO 阶段摘要
    ├── ✅ DONE → 📊 PMO 阶段摘要
    │   ├── 有 UI → 自动进入 Designer UI 还原验收
    │   └── 无 UI → 自动进入 Codex Review
    ├── ⚠️ DONE_WITH_CONCERNS → PMO 判断 concerns 严重性
    │   ├── 非阻塞 → 继续（同 DONE）
    │   └── 需用户判断 → ⏸️ 用户确认
    └── 🔁 QUALITY_ISSUE / 💥 FAILED → ⏸️ 升级给用户决定
    🚀 无待确认项，同一回复中继续！
...
UI 验收通过（或无 UI）
    ↓
🤖 Codex Code Review Subagent（规范：agents/codex-code-review.md）
    ├── 输入：PRD + TECH + TC + 代码变更（🔴 不给架构师 CR 报告）
    ├── ✅ DONE → 继续
    ├── 🔁 NEEDS_FIX → RD 出方案 → 架构师审核 → 修复 → 重跑 Codex（≤3 轮）
    └── ❌ FAILED → ⏸️ 用户选择跳过/重试
    ↓
    📊 PMO 阶段摘要 + 📋 PMO L2 预检（含 E2E 时升级 L3）（见 common.md「PMO 预检流程」）
    ↓ 🤖 PMO 启动 Verify Chain Subagent（规范：agents/verify-chain.md）
    └── Verify Chain 内部自动执行：
        ├── QA 代码审查（按 qa-code-review.md 完整执行）
        ├── 单元测试门禁（跑全量单元测试）
        ├── 集成测试前置检查 + 集成测试（按 integration-test.md 完整执行）
        └── API E2E（TC.md 标注需要时，按 api-e2e.md 完整执行）

🤖 Verify Chain Subagent 返回
    ↓
    📊 PMO 阶段摘要
    ├── ✅ DONE（全部验证通过）
    │   └── 继续读取 TC.md「Browser E2E 判断」（见下方）
    ├── 🔁 QUALITY_ISSUE（代码/测试问题）
    │   ├── PMO dispatch 小型 RD Fix Subagent → 修复 Verify Chain 报告中的问题
    │   ├── RD Fix 完成 → PMO 重新 dispatch Verify Chain（默认全量重跑）
    │   ├── 🔴 Verify-Fix 循环最多 3 轮，超出 → ⏸️ 用户
    │   └── 🔴 重新 Verify 范围：全量重跑（修复可能引入新问题）
    ├── ❌ BLOCKED（环境问题）→ ⏸️ 用户处理环境后重新 dispatch
    └── ⚠️ DONE_WITH_CONCERNS（TC 问题等）→ ⏸️ 用户确认处理方式

Verify Chain 通过后：Browser E2E 判断
    ↓
    PMO 读取 TC.md「Browser E2E 判断」章节：
    ├── Browser E2E = 需要 → PMO 给出建议 → ⏸️ 用户确认是否执行
    │   ├── 用户确认执行 → 🤖 Browser E2E Subagent（规范：agents/qa-e2e.md）
    │   │   ├── 功能缺陷 → RD 修复 → 重新 Browser E2E（最多 3 轮，超出 → ⏸️ 用户）
    │   │   └── 通过 ↓
    │   └── 用户明确跳过 → 记录原因 ↓
    └── Browser E2E = 不需要 / 无浏览器行为 → 自动跳过 ↓
...
> 敏捷需求 = Feature 流程的子集，砍掉 7 个评审/讨论环节，执行类环节行为与 Feature 完全一致。

```
PMO 初步分析（判断符合敏捷需求准入条件）
    ↓
📋 PMO 输出分析 + 准入条件逐项检查 + 📂 文档路径确认（✅ 自检通过）
    ↓
⏸️ 等待用户确认走敏捷需求流程
    ↓ 用户确认
🔄 PMO 切换到 PM
...
QA Write Cases（BDD case + API E2E case + Agent Browser E2E case）
    ↓ 🚀 自动（🔴 敏捷砍掉 TC 评审，直接流转 RD）
🔄 自动流转到 RD
RD 实现计划（文件清单 + 改动要点）
    ↓ 🚀 自动流转
📋 PMO L2 预检（见 common.md「PMO 预检流程」）
    ↓ 🚀 自动
🤖🔗 Dev Chain Subagent（与 Feature 一致：agents/dev-chain.md）
    内部流程：RD 开发+自查 → 🔴 架构师 CR（必须，不可跳过）→ 修复循环（≤3 轮）
    ↓
    📊 PMO 阶段摘要
    ↓ 🚀 自动
🤖 Codex Code Review Subagent（agents/codex-code-review.md）
    独立外部审查：PRD+TECH+TC+代码变更 → Codex CLI Review → 报告
    NEEDS_FIX → 三方修复流程（RD 出方案 → 架构师审核 → 修复 → 重跑，≤3 轮）
    ↓
    📊 PMO 阶段摘要 + 📋 PMO L2 预检（含 E2E 时升级 L3）
    ↓ 🚀 自动
🤖🔗 Verify Chain Subagent（与 Feature 一致：agents/verify-chain.md，砍掉 QA Lead）
    内部流程：QA 代码审查 → 单元测试门禁 → 集成测试 → API E2E → Browser E2E（可选）
    ↓
    📊 PMO 阶段摘要
    ├── 全部通过 → PM 验收
    ├── 功能缺陷 → RD 修复 → 重新验证（最多 3 轮）
    └── 环境/用例问题 → ⏸️ 用户确认
    ↓ 🚀 自动
🔄 自动流转到 PM
PM 验收通过
    ↓ 🚀 自动
🔄 回到 PMO
PMO 完成报告（含事后审计：实际文件数 ≤ 3 + 阶段链完整性检查）
    ↓
敏捷需求完成 ✅

砍掉的 7 个环节（与 Feature 流程的差异，其余阶段规范完全复用 Feature）：
├── PL-PM 讨论
├── PRD 评审（Subagent）
├── Designer 设计
├── TC 评审（Subagent）
├── 技术方案文档
├── 架构师方案 Review（Subagent）
└── QA Lead 质量总结（Subagent）
```
...
├── PMO 摘要 → ⏸️ 等待用户确认设计稿
└── 用户确认后 → 继续 QA Test Plan → QA Write Cases
...
```
[PMO 启动 Dev Chain Subagent]
[Dev Chain 返回 DONE：代码 + 测试 + 自查报告 + CR 报告]

📊 PMO 阶段摘要
├── ✅ 已完成：Dev Chain（RD 开发+自查+架构师 CR）
├── 📌 下一步：Codex Review
├── 🔴 待确认：无
└── 📋 整体进度：7/12
...
├── 📌 下一步：L2 预检（含 E2E 时升级 L3）+ Verify Chain
├── 🔴 待确认：无
└── 📋 整体进度：8/12

[执行 L2/L3 预检 → 启动 Verify Chain Subagent...]
```
...
❌ 输出 PMO 摘要后等待用户回复
❌ 每个阶段都问用户「是否进入下一阶段」
❌ PRD 完成后给出 A/B/C 选项让用户选择（PRD 评审是自动触发的）
...
🔴 自动流转不可跳过的阶段（即使无待确认项也必须显式执行）：
├── 架构师 Tech Review → 必须启动 Subagent 执行，禁止以「改动小」为由跳过
├── 架构师 Code Review → 必须显式执行，禁止从 RD 自查直接跳到 QA 验证
├── QA 代码审查 → 必须启动 Subagent 执行
└── 违反 = 流程不完整，PMO 必须回退到被跳过的阶段补执行
```
