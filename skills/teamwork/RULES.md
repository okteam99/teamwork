...
**必须保留的步骤**：RD 排查报告 → QA 补充测试用例（前置） → RD 修复 → RD 自查（完整自查，同 Feature 标准） → QA 验证用例 → PM 文档同步检查（判断是否需要更新 PRD/UI/TC） → PMO 判断总结 + 知识库更新 → PMO 结束流程（Bugfix 最终状态必须由 PMO 输出）
...
| Roadmap 待确认 | PMO 初步分析 | ⏸️ 用户确认，逐个启动 Feature |
...
| CHG 待确认 | PM Roadmap 编写 / PMO 初步分析 | ⏸️ 用户确认（级联 Planning → Roadmap；直接 Feature → PMO） |
