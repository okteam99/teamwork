...
> Micro = 零逻辑变更的最轻量通道。砍掉 QA、架构师、Codex 全部环节，只保留 PMO 分析 + 用户确认 + PMO 自行判断执行方式 + 用户验收。
> 🟢 v7.3 放宽（P0-16 明确）：Micro 流程下 PMO 可直接改代码（白名单内零逻辑变更），无需 Subagent / Execution Plan / dispatch 文件。若执行前判定超出 Micro 准入 → PMO 升级为 Plan 模式（走敏捷或 Feature）。
...
PMO 自行判断执行方式：
    ...
    │   ├── PMO 阶段摘要中 cite 1-2 句相关规范要点（证明真实 Read）
    │   ├── 按变更清单改文件 + 跑项目已有测试确认无回归
