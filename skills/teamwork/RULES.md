...
>
> **典型反模式（必须杜绝）**：AI 看到用户表达疑虑就**预测性简化**——主动提议"考虑到您说的情况，我建议跳过 X 步骤"。这违反红线 #3（禁止擅自简化）和红线 #12（非暂停点禁止暂停 / 不得自创豁免）。
...
3. 不主动建议跳过
   ...
   └── 🔴 禁止把"步骤简单"作为主动建议跳过的理由（红线 #3：「需求简单」「改动文件少」「纯移植」「技术风险低」不构成跳过流程阶段的理由）

4. 用户明确说「跳过」才豁免（红线 #3 兜底）
   ├── 仅当用户**显式且无歧义**说出「跳过 X」「不要 X」「省略 X」「我决定跳 X」时，才进入豁免分支
   ├── 豁免分支也必须先 cite 红线 #3 + 走暂停点确认（💡 + 📝 给跳过决策记录）
...
🟡 **当前继续执行原则**
按规范继续执行该步骤。如您希望跳过，请明确回复「跳过 {步骤名}」，PMO 会按红线 #3 兜底流程登记跳过决策（含理由 + state.json 写入）。
```
...
```
红线 #3（禁止擅自简化）   → 本规则规则 3-4 的硬基础
红线 #12（非暂停点禁止暂停）→ 本规则规则 3 防御「自创豁免」反模式
红线 #14（AI Plan 模式 + 声明即承诺）→ 本规则规则 1 强化"先 cite 规范"
```
...
4. 模糊确认处理
   ...
   │   ├── 🔴 前置条件：当前暂停点**必须至少有一个 💡 推荐**（红线 #10：暂停点必须给 💡 + 📝）
   │   │   └── 若罕见情况下无 💡（红线 #10 违反）→ 不适用本约定，退回到下方"≤5 字且非数字/字母"路径做复述+二次确认
...
> 📎 准入条件、白名单、规则详见 [FLOWS.md](./FLOWS.md)「六、Micro 流程」。Micro 不是红线 #1 的例外，是独立流程。
...
处理依赖请求（其他模块对我的请求）：
├── PMO 启动时扫描 → 主动提醒（见 stages/init-stage.md Step 0-A3）
