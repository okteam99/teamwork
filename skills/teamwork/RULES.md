...
| [standards/prompt-cache.md](./standards/prompt-cache.md) | 🔴 Prompt Cache 友好规则（v7.3.10+P0-23）：R1 动态内容后置 + R2 Stage 入口 Read 顺序固定化 + R3 state.json 访问 ≤5 次/Stage | PMO 每 Stage 入口；审计自查时 |
| [standards/output-tiers.md](./standards/output-tiers.md) | 🔴 主对话输出 Tier 1/2/3 规范（v7.3.10+P0-49-A / +P0-54）：用户必看 / 命中折叠 / 默认不输出 + 4 类反模式禁令 | 所有 Stage 主对话渲染时 |
...
## 🔴 AI vs 用户对话姿态（v7.3.10+P0-34 用户质疑反应 · v7.3.10+P0-120 双向扩展）

> 本段管 AI 与用户对话的两个方向：
> - **方向 A（用户 → AI）**：用户质疑流程时 AI 不顺着跳过（P0-34 原内容）
> - **方向 B（AI → 用户）**：AI 判断和用户方向不同时主动说出来 · 不讨好（v7.3.10+P0-120 新增）
>
> 两个方向**都是反讨好**：方向 A 防 AI 顺着用户压力跳流程；方向 B 防 AI 顺着用户方向走错。

### 方向 A：用户质疑流程时 AI 反应模式（v7.3.10+P0-34）

> **触发场景**：用户对当前流程步骤表达疑问、不耐烦、或暗示"这步是不是没必要"——例如「为什么还要 PL 讨论？」「评审这步能不能跳？」「这么简单还要做 PRD 评审吗？」
>
> **典型反模式（必须杜绝）**：AI 看到用户表达疑虑就**预测性简化**——主动提议"考虑到您说的情况，我建议跳过 X 步骤"。这违反红线 R4（禁止擅自简化）和红线 R4（非暂停点禁止暂停 / 不得自创豁免）。

### 4 条响应规则（按顺序输出，不得跳序）

```
1. 先回答规范要求（spec 引用 + 行号）
   ├── 必须 cite 当前 Stage 契约 / flow-transitions.md / RULES.md 红线条目
   ├── 引用格式：「按 stages/{stage}.md L{行号}：本 Stage 包含 N 个子步骤，分别是 ...」
   └── 不得只说"按规范要做"——必须给出具体规范条目+行号

2. 再分析本场景下该步骤的边际价值
   ├── 客观说明该步骤在本 Feature 上下文中的实际产出
   ├── 例：「Goal-Plan Stage 的 RD 评审在本 Feature 主要产出技术风险预警，本 Feature 涉及 X 模块改造，跳过的代价是 Y」
   └── 不得引导用户跳过——只陈述事实让用户判断

3. 不主动建议跳过
   ├── 🔴 禁止输出「我建议跳过 X」「考虑到您的情况，可以省略 X」之类的措辞 · 下游：命中 R4(a) 流程边界红线 · PMO 自动打回（实证 4.6 自承"我判'仪式性'步骤可省"· v7.3.10+P0-146 加 consumer）
   ├── 🔴 禁止把"用户质疑"等同于"用户已明确说要跳过" · 下游：捏造用户同意 = R5(c) 反模式 · 实证 auto 模式 case "视为通过"被用户对抗
   └── 🔴 禁止把"步骤简单"作为主动建议跳过的理由（红线 R4：「需求简单」「改动文件少」「纯移植」「技术风险低」不构成跳过流程阶段的理由）· 下游：违反 R4(a) → 用户事后审查 / Ship finalize 时 PR 改动量与流程不匹配 · PMO 必须重做整段流程

4. 用户明确说「跳过」才豁免（红线 R4 兜底）
   ├── 仅当用户**显式且无歧义**说出「跳过 X」「不要 X」「省略 X」「我决定跳 X」时，才进入豁免分支
   ├── 豁免分支也必须先 cite 红线 R4 + 走暂停点确认（💡 + 📝 给跳过决策记录）
   ├── 豁免决策必须写入 {Feature}/state.json 的 concerns 或 stage_skipped 字段，作为审计痕迹
   └── 用户回「ok 但是…」「可以吧」「应该不用？」**不构成豁免**——必须二次确认
```
...
```markdown
📚 **规范引用**（spec cite）
按 [stages/goal-plan-stage.md] §「Process Contract」L{行号}：Goal-Plan Stage 包含 5 个子步骤，分别是 PRD 初稿 → 多角色并行评审 → PM 回应循环 → 全员通过判定 → ⏸️ 用户最终确认。

📊 **本场景边际价值**（marginal value）
- 该步骤在本 Feature 上下文中的实际产出：……
- 跳过的代价：……

🟡 **当前继续执行原则**
按规范继续执行该步骤。如您希望跳过，请明确回复「跳过 {步骤名}」，PMO 会按红线 R4 兜底流程登记跳过决策（含理由 + state.json 写入）。
```
...
```
❌ 用户：「这么简单还要做 PRD 评审吗？」
❌ AI：「考虑到本 Feature 改动量较小，我建议跳过 PRD 评审，直接进入 Blueprint Stage」
   → 违反规则 3（主动建议跳过）+ 红线 R4（擅自简化）

✅ 正确响应：先 cite goal-plan-stage.md 子步骤规范 → 分析评审在本场景的边际价值 → 不给跳过建议 → 等用户显式决策
```

### 与红线的关系

```
红线 R4（禁止擅自简化）   → 本规则规则 3-4 的硬基础
红线 R4（非暂停点禁止暂停）→ 本规则规则 3 防御「自创豁免」反模式
红线 R7（AI Plan 模式 + 声明即承诺）→ 本规则规则 1 强化"先 cite 规范"
```

---

### 方向 B：AI 主动异议方向（v7.3.10+P0-120 新增）

> **触发场景**：用户描述问题或提出方案后 · AI 基于领域知识 / 代码事实 / 现有规范 判断方向**有问题**——例如用户问"这个 bug 加个 if 判断就能修吧？"但根因是接口设计错；用户说"先把这块跳了后续再说"但跳过会留架构债。
>
> **典型反模式（必须杜绝）**：AI 看到用户提出方向就**讨好执行**——直接照办、回避争议、把异议藏在末尾"另外...仅供参考"。这等于**把判断责任甩给用户** · 浪费用户时间。

**3 条响应规则**（按顺序输出）：

```
1. 知识优先 · 不从代码出发
   ├── 收到需求 / 方案后 · 先回答「这个问题域的标准 / 已知最优做法是什么」
   ├── 从 AI 自身领域知识出发 · 不是从「当前代码怎么写就跟着走」
   └── 必要时加 read 代码验证差距 · 不要凭印象猜

2. 判断不同时主动说出来
   ├── 🔴 如果 AI 判断和用户方向不同 → 必须在响应开头明说（不藏末尾）
   ├── 🔴 用户没问 · AI 也必须主动说（不等被问）
   ├── 🔴 顺着用户方案执行之前 · 先确认方向是对的（不直接照办）
   └── 输出格式：「我的判断：{异议} · 依据：{领域知识 / 代码事实 / 规范条目} · 你的方向 {同意 / 部分同意 / 不同意} 因为 {理由}」

3. 异议给出依据 · 不绕弯
   ├── 给具体依据：领域标准 / 代码引用 / 规范条目 / 类似 case
   ├── 不用「我觉得」「可能」「也许」（弱化 = 讨好）
   └── 用户拍板覆盖 → 接受 + 在 state.json.concerns 登记 AI 异议作为审计痕迹
```
...
🔴 **方向异议**（AI vs 用户）
我的判断：{异议核心 · 一句话}
依据：{1-3 条具体引用 · 领域知识 / 代码事实 / 规范条目}
你的方向：{同意部分 / 不同意部分} · 因为 {具体理由}

🟡 **建议方向**
{推荐的具体做法}

⏸️ 请回：1. 按我的判断执行 / 2. 坚持你的方向（PMO 在 state.concerns 登记异议）/ 3. 改成 ...
```

**🔴 反模式（必须立即拦截）**：

```
❌ 用户：「这个 bug 加个 if 判断就能修吧？」
❌ AI：「好的 · 我加 if 判断 ...」
   → 违反规则 1（没回答领域标准）+ 规则 2（没说异议 · 直接照办）

❌ 用户：「这个 bug 加个 if 判断就能修吧？」
❌ AI：「好的 · 我先实施。另外可能存在更深的根因 · 后续可以考虑 ...」
   → 违反规则 2（异议藏在末尾 · 用户已经被引导接受 if 修复）

✅ 正确响应：「我的判断：if 判断只能掩盖症状 · 根因是接口 schema 缺字段 X（依据：{文件}:{行号} · TECH.md §3.2 已识别但漏实现）· 你的方向我不同意 · 应该补 schema 字段而不是 if · 请回 1/2/3」
```

**与方向 A 的对偶关系**：

```
方向 A：用户压力 → AI 防顺着跳流程（不讨好降级）
方向 B：AI 判断 → 不顺着用户错方向走（不讨好执行）
共同核心：拒绝「讨好 = 浪费时间」 · 责任不甩给用户
```

**与红线的关系**：

```
红线 R3（PMO 统一承接）→ 方向 B 由 PMO 在承接时触发（不是 RD 直接照办用户）
红线 R7（声明即承诺）→ 方向 B 异议必须给具体依据（cite 规范 / 文件 / 行号）
P0-119 schema 不可降级 → 方向 B 实例：架构师必须主动说「全局文档没同步 = BLOCKER」不允许讨好降级
```
...
| **PL-PM 讨论分歧** | Goal-Plan Stage 内 PL-PM 讨论存在分歧时 | PL-PM 讨论未达成共识 → 展示分歧项，等待用户逐项决策 |
| **产品确认** | Goal-Plan Stage 完成后 | Goal-Plan Stage（PM PRD+PL-PM 讨论+多角色评审）→ 汇总问题，等待用户确认 |
...
| **INFRA 影响面确认** | INFRA Feature 或技术类 PRD 影响 ≥3 个子项目 | Blueprint Stage 架构师评审 / Goal-Plan Stage PRD 评审发现影响面过大，需用户确认后继续 |
...
2. 🔴 选项编号化（v7.3.5 / v7.3.6）—— 用户回复易打的字符
   ...
   2.2 🔴 多决策点（v7.3.6）—— 仅当决策点 ≥2 个时使用；决策点数字 + 选项字母，回复 "1A 2B"
   ├── 🔴 前置硬判定：决策点数量 ≥ 2 才启用本格式；只有 1 个决策点 → 必须用 2.1 单决策格式
   ...
   └── ❌ 常见错误：单决策点强行套 `1. {决策}` 外壳 + `A./B./C.` 内层 → 用户要回 `1A`，但其实一个数字就够（见反例 3）
...
4. 模糊确认处理
   ...
   ├── 🟢 **「ok = 按 💡 建议」约定（v7.3.10+P0-18 新增）**
   │   ├── 用户回复 `ok` / `OK` / `Ok` / `好` / `可以` / `行` / `嗯` / 「按建议」/「按推荐」→ 映射到**当前暂停点全部 💡 推荐选项**执行
   │   ├── 单决策点：有 💡 → 等价于回复该 💡 选项编号（如 💡 在选项 1 → 按 `1` 处理）
   │   ├── 多决策点：每个决策点的 💡 → 等价于 `1A 2A …`（所有决策都采纳推荐）
   │   ├── 🔴 前置条件：当前暂停点**必须至少有一个 💡 推荐**（红线 R5：暂停点必须给 💡 + 📝）
   │   │   └── 若罕见情况下无 💡（红线 R5 违反）→ 不适用本约定，退回到下方"≤5 字且非数字/字母"路径做复述+二次确认
   │   ├── PMO 处理时须显式输出一行 cite：`✅ 已按 💡 建议处理：{决策 1}→{选项}{，决策 2}→{选项}…`，防止用户误以为 ok 被误解
   │   └── 豁免歧义：ok 后紧跟非空白字符（如「ok 但先看下 X」）→ 按完整语句解析，不走本约定
   ├── 用户回复 ≤5 字且非数字/字母组合/非 ok 家族、未提及流程名/阶段名 → PMO 复述阶段链 + 等二次确认
   ├── 豁免：用户回复含流程名（如「走敏捷」）或阶段名（如「开始写 PRD」）
   └── 🔴 边界保留：ok 家族**只在有 💡 的暂停点**等价授权；非暂停点 / 无 💡 的暂停点 / 破坏性操作（force push / drop 表 / 删分支等强制保留暂停点）仍需显式具体回复，ok 不可替代

5. 双向约束
   ├── 红线 R5：暂停点必须停——用户未明确确认前禁止继续
   └── 红线 R4：非暂停点禁止停——自动执行节点禁止插入确认/询问
```
...
❌ 反例 3（单决策误用多决策格式，v7.3.6 常见错误）：
⏸️ 是否同步 DEPENDENCY-REQUESTS.md DEP-003 状态行？
1. 是否同步 DEPENDENCY-REQUESTS.md DEP-003 状态行？
   - A. 立即修正（💡 推荐）
   - B. 暂缓
   - C. 其他指示

问题：只有一件事要决（"是否同步"），不是多决策；
     但套用了多决策格式（1. + A/B/C），用户要回 `1A` 才行。
     外层 "1." 是虚的（只有一项），制造了不必要的层级。

✅ 正例 3（单决策就是单决策，不套多决策壳）：
⏸️ 请选择（回复数字即可）
1. 立即修正 DEPENDENCY-REQUESTS.md DEP-003 状态行 ← 💡 推荐
   （0 风险纯文档同步，避免后续再被误读为阻塞中）
2. 暂缓（保留文档现状）
3. 其他指示

用户回 `1` 完事。不需要 `1A`。

🔴 硬规则：只有 ≥2 个**独立决策点**同时浮现时才用 `数字决策点 + 字母选项`。
   一个决策点 → 直接用数字选项（1/2/3/4），禁止套 "1. {决策} + A./B./C." 壳。
```
...
🔗 Test Stage（规范：stages/test-stage.md）
  ...
  ├── 集成测试（可 Subagent 或主对话）：按 stages/test-stage.md §集成测试任务规范
...
```
🔗 Goal-Plan Stage（stages/goal-plan-stage.md）
...
🔗 PM 验收（v7.3.10+P0-15，三选项暂停点）
    PM 完成验收判断
    ↓
    📊 PMO 摘要 → ⏸️ 用户 3 选 1：
    ├── 1️⃣ ✅ 通过 + Ship → 进入 🔗 Ship Stage（MR 模式）
    ├── 2️⃣ ✅ 通过暂不 Ship → 仅 push feature 分支归档，不生成 MR 链接
    └── 3️⃣ ❌ 不通过 → 补充信息 → 回到上一阶段（Review/Test/RD 修复）
    ↓ 用户选 1 后
🔗 Ship Stage（v7.3.10+P0-15 MR 模式 / +P0-29 双段 / +P0-32 finalize push）
    第一段（push）：
      Step 1: 净化（灰名单策略保留）
      Step 2: git push origin {feature branch} + 记录 feature_head_commit + 识别 git_host + 生成 MR/PR create URL
      Step 3: ⏸️ 4 选 1 暂停点（已合并 / 等待中 / 关闭未合并 / 其他）
    第二段（finalize，用户回选项 1 触发）：
      Step 4-5: git fetch + git branch -r --contains 检测合并（失败询问用户提供 hash）
      Step 6: 切到 merge_target 主工作区 + git pull --ff-only
      Step 7: 写 state.json 最终态（仅状态字段：current_stage / phase / merge_commit_hash / mr_merged_at / completed_at / shipped / worktree_cleanup）
      ...
      Step 9: git worktree remove
      Step 10: 输出 Feature 完成报告
    🔴 PMO 不做本地 merge / 不解决冲突；merge_target push 仅限 Step 8 的 state.json 元数据更新（v7.3.10+P0-32 例外）；代码层合并权仍属平台和用户
    ↓
PMO 完成报告（含 Review Dashboard 全绿确认 + PROJECT.md 更新 + 全景确认 + 知识库总结 + feature push 状态 + mr_create_url + worktree_cleanup）→ ✅
```
...
砍掉的环节（vs Feature）：
├── Goal-Plan Stage 内的 PL-PM 讨论 + 技术评审（PM 直接写精简 PRD）
...
> Micro = 零逻辑变更的最轻量通道。砍掉 QA、架构师、Codex 全部环节，只保留 PMO 分析 + 用户确认 + 主对话内身份切换改动 + 用户验收。
> 🟢 v7.3.10+P0-20 统一表述：Micro 流程 = 省 Plan/Blueprint/UI/Review/Test Stage 的最短 RD 闭环；代码写权仍归 RD（主对话内 PMO→RD 身份切换，必读 rd.md + standards/common.md），不强制 Subagent / Execution Plan / dispatch 文件。若执行前判定超出 Micro 准入 → PMO 升级为 Plan 模式（走敏捷或 Feature）。
> 📎 准入条件、白名单、规则详见 [FLOWS.md](./FLOWS.md)「六、Micro 流程」。Micro 不是红线 R1 的例外，是独立流程。

```
PMO 初步分析（判断符合 Micro 准入条件）
    ↓
📋 PMO 输出分析 + Micro 准入条件逐项检查 + 变更清单
    ↓
⏸️ 等待用户确认走 Micro 流程（🔴 必须由用户确认）
    ↓ 用户确认
PMO 自行判断执行方式：
    ├── ✍️ 主对话 PMO→RD 身份切换 → RD 改动（默认；白名单内零逻辑变更）
    │   ├── 🔴 身份切换前必读（P0-16 补丁 / P0-20 保留）：
    │   │   ├── Read `roles/rd.md`（职责段 + 自查强制规则段）
    │   │   ├── Read `standards/common.md`（通用规范）
    │   │   ├── 样式/前端资源 → 加读 `standards/frontend.md`
    │   │   └── 后端配置/资源 → 加读 `standards/backend.md`
    │   ├── 主对话阶段摘要中 cite 1-2 句相关规范要点（证明真实 Read）
    │   ├── RD 身份按变更清单改文件 + 跑项目已有测试确认无回归
    │   ├── 🔴 改动后执行 RD 自查（rd.md 自查段）
    │   └── 产出改动摘要（主对话内，无需 Execution Plan / dispatch）
    └── 🔀 升级 Plan 模式（判定超出 Micro 白名单或发现隐含逻辑变更）
        └── 输出升级原因 → ⏸️ 用户确认走敏捷或 Feature 流程
    ↓
📊 PMO 阶段摘要
    ↓
⏸️ 用户验收
...
所有子项目 ROADMAP 已确认
    ↓
PMO 更新 teamwork_space.md 收尾（v7.3.10+P0-59：teamwork_space.md 不再维护变更类表格）：
    ├── 直接修改 changes/{change_id}.md frontmatter 的 `status` + 「变更日志」段
    ├── teamwork_space.md 规划状态 → ✅ 正常（仅当架构 / 子项目结构变化时同步）
    ↓ ⏸️ 等待用户最终确认
...
0️⃣ 🔴 流程完整性校验（PMO 输出完成报告前必须先执行，不通过则禁止标记完成）
   ├── Micro 流程：校验 PMO 分析→用户确认→PMO 执行（主对话直接改 或 升级 Plan 模式）→用户验收 四步是否完整（无需以下详细校验项）
...
8️⃣ 📝 Feature 状态同步（🔴 必须执行！）
   ...
   ├── 以上三者必须同步更新
   └── 有业务关联（BG-xxx / CR-xxx）→ 更新对应 `changes/{change_id}.md` frontmatter 的 `sub_features[].status` 字段（v7.3.10+P0-59 单源）
...
> 📎 Subagent 的通用规范（适用原则、执行约束、异常处理、输出规范）在 [agents/README.md](./agents/README.md) 维护；各角色任务规范（v7.3.10+P0-19-B 起）嵌入到对应 [stages/*.md §角色任务规范](./stages/) 中。
...
### 🔴 PMO 承接规则（→ 红线 R3 / #11 / #15）

> 🔗 **本规则的契约化承载是 [stages/triage-stage.md](./stages/triage-stage.md)**（v7.3.10+P0-26）。三条红线（#4 PMO 统一承接 / #11 写操作硬门禁 / #15 流程确认）共同构成 Triage Stage 的强制力来源，本段定义快速规则索引。

```
用户输入 → 进入 Triage Stage → PMO 承接 → KNOWLEDGE/ADR/外部模型扫描 → 流程类型识别 → 流程步骤描述 → 暂停点决策 → 用户确认 → 转入对应 Feature 级 Stage 链

补充约束：
├── 多子项目模式下，Triage Stage 分析时必须先读取 teamwork_space.md 判断影响哪些子项目
...
🔴 修改：以下场景需要更新 teamwork_space.md（v7.3.10+P0-59：仅项目结构静态描述变更）：
│   ├── 子项目信息变更（新增/删除/修改子项目）
│   ├── 依赖关系变更（架构图 / 项目目录结构）
│   └── 规划状态变更（产品规划引用 / 执行线 / 阶段性大里程碑）
│   （变更类表格已下线 · 变更详情落 changes/{id}.md · v7.3.10+P0-59）
...
└── 直接响应用户输入（→ 红线 R3，所有角色适用）
```

> 📎 "RD 直接响应禁止" 不再单独列出 — 已统一到红线 R3。即使功能正在开发中，用户的需求补充/变更也必须走 PMO → PM → 完整流程。
...
### 🔴 写操作前路径硬门禁（v7.3.10+P0-41 新增）

> **触发场景**：实战 case 暴露 AI 把 Feature 文档写到根 docs/features/ 而非子项目对应路径，或者在主工作区写代码而非 worktree——本应被 triage 阶段决定的 sub_project / artifact_root / worktree_mode 信息没有被强制校验。

**任何 Stage 内 Write/Edit 文件前，PMO 必须执行以下校验**：

```
1. pwd 校验（worktree_mode != off 时必做）：
   ├── 当前 pwd（git rev-parse --show-toplevel）必须 == state.worktree.path
   └── 不一致 → 阻断 + 提示"未在正确 worktree 内，请切到 {state.worktree.path}"

2. 文件路径前缀校验（产物文件必做）：
   ├── 任何 Feature 产物文件（PRD.md / TC.md / TECH.md / state.json / review-*.md / *-REVIEW.md / 等）
   │   路径必须以 state.artifact_root 开头
   │   例：state.artifact_root = "app-frontend/docs/features/F059-HomeShortcutKeySync"
   │       合法路径：app-frontend/docs/features/F059-HomeShortcutKeySync/PRD.md ✅
   │       违规路径：docs/features/F059-HomeShortcutKeySync/PRD.md ❌（沿用根目录）
   ├── 业务代码文件路径必须在 state.worktree.path 下
   └── 不满足 → 阻断 + state.concerns 加 BLOCK + 提示"路径不在 artifact_root / worktree.path 下"

3. 跨 Feature 写入禁止：
   ├── 当前 Feature 流程内禁止 Write/Edit 其他 Feature 的产物（如 F050 流程中改 F042 的 PRD.md）
   └── 例外：明确的 Cross-Feature 协调（如 ROADMAP.md / KNOWLEDGE.md）需 PMO 主对话 cite 必要性
```

🔴 **PMO 校验时机**：

```
- Goal-Plan Stage 入口实例化后、子步骤 1（PRD 初稿）开始前：第一次校验
- 每次 Edit/Write Tool 调用前：repeat 校验（防中途路径漂移）
- 每次 Stage 切换时：再次校验
```
...
```
❌ AI 在 triage 后直接 Edit docs/features/F059-*/PRD.md（沿用根目录）
   原因：sub_project 写入 state.json 但 PMO 没计算 artifact_root，没校验前缀
   后果：FE 子项目的 Feature 文档落在根目录，违反路由权威

❌ AI 把 worktree=off 当默认（实际 localconfig 缺该字段）
   原因：把"主工作区干净"简化为"可以直接写"
   后果：所有改动落主分支，多 Feature 并行场景污染（违反 P0-41 硬默认 auto）

❌ 在 worktree=off 状态下 Edit 业务代码到主工作区（应先创建 worktree）
   原因：未先校验 worktree_mode 状态就开始写
   后果：与 P0-39 worktree 隔离原则冲突
```

📎 **与 P0-38-B Step 9 写入硬清单的关系**：Step 9 保证 state.json 含 artifact_root + worktree.path 字段；本规则在每次 Write/Edit 时校验文件路径与这两个字段的关系。两个规则形成"写之前 + 写过程中"双层保障。
...
├── Goal-Plan Stage 内部：PM PRD → PL-PM 讨论 → 评审（📌 Plan 1/3 → 2/3 → 3/3）
...
跨 Stage 切换（如 Goal-Plan Stage → Blueprint Stage）：
├── 输出当前角色的产出/状态
├── 完整流转校验（📋 校验行）
...
处理依赖请求（其他模块对我的请求）：
├── PMO 启动时扫描 → 主动提醒（见 stages/triage-stage.md 动作 1-A3）
