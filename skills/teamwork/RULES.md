...
3. 模糊确认处理
   ├── 用户回复 ≤5 字且未提及流程名/阶段名 → PMO 复述阶段链 + 等二次确认
...
🔗 UI Design Stage（stages/ui-design-stage.md，仅有 UI 时）
    内部：Designer 设计当前 Feature UI（不动全景）
    ↓
    📊 PMO 摘要 → ⏸️ 用户确认设计稿
    ↓ 用户确认后（有问题 → 重新 dispatch，≤3 轮）
    ├── 涉及全景更新 → Panorama Design Stage
    └── 不涉及 → Blueprint Stage

🔗 Panorama Design Stage（stages/panorama-design-stage.md，涉及全景时）
    内部：Designer 同步 sitemap.md + overview.html
    ↓
    📊 PMO 摘要 → ⏸️ 用户确认全景
    ↓ 用户确认后 → Blueprint Stage
...
PM 验收 → PMO 完成报告（含 Review Dashboard 全绿确认 + PROJECT.md 更新 + 全景确认 + 知识库总结）→ ✅
```
