...
(分册按需查);v8.331 载体合并:HARD-RULES + backend 评审要点并入 standards/tech-rules.md(三时点唯一必读)—— standards/ 本就是框架级那层,DEV-RULES 模板早写明了这个分工,缺的是**读取路径**。
"""
import unittest
...
class TestHardRulesWhitelist(unittest.TestCase):
    def setUp(self):
        self.t = (STD / "tech-rules.md").read_text(encoding="utf-8")

    def test_compact_enough_to_be_required_reading(self):
        """必读文件必须短 —— 1773 行没法要求必读,~50 行可以。

        上限 85→260(v8.331 用户拍板):HARD-RULES + backend 评审要点合并为 tech-rules
        (三时点唯一必读)—— 消灭「详见分册」二跳;锁的初衷「防膨胀回 1773 行」不变。
        """
        self.assertLess(len(self.t.splitlines()), 260)

    def test_states_union_and_project_precedence(self):
        self.assertIn("同时满足", self.t)          # v8.331 用户拍板措辞
        self.assertIn("冲突部分以项目规范为准", self.t)
        self.assertIn("project-specs/DEV-RULES.md", self.t)   # 标准路径明示
...
class TestReadPathWired(unittest.TestCase):
    ...
    def test_blueprint_and_dev_wired(self):
        # v8.331 三时点:review 也接线(手段菜单 tech-rules 对照行)
        rv = self._rule1("review")
        self.assertIn("standards/tech-rules.md", rv, "review 未接 tech-rules 对照")
        for s in ("blueprint", "dev"):
            t = self._rule1(s)
            self.assertIn("standards/tech-rules.md", t, f"{s} 未接白名单")
...
class TestNoDanglingStandardsLinks(unittest.TestCase):
    ...
    def test_tdd_md_retired(self):
        ...
        h = (STD / "tech-rules.md").read_text(encoding="utf-8")
