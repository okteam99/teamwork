...
### 目标
PM 调研(自答优先)· 起草 PRD · 🔴 **并行派 3 个隔离 Agent 冷审**(QA/Architect/PL · 防鼓掌锚定)· (条件)早问门 · PM 整合修订 · 冷审循环收敛 · 用户确认 · 决策是否需要 UI。\n🔴 v8.215 风险比例化:`state.clarity=explicit`(明确/机械需求)→ 冷审降为 **1 个 QA 边界冷审**(AC 可测性仍值得查)· **PL 质疑跳过**(无产品歧义 · gate 自动放行)· PRD 照写(术语/决策沉淀载体)。
...
def _evidence_pl_challenge_present(state: dict, args) -> tuple[bool, str]:
    ...
    """
    # v8.215 风险比例化:明确需求(explicit)无产品歧义可质疑 → PL 对抗质疑跳过(留痕)
    if state.get("clarity") == "explicit":
        return True, "skipped(clarity=explicit · 明确需求无价值前提可质疑 · v8.215 风险比例化)"
...
🔴 v8.215:`state.clarity=explicit` → **external cross-review 跳过**(gate 自动放行 · 架构师单审)· review 三视角不受影响。\n🔴 **TECH 方案涉及数据库数据结构变更**(新建/删除/修改 表、字段、索引、约束、migration)·
...
def _evidence_external_review_artifact(state: dict, args) -> tuple[bool, str]:
    ...
    """
    # v8.215 风险比例化:blueprint + clarity=explicit → external 跳过(明确/机械方案 · 架构师单审足够)
    # 🔴 只作用 blueprint · review 阶段三视角不受 clarity 影响(明确 ≠ 不会写错 · 拦真主力)
    if (state.get("clarity") == "explicit"
            and state.get("current_stage") == "blueprint"):
        return True, "skipped(clarity=explicit · blueprint external 跳过 · v8.215 风险比例化)"
