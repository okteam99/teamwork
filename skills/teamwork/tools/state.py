...
def cmd_prepare_check(args: argparse.Namespace) -> None:
    ...
    payload["role_value_criteria"] = {
        ...
        "qa": "有边界条件/AC 可测性风险吗?(纯文案/配置 → 弱 · 有状态/并发/输入面 → 强)· goal 默认并入外审覆盖方向「可验证」· 判强才加回独立冷审(v8.243)",
        "architect": "有架构决策/跨模块影响吗?(单文件机械改 → 弱 · 新依赖/改契约/跨层 → 强)· goal 默认并入外审覆盖方向「可实现」· 判强才加回独立冷审(v8.243)",
        "external": "goal 默认在(v8.243 覆盖方向制:可实现/可验证 + AI 自主方向)· 其他 stage:多触发点/高风险/同模型盲区值得第三视角吗?(≥3 模块触发 · 或核心链路)",
